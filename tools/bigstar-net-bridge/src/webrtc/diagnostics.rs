...
impl DiagnosticsReporter {
    ...
    pub(super) fn set_phase(&mut self, phase: &str) {
        ...
        println!("bigstar-net-bridge diagnostics: phase={phase}");
        self.persist();
    }
    ...
    pub(super) fn set_signaling(&mut self, url: &str, session: &str) {
        ...
        println!(
            "bigstar-net-bridge diagnostics: signalingUrl={} session={} role={}",
    ...
    pub(super) fn set_ice_servers(&mut self, servers: Vec<String>, source: &str) {
        ...
        println!(
            "bigstar-net-bridge diagnostics: iceServers source={} values={:?}",
    ...
    pub(super) fn observe_remote_sdp(&mut self, label: &str, sdp: &str) {
        ...
        println!(
            "bigstar-net-bridge diagnostics: remoteSdp={} candidates={}",
            label,
            candidates.len()
        );
    ...
    pub(super) fn observe_event(&mut self, event: &datachannel_wrapper::PeerConnectionEvent) {
        ...
        println!("bigstar-net-bridge webrtc: event {:?}", event);
        match event {
            PeerConnectionEvent::IceCandidate(candidate) => {
                println!(
                    "bigstar-net-bridge webrtc: local candidate type={} mid={} value={}",
    ...
    pub(super) fn observe_selected_pair(&mut self, endpoint: &WebRtcEndpoint) {
        ...
        {
            println!(
                    "bigstar-net-bridge webrtc: selected candidate pair route={} local={} remote={} localAddress={} remoteAddress={}",
    ...
    pub(super) fn persist(&self) {
        ...
        if let Err(error) = write_result {
            eprintln!(
                "bigstar-net-bridge diagnostics: status write failed path={} error={error}",
                path.display()
            );
            return;
        }
        ...
        if let Err(error) = fs::rename(&temp, path) {
            eprintln!(
                "bigstar-net-bridge diagnostics: status rename failed path={} error={error}",
                path.display()
            );
        }
    }
