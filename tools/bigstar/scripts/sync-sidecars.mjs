...
const bridgeTargets = [
  resolve(guiDir, "src-tauri", "target", "release", "bigstar-net-bridge.exe"),
  resolve(
    guiDir,
    "src-tauri",
    "binaries",
    "bigstar-net-bridge-x86_64-pc-windows-msvc.exe",
  ),
];
...
if (process.env.CI === "true" && existsSync(bridgeSource)) {
  console.log(`using CI-provided bigstar-net-bridge sidecar: ${bridgeSource}`);
} else {
  console.log("building bigstar-net-bridge release sidecar...");
  execFileSync(
    "cargo",
    [
      "build",
      ...
      "webrtc",
      "--manifest-path",
      resolve(repoRoot, "tools", "bigstar-net-bridge", "Cargo.toml"),
    ],
    {
      cwd: repoRoot,
      stdio: "inherit",
