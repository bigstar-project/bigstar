...
function parseTauriArgs(args, command) {
  ...
  let edition = process.env.BIGSTAR_EDITION || 'insiders';
  let appVersion = process.env.BIGSTAR_APP_VERSION || null;
  const extraConfigs = [];
...
const child = spawn(
  ...
  [pnpmCli, 'tauri', command, ...tauriArgs],
  {
    ...
    env: {
      ...process.env,
      ...localEnv,
      ...(buildProfile ? { BIGSTAR_BUILD_PROFILE: buildProfile } : {}),
      BIGSTAR_APP_VERSION: resolvedAppVersion,
      BIGSTAR_APP_DATA_DIR_NAME: editionConfig.dataDirectoryName,
      BIGSTAR_DEFAULT_SIGNAL_URL: editionConfig.defaultSignalUrl,
      BIGSTAR_DEV_PORT: String(editionConfig.devPort),
      BIGSTAR_EDITION: edition,
    },
    shell: false,
    stdio: 'inherit',
  },
