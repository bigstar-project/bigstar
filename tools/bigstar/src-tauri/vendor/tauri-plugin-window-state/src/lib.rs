...
impl<R: Runtime> AppHandleExt for tauri::AppHandle<R> {
    fn save_window_state(&self, flags: StateFlags) -> Result<()> {
        ...
        let app_dir = resolve_state_directory(self, plugin_state.state_directory.as_deref())?;
...
impl Builder {
    ...
    /// Sets the directory used to save and restore window state.
    ///
    /// Relative paths are resolved against Tauri's platform data directory.
    /// Absolute paths are used as-is. By default, the plugin uses
    /// [`tauri::path::PathResolver::app_config_dir`].
    pub fn with_state_directory(mut self, directory: impl Into<PathBuf>) -> Self {
        self.state_directory.replace(directory.into());
        self
    }
    ...
    pub fn build<R: Runtime>(self) -> TauriPlugin<R> {
        ...
        PluginBuilder::new("window-state")
            ...
            .setup(move |app, _api| {
                let cache =
                    load_saved_window_states(app, &filename, state_directory.as_deref())
                        .unwrap_or_default();
...
fn load_saved_window_states<R: Runtime>(
    ...
    let app_dir = resolve_state_directory(app, state_directory)?;
...
fn resolve_state_directory<R: Runtime>(
    app: &AppHandle<R>,
    state_directory: Option<&Path>,
) -> Result<PathBuf> {
    match state_directory {
        Some(directory) => Ok(resolve_configured_state_directory(
            &app.path().data_dir()?,
            directory,
        )),
        None => Ok(app.path().app_config_dir()?),
    }
}

fn resolve_configured_state_directory(data_dir: &Path, directory: &Path) -> PathBuf {
...
#[cfg(test)]
mod tests {
    use super::resolve_configured_state_directory;
    use std::path::Path;

    #[test]
    fn relative_state_directory_is_resolved_from_platform_data_directory() {
        assert_eq!(
            resolve_configured_state_directory(Path::new("data"), Path::new("Bigstar Insiders")),
            Path::new("data").join("Bigstar Insiders")
        );
    }

    #[test]
    fn absolute_state_directory_is_preserved() {
        let absolute = if cfg!(windows) {
            Path::new(r"C:\Bigstar")
        } else {
            Path::new("/Bigstar")
        };
        assert_eq!(
            resolve_configured_state_directory(Path::new("data"), absolute),
            absolute
        );
    }
}
