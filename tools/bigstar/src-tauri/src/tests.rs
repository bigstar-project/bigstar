...
fn fake_bridge_smoke_executable(dir: &Path, name: &str, supports_smoke: bool) -> PathBuf {
    #[cfg(windows)]
    {
        ...
        let body = if supports_smoke {
            "@echo off\r\nif \"%1\"==\"webrtc-signaling-udp-pair-smoke\" (\r\n  echo bigstar-net-bridge webrtc: signaling udp pair smoke passed\r\n  exit /b 0\r\n)\r\necho usage\r\nexit /b 0\r\n"
        } else {
...
fn bridge_command_uses_signaling_offer_for_host() {
    ...
    assert_eq!(env_value(&command, "BIGSTAR_DETAILED_LOGS"), None);
...
fn bridge_command_enables_detailed_logs_when_requested() {
    ...
    assert_eq!(
        env_value(&command, "BIGSTAR_DETAILED_LOGS").as_deref(),
        Some("1")
    );
    assert_eq!(
        env_value(&command, "BIGSTAR_BRIDGE_LIVENESS").as_deref(),
        Some("1")
    );
    assert!(env_value(&command, "BIGSTAR_BRIDGE_EVENTS_FILE")
        .as_deref()
...
fn melon_env_carries_rollback_settings_when_enabled() {
    ...
    assert_eq!(env["MELONDS_NSML_ROLLBACK_PHASE_RECOVERY"], "1");
...
fn melon_command_sanitizes_inherited_melonds_environment() {
    ...
    command.env("BIGSTAR_SIGNAL_URL", "ws://127.0.0.1:8787/session");

    remove_inherited_melonds_env_keys(
        &mut command,
        [
            OsString::from("MELONDS_NSML_FORCE_PLAYER_DEATH_COUNTERS"),
            OsString::from("BIGSTAR_SIGNAL_URL"),
        ],
    );
    ...
    assert_eq!(
        env_value(&command, "BIGSTAR_SIGNAL_URL").as_deref(),
...
fn user_log_archive_uses_safe_allowlist() {
    ...
    fs::write(dir.join("bigstar-logs-old.zip"), "old archive").expect("write old archive");
