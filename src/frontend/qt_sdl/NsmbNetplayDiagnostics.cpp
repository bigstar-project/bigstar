...
std::string FormatTestStartupReport(
  ...
  return FormatPrintf(
      ...
      packetBridge.ForceTickStartFrame, packetBridge.MaxFrameLead,
      packetBridge.ThrottleTimeoutMs, packetBridge.ThrottleStartFrame,
      currentStage, currentSceneSettings, mvl.CourseMode.c_str(),
      mvl.BigStarTarget);
}
...
std::string FormatNetplayStartupReport(
  ...
  return FormatPrintf(
      ...
      packetBridge.ForceTickEnabled ? 1 : 0,
      packetBridge.ForceTickStartFrame, packetBridge.MaxFrameLead,
      packetBridge.ThrottleTimeoutMs,
      ...
      rollback.RestoreProbe ? 1 : 0, rollback.PredictionProbeModulo,
       ...
       rollback.PredictionHorizonTimeoutMs,
       rollback.PhaseRecoveryEnabled ? 1 : 0, mvl.MatchSeed,
