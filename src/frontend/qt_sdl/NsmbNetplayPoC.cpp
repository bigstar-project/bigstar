...
struct WireGameState
{
    ...
    melonDS::u32 Player0Powerup;
    melonDS::u32 Player1Powerup;
    melonDS::u32 Player0InventoryPowerup;
    melonDS::u32 Player1InventoryPowerup;
    melonDS::u32 Player0Dead;
    melonDS::u32 Player1Dead;
    melonDS::u32 Player0Character;
    melonDS::u32 Player1Character;
    melonDS::u32 PlayerTransitionStatus0;
    melonDS::u32 PlayerTransitionStatus1;
    melonDS::u32 Player0Lives;
    melonDS::u32 Player1Lives;
...
struct ObjectScanSample
{
    ...
    melonDS::u32 Extra154 = 0;
    melonDS::u32 Extra158 = 0;
};
...
struct State
{
    ...
    int PlayerStateMaxStaleTransformFrames = 0;
    ...
    bool RollbackNSMBProcessListMetadataRanges = false;
    ...
    melonDS::u32 LastSpawnedMovingHazardRemoteGUIDs[16][kMaxWorldMovingHazards] {};
    melonDS::u32 PendingMovingHazardRemoteGUIDs[16][kMaxWorldMovingHazards] {};
    melonDS::u32 PendingMovingHazardFirstMissingFrames[16][kMaxWorldMovingHazards] {};
    melonDS::u32 LastWorldMovingHazardLocalBases[16][kMaxWorldMovingHazards] {};
    ...
    melonDS::u32 PendingWorldActorSnapshotRemoteGUIDs[16][kMaxWorldActorSnapshots] {};
...
void ResetMvlRuntimeSyncStateForRestart(int instanceID, melonDS::u32 frame)
{
    ...
    std::fill(std::begin(G.LastSpawnedMovingHazardRemoteGUIDs[instanceID]), std::end(G.LastSpawnedMovingHazardRemoteGUIDs[instanceID]), 0);
    std::fill(std::begin(G.PendingMovingHazardRemoteGUIDs[instanceID]), std::end(G.PendingMovingHazardRemoteGUIDs[instanceID]), 0);
    std::fill(std::begin(G.PendingMovingHazardFirstMissingFrames[instanceID]), std::end(G.PendingMovingHazardFirstMissingFrames[instanceID]), 0);
    std::fill(std::begin(G.LastWorldMovingHazardLocalBases[instanceID]), std::end(G.LastWorldMovingHazardLocalBases[instanceID]), 0);
    ...
    std::fill(std::begin(G.PendingWorldActorSnapshotRemoteGUIDs[instanceID]), std::end(G.PendingWorldActorSnapshotRemoteGUIDs[instanceID]), 0);
        ...
        case ENET_EVENT_TYPE_RECEIVE:
            ...
            else if (event.packet->dataLength == sizeof(WireGameState))
            {
                ...
                if (packet.Magic == kMagic && packet.Version == kVersion && packet.Kind == kWireKindState)
                {
                    ...
                    sample.Player0Powerup = packet.Player0Powerup;
                    sample.Player1Powerup = packet.Player1Powerup;
                    sample.Player0InventoryPowerup = packet.Player0InventoryPowerup;
                    sample.Player1InventoryPowerup = packet.Player1InventoryPowerup;
                    sample.Player0Dead = packet.Player0Dead;
                    sample.Player1Dead = packet.Player1Dead;
                    sample.Player0Character = packet.Player0Character;
                    sample.Player1Character = packet.Player1Character;
                    sample.PlayerTransitionStatus0 = packet.PlayerTransitionStatus0;
                    sample.PlayerTransitionStatus1 = packet.PlayerTransitionStatus1;
                    sample.Player0Lives = packet.Player0Lives;
                    sample.Player1Lives = packet.Player1Lives;
...
bool IsRollbackNSMBRangeBackend()
{
    return G.RollbackBackendMode == RollbackBackend::NSMBRanges
...
void AddNSMBRollbackExtraAddressRanges(melonDS::NDS* nds, std::vector<RollbackNSMBRangeEntry>& ranges)
{
    const char* spec = std::getenv("MELONDS_NSML_ROLLBACK_NSMB_EXTRA_RANGES");
    if (!spec || !spec[0])
        return;
    ...
    while (*p)
    {
        ...
        else if (*p == '-')
        {
            ...
            if (end == p)
                break;
            const melonDS::u32 endAddress = static_cast<melonDS::u32>(endParsed);
            const melonDS::u32 start = static_cast<melonDS::u32>(startParsed);
            rangeLength = endAddress > start ? endAddress - start : 0;
            p = end;
        }

        AddNSMBRollbackRange(nds, ranges, static_cast<melonDS::u32>(startParsed), rangeLength);
    ...
    melonDS::u16 objectID)
{
    ...
    {
        if (G.RollbackNSMBFullPlayerObjectWithSafeRanges)
        {
            AddNSMBRollbackObjectRange(nds, ranges, base, NSMBRollbackObjectSnapshotLength(objectID));
            return true;
        }
...
{
    if (G.RollbackNSMBProcessListMetadataRanges)
        AddNSMBRollbackRange(nds, ranges, listAddress, 0x08);
    ...
    for (int i = 0; i < 512 && IsValidMainRAMRange(nds, node, 0x0C); i++)
    {
        ...
        if (G.RollbackNSMBProcessListMetadataRanges)
            AddNSMBRollbackRange(nds, ranges, node, 0x0C);
...
std::vector<RollbackNSMBRangeEntry> BuildNSMBRollbackRanges(melonDS::NDS* nds)
{
    ...
    AddNSMBRollbackExtraAddressRanges(nds, ranges);
...
bool PrepareRollbackDeltaSaveLocked(melonDS::u32 frame, RollbackStoredState& checkpoint, std::vector<melonDS::u8>& baseMainRAM)
{
    ...
    if (G.RollbackBackendMode != RollbackBackend::CoreDelta
        && G.RollbackBackendMode != RollbackBackend::CoreFrameDelta
        && G.RollbackBackendMode != RollbackBackend::TinyCoreRAMDelta
        && !(G.RollbackDeltaPageTrace && IsRollbackNSMBRangeBackend())
        && !IsRollbackPreimageBackend())
        return true;

    if (G.RollbackDeltaPageTrace && IsRollbackNSMBRangeBackend())
    {
...
void SaveRollbackCheckpointIfNeeded(int instanceID, melonDS::u32 frame, melonDS::NDS* nds)
{
    if (!G.RollbackEnabled || !G.InputNetplayOnly || !nds)
        return;
    if (!G.RollbackResimulate && !G.RollbackRestoreProbe)
        return;
    if (instanceID < 0 || instanceID >= 16)
        return;
    if (G.NetplayStartFrame != 0 && frame < G.NetplayStartFrame)
...
bool ReadPlayerActorByBase(melonDS::NDS* nds, melonDS::u32 base, melonDS::u32 expectedGUID, ObjectScanSample& actor)
{
    ...
    actor.Flags = flags;
    ReadObjectTransform(nds, off, actor);
    ReadMainRAMU32(nds, off + 0x154, actor.Extra154);
    ReadMainRAMU32(nds, off + 0x158, actor.Extra158);
    return true;
}
...
void AppendWirePlayerStateJson(std::ostream& out, const WirePlayerState& state, melonDS::u32 sampleFrame)
{
    out << "{";
    out << "\"sampleFrame\":" << sampleFrame << ",";
    out << "\"packetFrame\":" << state.Frame << ",";
    out << "\"player\":" << state.Player << ",";
    out << "\"found\":" << state.Found << ",";
    out << "\"guid\":" << state.GUID << ",";
    AppendJsonHex32(out, "settings", state.Settings); out << ",";
    out << "\"stateType\":" << state.StateType << ",";
    AppendJsonHex32(out, "flags", state.Flags); out << ",";
    AppendJsonHex32(out, "x", state.PosX); out << ",";
    AppendJsonHex32(out, "y", state.PosY); out << ",";
    AppendJsonHex32(out, "z", state.PosZ); out << ",";
    AppendJsonHex32(out, "prevX", state.PrevX); out << ",";
    AppendJsonHex32(out, "prevY", state.PrevY); out << ",";
    AppendJsonHex32(out, "velX", state.VelX); out << ",";
    AppendJsonHex32(out, "velY", state.VelY); out << ",";
    AppendJsonHex32(out, "action", state.ActionFlag); out << ",";
    AppendJsonHex32(out, "subAction", state.SubActionFlag); out << ",";
    AppendJsonHex32(out, "physics", state.PhysicsFlag); out << ",";
    AppendJsonHex32(out, "damageCooldown", state.DamageCooldown); out << ",";
    AppendJsonHex32(out, "transitionFlag", state.TransitionFlag); out << ",";
    AppendJsonHex32(out, "collisionFlag", state.CollisionFlag); out << ",";
    AppendJsonHex32(out, "environmentFlag", state.EnvironmentFlag); out << ",";
    out << "\"runtimeFlags0\":" << state.RuntimeFlags0 << ",";
    out << "\"runtimeFlags1\":" << state.RuntimeFlags1 << ",";
    out << "\"playerCount\":" << state.PlayerCount << ",";
    out << "\"powerup\":" << state.Powerup << ",";
    out << "\"inventoryPowerup\":" << state.InventoryPowerup << ",";
    out << "\"dead\":" << state.Dead << ",";
    out << "\"character\":" << state.Character << ",";
    out << "\"transitionStatus\":" << state.TransitionStatus << ",";
    out << "\"lives\":" << state.Lives << ",";
    out << "\"battleStars\":" << state.BattleStars << ",";
    out << "\"coins\":" << state.Coins << ",";
    out << "\"score\":" << state.Score << ",";
    out << "\"displayedStars\":" << state.DisplayedStars << ",";
    out << "\"deaths\":" << state.Deaths << ",";
    out << "\"collectedStars\":" << state.CollectedStars;
    out << "}";
}
    ...
    const GameStateSyncHashes& remote)
{
    ...
    const bool wifiCandidateMatches = local.WifiCandidate == remote.WifiCandidate;
    const bool renderCandidateMatches = local.RenderCandidate == remote.RenderCandidate;
    if (basicMatches && playerGlobalMatches && wifiCandidateMatches && renderCandidateMatches)
        return;
    ...
    std::ostringstream json;
    json << "{\"event\":\"" << (playerGlobalMatches ? "game_state_mismatch" : "player_global_mismatch") << "\","
         << "\"role\":\"" << (G.NetRole == Role::Host ? "host" : "client") << "\","
         << "\"instance\":" << instanceID << ","
         << "\"frame\":" << frame << ",";
    json << "\"basicMatches\":" << (basicMatches ? "true" : "false") << ",";
    json << "\"playerGlobalMatches\":" << (playerGlobalMatches ? "true" : "false") << ",";
    json << "\"wifiCandidateMatches\":" << (wifiCandidateMatches ? "true" : "false") << ",";
    json << "\"renderCandidateMatches\":" << (renderCandidateMatches ? "true" : "false") << ",";
    ...
    for (melonDS::u32 player = 0; player < 2; player++)
    {
        ...
        if (FindLatestRemotePlayerStateLocked(player, frame, state, sampleFrame))
            AppendWirePlayerStateJson(json, state, sampleFrame);
        else
            json << "{\"player\":" << player << ",\"sampleFrame\":null}";
    }
    json << "],";
    if (latest)
    {
...
void AppendNearbyHazardsJson(std::ostream& out, melonDS::NDS* nds)
{
    ...
    const std::vector<ObjectScanSample> hazards =
        FindActiveObjectsByIDAndSettings(nds, kVsMovingHazardObjectID, kVsMovingHazardSettings, true);
...
std::vector<ObjectScanSample> GetWorldMovingHazardsCached(int instanceID, melonDS::u32 frame, melonDS::NDS* nds)
{
    ...
    if (!periodicRescan)
    {
        ...
        for (melonDS::u32 i = 0; i < count; i++)
        {
            ...
            if (!ReadObjectByBase(
                    ...
                    actor) ||
                actor.StateType != 1 && actor.StateType != 2)
            {
                valid = false;
                break;
            }
    ...
    actors = FindActiveObjectsByIDAndSettings(nds, kVsMovingHazardObjectID, kVsMovingHazardSettings, true);
...
WorldActorSnapshotCandidate FindDormantWorldActorSnapshotCandidate(
    melonDS::NDS* nds,
    melonDS::u16 objectID,
    melonDS::u32 settings)
{
    WorldActorSnapshotCandidate result {};
    const GameStateObjectScanCache cache = BuildGameStateObjectScanCache(nds);
    for (const GameStateObjectScanEntry& entry : cache.Entries)
    {
        if (entry.ObjectID != objectID || entry.Actor.Settings != settings)
            continue;
        if (entry.LifecycleState != 0 && entry.Actor.StateType != 0)
            continue;
        if (entry.Actor.Flags >= 0x10000000)
            continue;
...
bool IsWorldActorHeaderAtBase(
    ...
    melonDS::u32 expectedSettings)
{
    if (!IsValidMainRAMRange(nds, base, 0x14))
        return false;

    melonDS::u16 objectID = 0;
    melonDS::u32 settings = 0;
    if (!ReadMainRAMAddressU16(nds, base + 0x0C, objectID) ||
        !ReadMainRAMAddressU32(nds, base + 0x08, settings))
        return false;
    return objectID == expectedObjectID && settings == expectedSettings;
}
...
melonDS::u32 FindBestLastMovingHazardBase(int instanceID, melonDS::NDS* nds, const WireWorldActorState& remoteActor)
{
    ...
    melonDS::u32 bestBase = 0;
    melonDS::u64 bestDistance = std::numeric_limits<melonDS::u64>::max();
    for (std::size_t i = 0; i < kMaxWorldMovingHazards; i++)
    {
        const melonDS::u32 base = G.LastWorldMovingHazardLocalBases[instanceID][i];
        if (base == 0 ||
            !IsWorldActorHeaderAtBase(nds, base, kVsMovingHazardObjectID, remoteActor.Settings))
            continue;

        ObjectScanSample localActor {};
        localActor.Found = 1;
        localActor.Base = base;
        localActor.Settings = remoteActor.Settings;
        ReadObjectTransform(nds, base - kMainRAMBase, localActor);
        const melonDS::u64 distance = WorldMovingHazardMatchDistance(remoteActor, localActor);
...
melonDS::u32 WorldStateMovingHazardPredictLimit(melonDS::u32 frame)
{
    if (G.WorldStateMovingHazardMaxPredictFrames < 0)
        return static_cast<melonDS::u32>(std::max(0, G.WorldStateMaxPredictFrames));

    if (G.WorldStateMovingHazardMaxPredictEndOffset >= 0 &&
        frame > G.NetplayStartFrame + static_cast<melonDS::u32>(G.WorldStateMovingHazardMaxPredictEndOffset))
        return static_cast<melonDS::u32>(std::max(0, G.WorldStateMaxPredictFrames));
...
void ApplyRemoteWorldActorSnapshotState(int instanceID, melonDS::u32 frame, melonDS::NDS* nds)
{
    ...
    for (std::size_t i = 0; i < remoteCount; i++)
    {
        ...
        if (localIndices[i] < 0)
        {
            if ((G.WorldStateActivateDormantActorSnapshotStarCandidate ||
                    G.WorldStateSpawnActorSnapshotStarCandidate) &&
                remoteActor.ObjectID == kVsBattleStarCandidateObjectID &&
                remoteActor.Actor.Found &&
                (remoteActor.Actor.StateType == 1 || remoteActor.Actor.StateType == 2))
            {
                melonDS::u32& pendingGUID = G.PendingWorldActorSnapshotRemoteGUIDs[instanceID][i];
                melonDS::u32& pendingFrame = G.PendingWorldActorSnapshotFirstMissingFrames[instanceID][i];
                melonDS::u32& lastSpawnedGUID = G.LastSpawnedWorldActorSnapshotRemoteGUIDs[instanceID][i];
                if (pendingGUID != remoteActor.Actor.GUID)
                {
                    pendingGUID = remoteActor.Actor.GUID;
                    pendingFrame = frame;
                }

                if (lastSpawnedGUID != remoteActor.Actor.GUID &&
                    frame - pendingFrame >= kVsWorldItemNaturalSpawnGraceFrames)
                {
                    if (G.WorldStateActivateDormantActorSnapshotStarCandidate)
                    {
                        const WorldActorSnapshotCandidate dormantActor =
                            FindDormantWorldActorSnapshotCandidate(
                                nds,
                                static_cast<melonDS::u16>(remoteActor.ObjectID),
                                remoteActor.Actor.Settings);
                        if (dormantActor.Actor.Found &&
                            ApplyWireWorldActorLifecycleState(nds, remoteActor.Actor, dormantActor.Actor.Base) &&
                            ApplyWireWorldActorState(nds, remoteActor.Actor, predictFrames, dormantActor.Actor.Base))
                        {
                            lastSpawnedGUID = remoteActor.Actor.GUID;
                            activated++;
                            nextRemoteGUIDs[i] = remoteActor.Actor.GUID;
                            nextLocalGUIDs[i] = dormantActor.Actor.GUID;
                            mapChanged = true;
                        }
                    }
        ...
        G.PendingWorldActorSnapshotRemoteGUIDs[instanceID][i] = 0;
        G.PendingWorldActorSnapshotFirstMissingFrames[instanceID][i] = 0;
        G.LastSpawnedWorldActorSnapshotRemoteGUIDs[instanceID][i] = remoteActor.Actor.GUID;
    ...
    if ((G.InputTraceEnabled || G.InputNetplayTraceEnabled) &&
        (G.InputTraceInterval <= 1 || (frame % static_cast<melonDS::u32>(G.InputTraceInterval)) == 0))
    {
        std::printf("NSMB WorldActors: apply inst=%d frame=%u sampleFrame=%u count=%zu applied=%u pruned=%u spawned=%u activated=%u predict=%u mapChanged=%d\n",
            instanceID,
            frame,
            sample.Frame,
            remoteCount,
    ...
    const char* label)
{
    ...
    std::printf(
        "NSMB %s: spawn inst=%d frame=%u object=%03X remoteGuid=%u settings=%08X pos=%08X/%08X/%08X trampoline=%08X return=%08X\n",
        label,
        instanceID,
        frame,
        objectID,
...
bool SpawnRemoteWorldItem(
    ...
    const WireWorldActorState& state)
{
    return SpawnRemoteWorldActor(instanceID, frame, nds, state, kVsWorldItemObjectID, "WorldItem");
}
...
void ApplyRemoteWorldState(int instanceID, melonDS::u32 frame, melonDS::NDS* nds)
{
    ...
    bool movingHazardApplied = false;
    if (G.WorldStateApplyMovingHazard && applyHostAuthoritativeWorld && sample.MovingHazard.Found)
    {
        const ObjectScanSample localHazard = FindNewestActiveObjectByIDAndSettings(
            nds,
            kVsMovingHazardObjectID,
            kVsMovingHazardSettings,
            true);
        if (localHazard.Found)
            movingHazardApplied = ApplyWireWorldActorState(
                nds,
                sample.MovingHazard,
                movingHazardPredictFrames,
                localHazard.Base);
    }
    ...
    if ((G.InputTraceEnabled || G.InputNetplayTraceEnabled) &&
        (G.InputTraceInterval <= 1 || (frame % static_cast<melonDS::u32>(G.InputTraceInterval)) == 0))
    {
        std::printf("NSMB WorldState: apply inst=%d frame=%u sampleFrame=%u predict=%u hazardPredict=%u star=%d neutralItem=%d neutralSpawn=%d item=%d itemSpawn=%d droppedItem=%d droppedSpawn=%d hazard=%d hazardPos=%08X/%08X\n",
            instanceID,
            frame,
            sample.Frame,
            predictFrames,
...
void ApplyRemoteMovingHazardState(int instanceID, melonDS::u32 frame, melonDS::NDS* nds, bool preferFreshSample = false)
{
    ...
    for (std::size_t i = 0; i < pairCount; i++)
    {
        ...
        if (localIndices[i] < 0)
        {
            ...
            {
                melonDS::u32& pendingGUID = G.PendingMovingHazardRemoteGUIDs[instanceID][i];
                melonDS::u32& pendingFrame = G.PendingMovingHazardFirstMissingFrames[instanceID][i];
                melonDS::u32& lastSpawnedGUID = G.LastSpawnedMovingHazardRemoteGUIDs[instanceID][i];
                if (pendingGUID != remoteActor.GUID)
                {
                    pendingGUID = remoteActor.GUID;
                    pendingFrame = frame;
                }
                    ...
                    if (G.WorldStateActivateDormantMovingHazard)
                    {
                        const WorldActorSnapshotCandidate dormantActor =
                            FindDormantWorldActorSnapshotCandidate(
                                ...
                                remoteActor.Settings);
                        if (dormantActor.Actor.Found &&
                            ApplyWireWorldMovingHazardState(
                                nds,
                                remoteActor,
                                predictFrames,
                                dormantActor.Actor.Base))
                        {
                            if (G.WorldStateClearMovingHazardLinkFields)
                            {
        ...
        G.LastWorldMovingHazardLocalBases[instanceID][i] = localActor.Base;
        G.PendingMovingHazardRemoteGUIDs[instanceID][i] = 0;
        G.PendingMovingHazardFirstMissingFrames[instanceID][i] = 0;
        G.LastSpawnedMovingHazardRemoteGUIDs[instanceID][i] = remoteActor.GUID;
        mapChanged = mapChanged ||
    ...
    if ((mapChanged || spawned > 0 || activated > 0 || restored > 0) && G.WorldStateTraceMovingHazards)
    {
        std::printf("NSMB WorldHazards: map inst=%d frame=%u remote=%zu local=%zu applied=%u spawned=%u activated=%u restored=%u",
            instanceID,
            frame,
            remoteCount,
            localActors.size(),
...
bool WritePlayerRuntimeStateByBase(melonDS::NDS* nds, melonDS::u32 base, const WirePlayerState& state)
{
    ...
    nds->ARM9Write8(base + kPlayerBaseTransitionStepOffset, static_cast<melonDS::u8>((state.RuntimeFlags1 >> 24) & 0xFFu));
    return true;
}
...
bool WritePlayerMinimalTransitionStateByBase(melonDS::NDS* nds, melonDS::u32 base, const WirePlayerState& state)
{
    ...
    ok = WriteMainRAMAddrU8IfChanged(
        nds,
        base + kPlayerBaseTransitionStepOffset,
        static_cast<melonDS::u8>((state.RuntimeFlags1 >> 24) & 0xFFu)) && ok;
    return ok;
}
...
void ApplyRemotePlayerState(int instanceID, melonDS::u32 frame, melonDS::NDS* nds, bool preferFreshSample = false)
{
    ...
    const auto waitDeadline = std::chrono::steady_clock::now() +
        std::chrono::microseconds(std::max(0, G.PlayerStateFreshWaitUs));
    {
        while (true)
        {
            {
    ...
    const bool allowStaleGlobalsOnly =
        staleFreshSample &&
        G.PlayerStateGlobalsEnabled &&
        G.PlayerStateMaxStaleGlobalFrames > 0 &&
        staleFrames <= static_cast<melonDS::u32>(G.PlayerStateMaxStaleGlobalFrames);
    const bool allowStaleCountersOnly =
        staleFreshSample &&
        G.PlayerStateGlobalsEnabled &&
        G.PlayerStateMaxStaleCounterFrames > 0 &&
        staleFrames <= static_cast<melonDS::u32>(G.PlayerStateMaxStaleCounterFrames);
    const bool allowStaleTransform =
        staleFreshSample &&
        G.PlayerStateMaxStaleTransformFrames > 0 &&
        staleFrames <= static_cast<melonDS::u32>(G.PlayerStateMaxStaleTransformFrames);
    ...
    if ((allowStaleGlobalsOnly || allowStaleCountersOnly) && !allowStaleTransform)
    {
        if ((G.InputTraceEnabled || G.InputNetplayTraceEnabled) &&
            (G.InputTraceInterval <= 1 || (frame % static_cast<melonDS::u32>(G.InputTraceInterval)) == 0))
        {
            std::printf("NSMB PlayerState: apply stale-globals inst=%d frame=%u sampleFrame=%u remotePlayer=%d stale=%u countersOnly=%d globals=%d lives=%u deaths=%u dead=%u stars=%u\n",
                instanceID,
                frame,
                sampleFrame,
                remotePlayer,
                staleFrames,
                (allowStaleCountersOnly && !allowStaleGlobalsOnly) ? 1 : 0,
                globalsApplied ? 1 : 0,
                sample.Lives,
                sample.Deaths,
                sample.Dead,
...
void ApplyRemoteGameState(int instanceID, melonDS::u32 frame, melonDS::NDS* nds)
{
    ...
    if (G.GameStateApplyCriticalGlobals)
    {
        ...
        nds->ARM9Write8(kGamePlayerPowerupAddr, static_cast<melonDS::u8>(sample.Player0Powerup & 0xFFu));
        nds->ARM9Write8(kGamePlayerPowerupAddr + 1, static_cast<melonDS::u8>(sample.Player1Powerup & 0xFFu));
        nds->ARM9Write8(kGamePlayerInventoryPowerupAddr, static_cast<melonDS::u8>(sample.Player0InventoryPowerup & 0xFFu));
        nds->ARM9Write8(kGamePlayerInventoryPowerupAddr + 1, static_cast<melonDS::u8>(sample.Player1InventoryPowerup & 0xFFu));
        nds->ARM9Write8(kGamePlayerDeadAddr, static_cast<melonDS::u8>(sample.Player0Dead & 0xFFu));
        nds->ARM9Write8(kGamePlayerDeadAddr + 1, static_cast<melonDS::u8>(sample.Player1Dead & 0xFFu));
        nds->ARM9Write8(kGamePlayerCharacterAddr, static_cast<melonDS::u8>(sample.Player0Character & 0xFFu));
        nds->ARM9Write8(kGamePlayerCharacterAddr + 1, static_cast<melonDS::u8>(sample.Player1Character & 0xFFu));
        nds->ARM9Write32(kGamePlayerTransitionStatusAddr, sample.PlayerTransitionStatus0);
        nds->ARM9Write32(kGamePlayerTransitionStatusAddr + sizeof(melonDS::u32), sample.PlayerTransitionStatus1);
        nds->ARM9Write32(kGamePlayerLivesAddr, sample.Player0Lives);
        nds->ARM9Write32(kGamePlayerLivesAddr + sizeof(melonDS::u32), sample.Player1Lives);
    ...
    if (G.GameStateApplyPlayerActors)
    {
        const PlayerActorScanSample localPlayers = FindPlayerActors(nds);
...
GameStateSample ReadGameStateSample(melonDS::NDS* nds)
{
    ...
    const ObjectScanSample movingHazard =
        FindNewestActiveObjectByIDAndSettings(nds, kVsMovingHazardObjectID, kVsMovingHazardSettings, true);
...
void SyncGameState(int instanceID, melonDS::u32 frame, melonDS::NDS* nds)
{
    ...
    packet.Player0Powerup = sample.Player0Powerup;
    packet.Player1Powerup = sample.Player1Powerup;
    packet.Player0InventoryPowerup = sample.Player0InventoryPowerup;
    packet.Player1InventoryPowerup = sample.Player1InventoryPowerup;
    packet.Player0Dead = sample.Player0Dead;
    packet.Player1Dead = sample.Player1Dead;
    packet.Player0Character = sample.Player0Character;
    packet.Player1Character = sample.Player1Character;
    packet.PlayerTransitionStatus0 = sample.PlayerTransitionStatus0;
    packet.PlayerTransitionStatus1 = sample.PlayerTransitionStatus1;
    packet.Player0Lives = sample.Player0Lives;
    packet.Player1Lives = sample.Player1Lives;
...
void SyncWorldState(int instanceID, melonDS::u32 frame, melonDS::NDS* nds)
{
    ...
    ObjectScanSample movingHazard;
    if (G.WorldStateApplyMovingHazard)
    {
        movingHazard = FindNewestActiveObjectByIDAndSettings(
            nds,
            kVsMovingHazardObjectID,
            kVsMovingHazardSettings,
            true);
    ...
    FillWireWorldActorState(movingHazard, packet.MovingHazard);
...
void InitFromEnvironment()
{
    ...
    if (const char* applyMode = std::getenv("MELONDS_NSML_STATE_APPLY_MODE"))
    {
        ...
        else if (!std::strcmp(applyMode, "host-player-globals"))
        {
            G.GameStateApplyCriticalGlobals = true;
            G.GameStateApplyStarObjects = false;
            G.GameStateApplyStageObjects = false;
    ...
    G.PlayerStateMaxStaleTransformFrames = std::max(0, EnvInt("MELONDS_NSML_PLAYER_STATE_MAX_STALE_TRANSFORM_FRAMES", 0));
    ...
    G.WorldStateMovingHazardMaxPredictFrames =
        EnvInt("MELONDS_NSML_WORLD_STATE_MOVING_HAZARD_MAX_PREDICT_FRAMES", -1);
    if (G.WorldStateMovingHazardMaxPredictFrames < -1)
        G.WorldStateMovingHazardMaxPredictFrames = -1;
    G.WorldStateMovingHazardMaxPredictEndOffset =
    ...
    G.RollbackNSMBProcessListMetadataRanges =
        EnvFlag("MELONDS_NSML_ROLLBACK_NSMB_PROCESS_LIST_METADATA_RANGES");
    ...
    if (G.TestEnabled)
    {
        ...
        std::printf("NSMB Test: enabled frames=%u instances=%d frameBarrier=%d serialRun=%d input=%s hashLog=%s interval=%d screenshotDir=%s screenshotInterval=%d ramDumpDir=%s ramDumpInterval=%d ramDumpRanges=%zu gameStateTrace=%s gameStateTraceInterval=%d stateSync=%d stateApply=%d stateSyncInterval=%d playerStateSync=%d playerStateApply=%d playerStateGlobals=%d playerStateTransitionTransform=%d playerStateTransitionTransformStartOffset=%d playerStateInterval=%d playerStatePredict=%d playerStateFreshWaitUs=%d playerStateStaleGlobals=%d playerStateStaleCounters=%d playerStateStaleTransform=%d memPatchFile=%s memPatchFrame=%u memPatchRanges=%zu netRandomEnabled=%d netRandomAuto=%d netRandomFrame=%u netRandomValue=0x%08X stateSaveDir=%s stateSaveFrame=%u stateLoadDir=%s stateLoadFrame=%u waitTimeoutMs=%d quitGraceMs=%d inputTrace=%d inputTraceInterval=%d seedWaitMs=%d waitForPeer=%d waitForPeerAtStart=%d deferNetworkUntilStart=%d netplayFrameBarrier=%d packetBridge=%d packetBridgeOnly=%d packetBridgePreGame=%d packetBridgeTrace=%d packetBridgeWait=%d packetBridgeWaitMs=%d packetBridgeWaitStart=%u packetBridgeWaitAhead=%d packetBridgeDirect=%d packetBridgeForceTick=%d packetBridgeForceTickStart=%u packetBridgeMaxTickLead=%d packetBridgeMaxFrameLead=%d packetBridgeThrottleMs=%d packetBridgeThrottleStart=%u directBoot=%d directBootFrame=%u directBootScene=%d directBootStage=%d directBootPlayerID=%d directBootLoadSM=%d directBootPatchLoadSMOnly=%d directBootCallUpdateSM=%d mvlSceneSettings=0x%08X mvlCourseMode=%s mvlBigStarTarget=%d\n",
            ...
            G.PlayerStateMaxStaleTransformFrames,
