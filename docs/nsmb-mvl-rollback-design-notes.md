...
- Added compact multi-instance moving-hazard correction:
  - Host sends up to four active `0x0053 settings=0` actors ordered by creation GUID. Client aligns the newest actors by order and only applies when host/client active counts match, avoiding replacement-boundary corruption.
...
- Added diagnostic-only actor lifecycle tracing:
  - `MELONDS_NSML_WORLD_STATE_TRACE_OBJECT_LIFECYCLES=1` scans Main RAM for active/dead actor records and prints GUID, object ID, settings, lifecycle state, type, flags, vtable, base, and `PosX/Y/Z`.
  - The LAN and split smoke scripts expose `-WorldStateTraceObjectLifecycles`, interval, start-frame, and end-frame parameters.
  - This full-RAM scanner is intentionally not enabled by `-PlanDActorSnapshot`; dense 2-frame scans raise active averages to about `18-19ms`.
...
- Added item-specific lightweight world replication:
  ...
  - Normal runtime does not enable the diagnostic full-RAM lifecycle scanner. The split wrapper exposes `-WorldStateSpawnItem` and the lightweight `-RequireWorldItemSpawn` gate.
...
- Reduced performance-observer interference in strict automation:
  - `ReadGameStateSample()` used to repeat full Main RAM actor scans for each queried actor. It now builds one scoped plausible-actor cache per sample and reuses it for player, star, stage, MvL object, and lifecycle queries. The cache is active only while producing the diagnostic CSV; the normal Plan-D actor/world snapshot path is unchanged.
  - Added `NSMB AfterHookPhaseSpike` for strict runs. It splits `AfterRunFrame()` into init, pre-snapshot, actor apply, trace, and sync costs.
  ...
  - Stall detection now writes and flushes a tiny dedicated heartbeat file every `120` frames by default. The heartbeat line remains in stdout, but no longer flushes all pending diagnostic output.
  - Rejected intermediate measurements exposed observer costs correctly: `logs/codex-pland-singlescan-filtered-result-restart-trace30-6200-20260602` still hit `afterHook=310-312ms`, while `logs/codex-pland-heartbeatfile-repeat-3600-20260602` passed after the heartbeat split with max `69.977/72.892ms`.
  - Final repeated result/restart validation passed: `logs/codex-pland-final-result-restart-6200-20260602` reached the second game, kept Big Star drift `0/0`, moving-hazard max drift `4096/0`, and passed the 34-field manager/global gate. Active avg was `17.148/17.148ms`, max `48.995/48.605ms`, and max consecutive slow frames `1/1`.
...
- Coin-sync result/restart regression also detected a paired-process stall: `logs/codex-pland-coinsync-result-restart-6200-20260602` reached `750.360/753.504ms` at frame `2971` and `221.748/225.920ms` at frame `3361`, with rollback restores/resims still `0/0`. The immediate identical rerun passed. The spike gate is working; the remaining shared-process stall source is still unresolved.
- Item-sync stage-matrix stress also detected the same class of occasional paired-process stall: `logs/codex-pland-itemspawn-stage0-generated-stress-2400-20260602` reached `481.468/493.584ms` at frame `1745`, with rollback restores/resims still `0/0`. The immediate rerun passed, and the post-dedup course `0` run also passed.
...
- Keep the phase trace and dedicated heartbeat stall detector enabled in automation. Remaining occasional single-frame costs are mostly emulator `runFrame` or peer-wait-sensitive `beforeHook`; use `NSMB AfterHookPhaseSpike` when an `afterHook` regression reappears.
