...
**Current blocker:** selective LCD-event deferral now keeps the tested depth-4 transaction inside one emulated display frame, but it still takes `24.597/23.463ms` on the two local peers and the corrected same-role image does not converge. The depth-1 version stayed around `12.6-13.7ms` in seven of eight role samples but produced one `22.605ms` outlier, so it is not yet a repeatable 60 FPS path. The depth-4 endpoint and one-tick recovery images differed by `762` and `322` of `98,304` pixels respectively. These failures take precedence over production integration.

**Next action:** keep the LCD deferral diagnostic-only and identify the missing display latch/buffer sequence that leaves character/lower-screen pixels stale, while separately reducing depth-1 wall-time variance. A useful next visual experiment is to advance only the minimum LCD/GPU presentation events after the final render, without executing another gameplay update; the current blanket LCD pause proves the one-frame boundary is attainable but not that its framebuffer is correct. The gameplay block profile rejects broad process-list or render-cache skipping: collision and arithmetic dominate and are gameplay-critical. A replacement must keep the corrected endpoint inside one presentation interval with repeatable margin and make the final same-role image converge. Only after those gates pass should `InputTimeline::ResolveReplayFrameInputs()` be serialized into the guest table or a real mismatch restore be connected.
...
Four symmetric depth-1 repetitions under `logs/slippi-defer-lcd-depth1-performance-run{1,2,3,4}` all used one display frame. Seven of eight role samples were `12.604-13.692ms`, but one client sample was `22.605ms`. The median has useful headroom, yet the outlier prevents a stable-60-FPS claim. LCD deferral remains an opt-in diagnostic because neither the image gate nor repeatable wall-time gate passes.
