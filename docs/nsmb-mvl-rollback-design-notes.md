...
This section records the 2026-06-02 Plan-D experiment. It was not promoted to the production correctness route. As of 2026-07-15, the unused item-specific client-spawn, generic actor snapshot, world effect snapshot, and supplemental player snapshot branches described below have been removed. The remaining Big Star and moving-hazard correction packets also have no GUI/normal-fast consumer; a matched 1,100-frame A/B run produced identical zero drift with correction enabled and disabled, so their removal is the next refactor step. Observation-only world traces and drift gates remain useful independently.
...
- Current wire packets after the 2026-07-15 cleanup: `WireWorldState`, 120 bytes, carries only the real Big Star actor; `WireMovingHazardState`, 424 bytes, remains separate for up to four active moving-hazard actors. The earlier 520-byte `WireWorldState` also carried `Item`, `NeutralItem`, `DroppedStarItem`, and a redundant moving-hazard slot; those fields and their apply/spawn state were removed with the unused experiment.
...
- `scripts/run-nsmb-mvl-manual-local.ps1 -PlanDActorSnapshot` retains input delay `0`, max frame lead `4`, and network pump `50us`, but no longer enables the removed generic actor snapshot. Lead `4` replaced the earlier lead `8` default after manual logs showed remote-input-wait spikes during freeze reports.
...
- Current remote world and moving-hazard samples reject older packet frames instead of allowing late unreliable packets to overwrite a newer sample. The removed item, generic actor, effect, and player branches used the same rule historically.
- Current after-frame artifact correction syncs first and applies only a fresh same-frame moving-hazard sample. The historical player route also applied a fresh-only sample here and used the latest eligible sample before frame execution; both player hooks were removed with that route.
...
- Current fixed-size packets are classified before the input-bundle branch so `WireNSMLPacket`, `WireGameState`, `WireWorldState`, and `WireMovingHazardState` reach their exact handlers.
...
- `scripts/run-nsmb-mvl-manual-local.ps1 -PlanDActorSnapshot -AllowJit` remains as a lightweight Big Star/moving-hazard manual-play command with input delay `0`, frame lead `4`, network pump `50us`, and gameplay heartbeat. It no longer enables player actor/global, item-specific spawn, generic actor, or effect correction. Plan-D defaults to world snapshot interval `1`, world prediction `2`, a 30-frame process-list rescan interval, and fresh same-frame after-hook moving-hazard apply.
...
- Historical stage-object activation / dropped-star refinement (item packet removed 2026-07-15):
  ...
  - The historical `WireWorldState` was sent both directions, with host-side application limited to `DroppedStarItem`; these item slots and the bidirectional item path were removed on 2026-07-15. The current 120-byte packet carries only the host-authoritative Big Star.
