...
This section records the 2026-06-02 Plan-D experiment. It was not promoted to the production correctness route. As of 2026-07-15, the unused item-specific client-spawn and generic actor snapshot branches described below have been removed; the Big Star, moving-hazard, effect, and player snapshots remain under separate review.
...
- Remote world, moving-hazard, actor-snapshot, and effect samples reject older packet frames instead of allowing late unreliable packets to overwrite a newer sample. The item-specific branch also used this rule historically; its short-lived item packet is no longer present.
...
- `scripts/run-nsmb-mvl-manual-local.ps1 -PlanDActorSnapshot -AllowJit` is the retained lightweight manual-play command. It enables input delay `0`, frame lead `4`, network pump `50us`, player actor/global, Big Star, compact multi-instance moving-hazard/effect snapshots, and gameplay heartbeat. Item-specific client spawning and the generic process-list actor snapshot are no longer included. Plan-D defaults to world snapshot interval `1`, world prediction `2`, player prediction `1`, a 30-frame process-list rescan interval, fresh same-frame after-hook moving-hazard apply, and fresh-only after-hook player apply. It intentionally leaves detailed game-state/perf breakdown traces off unless explicitly requested.
...
- Effect slots are synchronized only for fixed active Effect slots and without clearing local-only effects. The historical bidirectional dropped-star item slot was removed with the unused item-spawn branch; other host/client-only actors still require case-by-case ROM/memory classification rather than generic actor cloning.
