...
- New host-authoritative generic actor packet: `WireWorldActorSnapshotState`, 1272 bytes, for up to twelve non-player, non-manager process-list actors. It carries object ID plus the same compact transform/velocity/common actor fields as the world actor state. This is the current root-level Plan-D refinement: it broadens coverage beyond hand-picked enemy/star examples without cloning all Main RAM or blindly spawning every host-only actor.
...
- `scripts/run-nsmb-mvl-manual-local.ps1 -PlanDActorSnapshot -AllowJit` is the lightweight manual-play command. It enables input delay `0`, frame lead `4`, network pump `50us`, player actor/global, Big Star, item-specific client spawn, compact multi-instance moving-hazard/effect snapshots, generic process-list actor snapshots, and gameplay heartbeat. World snapshots run every `2` frames with at most `1` prediction frame and a 30-frame process-list rescan interval. It intentionally leaves detailed game-state/perf breakdown traces off unless explicitly requested. The old lead-`8` launch `logs/codex-manual-pland-runtimelean-launch-1800-20260602` remains a historical propagation check, not the current manual default.
...
- User manual runs still reported 3/3 freezes before this change set. The analyzer now catches those aborted/missing-frame-limit logs, but the new generic actor snapshot + lead-4 + gameplay-heartbeat path still needs manual validation.
- The generic actor snapshot is intentionally transform/common-state only. It is a broader root-level correction than enemy/star-specific patches, but it still does not spawn arbitrary missing actors or synchronize object-specific private state. Use lifecycle and heartbeat gates to find persistent classes of drift before adding any narrower ROM/memory field.
...
- Run longer manual play with the promoted Plan-D path including generic actor snapshot, Effect sync, ROM activation patch, dropped-star Type `2` bidirectional slot, lead `4`, and gameplay heartbeat. Focus on overall drift/freeze, not only enemies or stars: dropped-star movement after damage, enemy stomp/death, course `1` item interactions, repeated result/restart, fall death, respawn, and pipe transitions. Keep automatic FPS-spike and gameplay-heartbeat logging enabled in manual Plan-D mode.
- Add a dedicated automatic route that reproduces the user's "manual input causes complete stop" more closely. It should combine simultaneous move/dash/jump, fall death, item/star loss, and continued inputs, then rely on frame heartbeat plus gameplay-heartbeat plateau detection rather than full CSV tracing.
