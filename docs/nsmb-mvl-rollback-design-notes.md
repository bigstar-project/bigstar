...
- 2026-06-23 exact/tiny-core continuation after StateSync mismatch detection landed:
  ...
  - Narrowed `tinycoreramdelta` flags. Base `0x241` is correctness-unsafe under complex input. `0x249` (`GPU2D timing + palette/OAM + GPU3D light + Wifi`) with full Main RAM delta and `RollbackMaxResimFrames=8` is the lightest measured StateSync-passing exact-ish point so far on `dualstressmid`: `18.490/18.449ms`, max `179.892/186.715ms`, save avg `~1.3ms`, restore avg `~20ms`, and resim total avg `~121ms`.
  - The other flag directions establish the boundary: cart-only, SPU-only, and Mic/SPI/RTC-only also pass with similar timing; all-device flags pass but cost more; full GPU is unnecessary and slower. `maxresim6` fails correctness, so the resim window cannot be reduced that way under this route.
  - Design implication: tiny-core + Main RAM delta is a usable correctness oracle and a better exact-ish baseline than full core, but it still cannot hit 60fps because rollback correction is foreground restore plus multiple `RunFrame` replays. The next serious path should either make selected lifecycle snapshots pointer-safe, or move exact correction into a shadow/non-blocking path; more page-delta slimming alone will not solve the active goal.
