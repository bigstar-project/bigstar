...
The ROM-side loop remains semantically more promising than the direct `ProcessList::execute()` experiment. A diagnostic ROM patch loops through NSMB's own main game path, advances `Math::frameCounter` naturally, and skips the render process list plus `Font::updateFont()` on intermediate catch-up ticks. A guest-owned seven-entry input table drives the loop without an interpreter instruction hook, including under JIT. The tested transactions still matched all 172 selected gameplay fields across the two peers. A single catch-up tick now passes the tested no-draw performance and fixed-condition image gates, but two or more ticks in one host frame do not; the current multi-tick design therefore still fails the overall playability gate.
...
**Current blocker:** a monolithic depth-4 correction remains both too slow and visually incorrect: selective LCD-event deferral keeps it inside one emulated display frame, but it takes `24.597/23.463ms`, and the endpoint/recovery images differ by `762/322` of `98,304` pixels. Depth 2 also missed the host-frame budget in two core-group-isolated repetitions (`16.720/20.347ms` and `17.862/28.513ms`). By contrast, the depth-1 same-role endpoint image matched the normal control exactly (`0/98,304` changed pixels), so the remaining architectural question is whether one correction tick per displayed frame can drain a real rollback backlog without exposing a lagging game state or duplicating audio/network side effects.

**Next action:** stop trying to fit depth 2-7 into one host frame. Add a diagnostic amortized scheduler that restores once, executes at most one historical correction tick in each displayed frame, and proves that a fixed depth-4 mismatch drains in four or fewer presentations while new predictions continue. Each step must retain the depth-1 `0`-pixel image result, remain below `16.667ms`, and suppress duplicate audio/network effects. If the backlog cannot drain or visible game latency grows, reject this ROM-loop architecture and keep the existing delayed full-frame fallback. Production guest-table serialization and real mismatch restore remain deferred until that test passes.
...
Two further core-group-isolated depth-2 repetitions under `logs/slippi-window-depth2-affinity-run{1,2}` measured `16.720/20.347ms` and `17.862/28.513ms`. All four role samples missed the strict `16.667ms` gate or exceeded it materially. Since depth 2 already lacks stable headroom, depth 3-7 are no longer useful monolithic optimization targets under the performance-first requirement. The viable research boundary is one correction tick per presentation, not an all-at-once multi-tick catch-up.
...
The current NSMB ROM loop follows that direction but has not isolated the game tick to the same degree. Each repeated update still consumes ARM cycles and lets the DS hardware scheduler, ARM7, timers, events, and eventually VBlank advance. The depth-4 display spill and character-region phase error are consequences of that boundary, not evidence that the software rasterizer is too slow. Since the narrower depth-1 path now passes the tested timing and image gates while depth 2 does not, the next work is an amortized one-tick-per-presentation scheduler with explicit side-effect control. Renderer tuning, checkpoint-byte reduction, process priority, or affinity changes should not displace that gate. A trace/superblock or different JIT backend is the general-emulator alternative, but the offline experiment shows that it is a substantially larger project than the current rollback integration.
...
- Guest catch-up wall-time/presentation gate: **PASS only for depth 1 on the tested route**; affinity-isolated no-draw timing was below `16.667ms` in six of six samples and the same-role endpoint image was exact. Depth 2 already fails stable timing, and depth 4 fails both timing and image convergence.
