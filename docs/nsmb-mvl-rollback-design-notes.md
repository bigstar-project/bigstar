...
**Current blocker:** selective LCD-event deferral keeps the tested depth-4 transaction inside one emulated display frame, but it still takes `24.597/23.463ms` on the two local peers and the corrected same-role image does not converge. The depth-4 endpoint and one-tick recovery images differed by `762` and `322` of `98,304` pixels respectively. Hardware-state tracing now identifies the endpoint error as an LCD/OAM phase problem rather than a divergent final game render: the final-render Main RAM transition is identical apart from 15 timing/network bytes, but the deferred path holds `VCount` at 197 and retains a different OAM image while the normal path advances `VCount` from 217 to 260. These failures take precedence over production integration.

**Next action:** keep LCD deferral diagnostic-only and return to the depth-4 guest CPU cost before refining presentation. The two bounded scanline alternatives tested here either remained above budget or damaged the RAM change mask, so there is no justified display fix to polish yet. The gameplay block profile rejects broad process-list or render-cache skipping: collision and arithmetic dominate and are gameplay-critical. The next performance experiment should therefore target a measured JIT/gameplay hot path and must reduce the depth-4 total below `16.667ms` without changing the 172-field result. Once that margin exists, use the new final-render/OAM trace to replace the blanket LCD pause with the smallest correct DMA/scanline sequence. Only after both gates pass should `InputTimeline::ResolveReplayFrameInputs()` be serialized into the guest table or a real mismatch restore be connected.
...
`logs/slippi-jit-block-profile-depth4-run1` enabled an x64 JIT diagnostic counter only for the four gameplay-list calls. Host and client produced the same dominant blocks. Among the retained top 256 blocks, `CollisionMgr::updateBottomSensor(bool)` accounted for `105,880` dynamic guest instructions (`48.7%`) and the ARM `_ll_div` helper for `22,570` (`10.4%`). Other collision-side/top routines, `Base::processUpdate()`, actor pre/post update, player update, input, tile behavior, and process traversal made up most of the named remainder. Candidate render-adjacent entries such as coin animation, particle cleanup, and projection setup were individually around `1%` or less. This profile does not prove that every named instruction is semantically indispensable, but it rules out a safe broad render-cache omission capable of the nearly `2x` emulated-cycle reduction required by the current VBlank phase. Optimizing or HLE-replacing `_ll_div` may be a later bounded experiment, but its entire observed share is insufficient on its own and collision HLE would be correctness-critical. The profiled asymmetric regression at `logs/slippi-jit-block-profile-depth4-semantic` still matched all `172` gameplay fields at the endpoint and after recovery; the counters are host-only and diagnostic.
...
The current NSMB ROM loop follows that direction but has not isolated the game tick to the same degree. Each repeated update still consumes ARM cycles and lets the DS hardware scheduler, ARM7, timers, events, and eventually VBlank advance. The measured two-display-frame endpoint and persistent character-region screenshot phase difference are consequences of that boundary, not evidence that the software rasterizer is too slow. The next performance work should therefore prioritize a narrower NSMB update kernel plus explicit VBlank/presentation and side-effect deferral. Renderer tuning, checkpoint-byte reduction, process priority, or affinity changes should not displace that gate. A trace/superblock or different JIT backend is the general-emulator alternative, but the offline experiment shows that it is a substantially larger project than the current rollback integration.
...
- Guest catch-up wall-time/presentation gate: **FAIL** for the current implementation; affinity-isolated depth 1 was below `16.667ms` in six of six samples, but depth 4 remains above budget even with LCD deferral and its corrected image does not converge.
