...
- `scripts/run-nsmb-mvl-manual-local.ps1 -PlanDActorSnapshot -AllowJit` is the lightweight manual-play command. It enables input delay `0`, frame lead `8`, network pump `50us`, player actor/global, Big Star, and compact multi-instance moving-hazard snapshots every `2` frames, at most `1` prediction frame, and a 30-frame process-list rescan interval. It intentionally leaves detailed game-state/perf breakdown traces off unless explicitly requested. A short visible-window launch `logs/codex-manual-pland-runtimelean-launch-1800-20260602` propagated `playerStateSync=1 playerStateApply=1 playerStateGlobals=1`, ran with no rollback/resim, and measured active avg `17.665/17.663ms`, max `33.253/33.132ms`, `over33ms=0/0`.
...
- Dense lifecycle diagnosis found a concrete host-only world actor gap on generated course `1` stress:
  ...
  - ROM symbols provide `Actor::spawnActor` at `0x020A0B64`, and the frontend already has an ARM trampoline pattern. The next experiment is a narrow `Item` event packet plus client spawn, not a generic actor clone.
...
- Course `1` now has a concrete host-only `Item` actor creation gap. The client-spawn experiment must stay item-specific until lifecycle and performance validation proves it is safe; blindly spawning every host-only actor would also replicate local-only effects.
...
- Add a narrow `Item(0x01F)` world event snapshot and client `Actor::spawnActor` trampoline experiment. Validate the course `1` gap without enabling the diagnostic full-RAM scanner in normal runtime.
