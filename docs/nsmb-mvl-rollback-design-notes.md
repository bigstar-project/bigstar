...
- Latest user feedback says the current Plan-D actor snapshot path no longer hard-freezes as readily, but complex play still desyncs. Treat freezes as improved but not closed; the available `181856` manual log still contains a watchdog stall and heartbeat object-count divergence.
...
- Investigate the newly exposed `053:00000000` host-only moving-hazard lifecycle gap from `logs/codex-pland-actor16-exclude10b-heartbeatids-stage0-2600-20260602`. Avoid the already rejected "kill extra local hazard" approach; prefer classifying why the client lacks the second hazard at the activation boundary and whether a narrow stage-object activation or host-authoritative spawn/confirm path is justified.
- Add a longer automatic route that stresses simultaneous move/dash/jump, fall death, item/star loss, enemy contact/stomp, and continued inputs. It should rely on frame heartbeat, gameplay heartbeat, active-ID object diff, and FPS-spike gates rather than full CSV tracing.
