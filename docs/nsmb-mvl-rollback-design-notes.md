...
This section records the 2026-06-02 Plan-D experiment. It was not promoted to the production correctness route. As of 2026-07-15, the unused item-specific client-spawn, generic actor snapshot, and world effect snapshot branches described below have been removed; the Big Star, moving-hazard, and player snapshots remain under separate review.
...
- `scripts/run-nsmb-mvl-manual-local.ps1 -PlanDActorSnapshot -AllowJit` is the retained lightweight manual-play command. It enables input delay `0`, frame lead `4`, network pump `50us`, player actor/global, Big Star, compact multi-instance moving-hazard snapshots, and gameplay heartbeat. Item-specific client spawning, generic process-list actor snapshots, and effect correction are no longer included. Plan-D defaults to world snapshot interval `1`, world prediction `2`, player prediction `1`, a 30-frame process-list rescan interval, fresh same-frame after-hook moving-hazard apply, and fresh-only after-hook player apply. It intentionally leaves detailed game-state/perf breakdown traces off unless explicitly requested.
