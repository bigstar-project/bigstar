...
**Current blocker:** robust one-frame correction is still borderline in the existing full-frame path. Before removing a duplicate final checkpoint save, capped one-frame timing samples were `14.797ms` and `17.381ms`. After the fix they were `16.784ms` and `16.210ms`; one of two still exceeded `16.667ms` by `0.117ms` because restore varied from `3.942` to `5.126ms` and `RunFrame()` from `11.360` to `11.978ms`. These capped runs deliberately discard the older mismatching frame and are timing probes, not correctness evidence. A normal two-frame forced mismatch did pass the semantic smoke after the save fix, but took `27.138ms`.

**Next action:** stop production work on the ROM-loop path and keep its diagnostics only as research evidence. Continue on the existing full-frame rollback by creating a correctness-preserving forced mismatch that is exactly one frame old, then collect enough samples to judge p95 after the duplicate-save removal. The next optimization target is the measured `3.3-5.1ms` restore variance; deeper corrections should be prevented primarily through input delay/pacing and handled by the existing fallback rather than promised as 60 FPS rollback. Do not serialize the guest history table into production.
