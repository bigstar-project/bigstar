...
- Two attempted shortcuts are rejected: 512-byte CodeMem bitmask pruning of restored-range invalidation caused contact `playerActor0X` mismatch at frame `2820`, and larger Main RAM delta pages (`512`/`1024`) also mismatched contact. Keep page256 plus conservative invalidation as the current safe baseline.
