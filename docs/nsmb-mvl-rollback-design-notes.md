...
- **Slippi current-source cross-check:** 2026-08-20時点の公式`Ishiiruka` `slippi` branch HEAD `e7711b1`には、rollback上限待機とは別のtime-sync層がある。受信したpad packetのframe番号、localの直近送信frameと時刻、RTTの半分から`timeOffsetUs`を推定し、直近30 sampleをsortして上下各3分の1を除いた平均を使う。従ってSlippiも単純な`latestRemoteFrame-currentFrame`だけではなく、packet時刻を含む相対進行差を観測している。
- **Slippi recovery control:** Slippiは30 frameごとにtime offsetを再評価し、遅れ側を最大`+1%`、先行側を最大`-0.5%`へ動的に調整する。さらに試合開始120 frameより後、遅れが`16683 + 10000us`を超えると、1回の評価で最大3個の追加game frameを要求し、5 frameごとに1個ずつ分散実行する。Dolphinはresponse `4`をASMへ返し、game側の`ForceInputRefetchOnAdvance`がinput renewalをもう一度呼んで次のengine loopを2回走らせる。先行側のtime-sync目的の明示stallは開始120 frame以内だけだが、通常対戦中も遅れ側の追加frame、両側のspeed補正、rollback上限stallが組み合わさって片側差を縮める。これは不具合発生時のBigstarになかった制御である。
- **Slippi interpretation for this bug:** 今回と同じ7-frame差なら、Slippiではlaggerがadvance thresholdを大きく超えるため、pad受信とtime-offset sampleが再開した後、30-frame評価ごとに最大3 frameを分散catch-upする。同時にlaggerは最大101%、leaderは最大99.5%へ寄せられる。30-sample filterとnetwork timingがあるため正確な収束時間はsourceだけから一意に決まらないが、7-frame差をそのまま恒久維持する設計ではない。不具合発生時はrollback horizonだけをSlippiから移し、time-syncを移していなかったことが設計差だった。
- **Slippi disconnect boundary:** current sourceはrollback上限stallが`60 * 7`回を超えると、そのremote playerをforce-disconnectする。従って今回の約5.5秒停止は再開後catch-upの対象になり得る一方、約7.9秒停止をSlippiが維持・回復できるとは言えず、通常の60 Hz進行なら先に7秒切断へ達する可能性が高い。Bigstarはユーザー要件でhorizon timeoutを60秒にしているため、Slippiより長く接続を維持するならphase recoveryも独自に備える必要がある。
...
- **Implemented phase estimator:** rollback有効時だけ独立した`PhaseRecovery::Runtime`を動かす。local inputの初回送信時刻を最大1024 frame保持し、bundle ACKが前進した時だけRTT標本を採る。remote inputの新しいframeを受信すると、直近local送信からの経過時間、half RTT、local/remote frame差からoffsetを推定する。RTT標本が3件未満の間は補正せず、重複・逆順remote frameも標本にしない。offsetとRTTは各30標本を保持し、sort後の上下各3分の1を捨てた平均を使う。remote観測が500ms途絶えた場合は速度と追加frameをneutralへ戻し、古い標本だけで補正を継続しない。
- **Implemented recovery control:** 30 frameごとにoffsetを評価し、Slippiと同じ上限でlaggerを最大`+1%`、leaderを最大`-0.5%`へ補正する。frame 120以後にlagが`26.683ms`を超えた場合は最大3個のcatch-up機会を作り、5 frameごとに1回ずつframe limiterとaudio waitを省く。MeleeのASMによる1 outer-loop内2 game tickはNSMBへ直接移植せず、既存のmelonDS outer loopを一度だけ早く次へ進める実装にして、ROM-loop訂正処理とは分離した。開始・世代変更時には全標本とpending catch-upをresetする。
- **Configuration and test harness:** `MELONDS_NSML_ROLLBACK_PHASE_RECOVERY=1`で有効化し、Bigstarはrollback有効時だけ設定する。E2E専用にrole、logical frame、停止時間を指定する一回限りのemulation pause knobを追加した。停止はemulation threadだけをsleepし、別のnetwork pump threadは動かしたままなので、controller dialogに近い片側clock停止を再現できる。
- **No seven-second disconnect:** Slippiの`60 * 7`回待機後のforce-disconnectは実装していない。既存のBigstar prediction-horizon timeout `60000ms`も変更していないため、7秒停止したという理由だけでは切断しない。
- **Fixed E2E result:** `logs/nsmb-phase-recovery-manual-e2e-fixed-20260820`でclientだけをlogical frame 1050から2秒停止した。再開直後のclient offsetは`-30.768ms`、次の評価では`-85.093ms`となり、合計7回の分散catch-upを実行した。clientはlogical frame 1158で`+8.160ms`、hostも1159で`-5.786ms`まで戻り、その後は概ね`±10～14ms`で推移した。両roleは1550 frameへ到達し、frame 900以後の共通22標本・38 stable/input fieldは差0、timeout、checkpoint missing、訂正失敗、critical mismatchも0だった。解析器の性能失敗表示は意図した2秒sleepを単一slow frameとして数えたためで、回復後の同期失敗ではない。
- **Verification status:** Release melonDS build、CTest `17/17`、Bigstarの`corepack pnpm run ci`（typecheck、Biome、edition 15件、unit 37件、browser 69件、Playwright 2件）、Rustのformatとwarning-as-error Clippyがpassした。phase recovery単体testはRTT未確立時のneutral動作、同期時無補正、速度上限、3回/5-frame間隔のcatch-up、観測stale、generation resetを固定した。初回E2Eでclientに補正ログが出なかったことから、別プロセスではemulator instance IDが両方0なのにplayer IDを比較していた統合バグを発見し、instance IDの範囲検証へ修正した後に上記E2Eを再実行した。
- **Current blocker:** 片側停止後にframe advantageが`P=7`へ恒久固定される実装blockerは解消した。online中の設定dialogを協調pauseまたは禁止するUI policyは未実装であり、実Bigstar GUIで同じcontroller設定操作をした目視確認も残る。既知のrollback音声取り消し不能、player spriteの訂正直後最大3-frame位相差、再戦checkpoint復元の約`0.15秒`停止、stage開始直後hitchは別件として残る。
- **Next action:** 実Bigstar GUIの独立2プロセスでcontroller設定dialogを開閉し、ログのoffset、分散catch-up、訂正depth、相手spriteの見え方を確認する。自動gateを強化する場合は2秒停止runの収束条件をスクリプトassertionへ昇格し、追加で5～8秒停止も実行する。ただし7秒超を切断条件にはしない。
...
修正前に保存された完全入力と同一seedを再生した `logs/codex-repro-manual-depth7-exit-20260818` は2800 frameを完走した。host/clientは`107/62`訂正、horizon block/resumeは`631/258`件、fatalは0だった。従って入力列だけで決まる不具合ではなく、packet到着と複数outer frameにまたがるROM-loop transactionが重なったときのtiming-dependent raceであることを確認した。旧gateは新しいtickの入力解決前には働くが、既に走っているtransaction中に生じたpending mismatchの老化を止めなかった。

原因は、ROM-loopが要求された全game tickを最初のouter frame内で完了しても、次のguest input gateへ到達するまでtransactionをin-flightに残したことだった。旧ログはarmした`current + 2`でcompleteしており、この空白frameでは次のpending mismatchをarmできなかった。`CanFinalizeTransaction()`へ完了条件を分離し、RAM restore済み、history index/count完了、game frame完了を満たせば、guest exit gate前でもouter-frame末尾でcontrolをclearするよう変更した。新しいpending mismatchは次のbefore-frameで連続transactionとしてarmできる。未実行history、空history、inactiveなstale counter、frame wrapを拒否するunit testも追加した。

修正後の同入力・同seed再生 `logs/codex-depth7-atomic-finalize-replay-20260818` は3200 frameを完走し、訂正はhost/client `89/97`、cannot-arm、fatal、19共有heartbeat差は0だった。completeは全件`current + 1`になり、問題の空白frameを除去できた。長時間境界stress `logs/codex-depth7-atomic-finalize-stress-5400-20260818` は人工送信遅延8-9 frameでhost/client `102/95`訂正、最大mismatch depth `7/7`、horizon block/resume `1123/1047`件、cannot-arm、fatal、76共有heartbeat差は0だった。active平均`16.737/16.739ms`、最大`57.092/66.242ms`、33ms超`72/71`であり、安全性は通るがこのping 270-300ms相当条件を快適とは判定しない。

通常性能gate `logs/codex-depth7-atomic-finalize-practical-3600-20260818` は人工送信遅延2-3 frameでhost/client各79訂正、最大mismatch depth `2/1`、cannot-arm、fatal、46共有heartbeat差は0だった。active平均は両role `16.667ms`、最大`34.548/30.463ms`、33ms超`1/0`、連続slow frame `1/0`である。hostの単発最大は次の訂正をarmする前のframe 1631で、持続的な60fps低下はない。初回gameplay correction付近のrun-frameは別に`33-36ms`あり、完全無停止とは扱わないが、今回のfinalize変更による持続性能悪化は検出していない。

次のユーザー手動run `logs/manual-slippi-depth7-20260818-194536` は初戦中の状態差やOS例外ではなく、再戦入口で世代が片側だけ進む停止だった。clientはframe 6600で結果を確定し、frame 6719でbootstrap checkpointを復元してgeneration 1へ進んだ。一方hostはraw frame 6598、contiguous remote 6590でP=7上限に止まり、generation 0のframe 6591以降を待った。clientがframe 6600まで作ったinput bundleは人工送信遅延8-9 frameのqueueに残っていたが、結果画面ではinput scratch経路を止めるためnetwork pumpも呼ばれず、generation reset時の`Delivery.Clear()`で破棄された。その後clientはhostのgeneration 0 bundleを捨て、hostは7秒後にexit 73、client wrapperは進捗停止として終了した。これは「再戦時にmelonDSがクラッシュした」のではなく、旧世代最終入力の未排出によるprotocol deadlockである。

修正後は、結果sceneへ入った最初のframeで旧generationの遅延input payloadを順序どおりreliable送信してqueueを空にし、結果timer中もENet serviceを継続する。処理は次試合がある結果だけでなく最終結果にも適用し、通常gameplay frameの経路は増やしていない。unit testはrelease予定が120 frame先のpayload 2件もgeneration transition時に順序を保ってdrainできることを固定した。

決定的な再戦再現 `logs/codex-rematch-delay-flush-forced-paced-20260818` はsoftware renderer・60fps制限・人工送信遅延8-9 frameで、結果入口に残っていたhost/client `8/10` payloadを実際に排出した。両roleともframe 1838で同じ勝敗、1957でgeneration 1 reset、2133でstart-ready交換を完了し、2500 frameまでtimeout、旧generation拒否loop、cannot-arm、状態差なしで完走した。active平均は`16.793/16.788ms`で、flush frame自体に25ms超spikeはない。最大`119.499/113.930ms`はframe 1958のbootstrap restore/JIT再初期化であり、今回の送信排出ではなく既知の再戦checkpoint hitchである。従ってdeadlock修正は性能gateを悪化させていないが、再戦を完全無停止にしたとは判定しない。

人間操作記録の等速再生 `logs/codex-rematch-delay-flush-replay-paced-20260818` は7200 frameを`59.32/59.51fps`、53共有heartbeat差0、fatal 0で完走したが、元runの勝敗scene自体を再現しなかった。packet timingでprediction列が変わるため、これは通常play継続と性能の補助証拠に留め、再戦修正の合格根拠には上の決定的再現を使う。

修正後のユーザー手動run `logs/manual-slippi-depth7-20260818-200841` は初戦と再戦で状態差を生じず、再戦境界deadlockの目視gateを通った。一方、初戦より2戦目の死亡時黒枠animationが明らかに滑らかだった。区間集計では初戦にhost/client `441/473`件のprediction mismatchと同数のROM-loop訂正、prediction horizon block `1864/2194`件、25ms超outer frame `1154/1800`件があり、33ms超は`204/238`件、最大`66.529/74.112ms`だった。2戦目は両roleともmismatch、訂正、horizon blockがすべて0で、25ms超は各2件、最大`31.722/30.121ms`に急減した。従って滑らかさの差は主観だけではなく、2戦目で人工遅延とrollback負荷が消えた実測差である。
...
12. 完了: Bigstarの旧`coredelta`起動を試験版ROM-loop契約へ切り替え、game-tick ROMのcache identity、GUI表示、起動環境、通常非rollback回帰、WebRTC sidecar E2E、配布bundleを検証した。次は実GUIの人間操作でWAN対戦と再戦を確認する。音声artifactはユーザー指示により後回しだが、解決済みとは扱わず既知制約として残す。再開時は、(a)知覚的緩和と、(b)最大7 frame分のaudio遅延＋ARM7/SPU rollbackを性能gateからやり直す高リスク案を分離する。
