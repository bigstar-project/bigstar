...
- Added diagnostic-only actor lifecycle tracing:
  ...
  - `logs/codex-pland-processlist-lifecycle-luigi-death-3720-20260602`: a 30-frame lifecycle trace passed Luigi death, pipe-respawn visibility, moving-hazard progress, Big Star drift `0/0`, and the 34-field manager/global gate across `95` rows. Moving-hazard max drift briefly reached `2048/8192` but had no sustained row. Active avg was `17.302/17.303ms`, max `46.113/56.172ms`, and max consecutive slow frames `1/1`. The only aggregated differences were the regular local-role `StageFX(0x012 settings=0x00008000/0x00008010)` pair.
