...
- Use the per-frame phase trace on the next paired-process stall and classify whether the dominant cost is emulator `runFrame`, PoC `beforeHook` / `afterHook`, input/MP processing, rendering, or external scheduling (`unaccounted`).
