...
**Next action:** inspect `51381858` and port only resimulation optimizations that do not depend on the rejected condition/register-allocation changes; benchmark each behind an explicit ROM-probe switch. The gameplay block profile rejects broad process-list or render-cache skipping as the next direction: collision and arithmetic dominate, and they are gameplay-critical. The retained exact block chain reduced host time modestly but did not change the two-display-frame result, and no JIT optimization can change the fixed emulated VBlank budget by itself. Presentation/VBlank deferral remains required alongside host-time reduction. A replacement must first keep the corrected endpoint inside one presentation interval with repeatable margin and make the final same-role image converge. Only after those gates pass should `InputTimeline::ResolveReplayFrameInputs()` be serialized into the guest table or a real mismatch restore be connected.
