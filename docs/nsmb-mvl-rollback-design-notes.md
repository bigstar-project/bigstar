...
- 2026-06-26 retake tightened the boundary:
  - Latest retake strengthens the semantic Plan-D path rather than foreground replay. Lightweight FPS-spike tracing was split from phase tracing, and the current target keeps StateSync/FPS validation enabled without the expensive `MELONDS_NSML_PERF_SPIKE_PHASE_TRACE` path.
  - Star candidate `0x010C` needs a dedicated authoritative lifecycle lane, but not per-packet full scanning or spawning. The current shape is active-only cached sampling plus low-rate absent/deep scan and low-rate extra-candidate pruning. This lets the candidate use `30F` star deep scans again while avoiding the duplicate `0x10C/0x1120` mismatch that appeared when the scan was coarse.
  - Moving hazards are now explicitly host-authored for update/lifecycle in this line: the local freeze skip-update bit is ignored in sync hashing, and a bounded stale fresh sample can still be applied. This reduces false mismatch and avoids dropping useful host samples, but collision/damage behavior still needs manual-style validation.
  - Current measured boundary for `pland-p0-kf30-hz-pfresh1500-gates-worddiff-hzfreeze` under `InputDelayFrames=2`, send delay/jitter `4/4`, StateSync, and lightweight FPS tracing: `dualstressmid` and `chaos` pass in `logs/pland-semantic-continue/20260626-074951` with averages around `16.8-17.3ms` and no transient mismatch; `contact` passes in `logs/pland-semantic-continue/20260626-075221` at `16.899/16.898ms` with no transient mismatch. Max frame spikes still reach roughly `46-49ms` on host in complex routes, so the active goal is not complete.
  - Fixed-seed retakes confirmed the current HEAD can pass the suspect chaos and dualstresslong seeds after discarding the spawn experiment (`logs/pland-semantic-fixed/20260626-080820`, `logs/pland-semantic-fixed/20260626-080947`). Moving-hazard spawn remains rejected for this target: when enabled directly on `hzfreeze`, chaos reintroduced a star-candidate duplicate mismatch, so missing-hazard gaps should be handled through restore/activate/mapping rather than unconditional spawning.
  - Route-health note: `luigistar` and `mariostarleft` currently fail their scripted pickup assertions while host/client stay aligned; the fixed star position no longer matches those input scripts. They are route-repair tasks, not rollback rejection evidence until updated.
  - New Plan-D moving-hazard result: the lifecycle trace shows client-side game update can destroy a host-authoritative moving hazard after the remote sample has been applied. This is a lifecycle ownership problem, not simply a missing byte range. Manual process-list relink remains rejected because it can corrupt list topology/self-loop and collapse FPS.
  - The safer current Plan-D hypothesis is host-authoritative moving-hazard transform plus local update freeze. `MELONDS_NSML_WORLD_STATE_FREEZE_LOCAL_MOVING_HAZARD_UPDATE=1` sets the local skip-update bit after applying host samples, preventing the client from independently destroying/moving the hazard. `pland-p0-kf30-hz-pfresh1500-gates-worddiff-hzfreeze` passed artificial-delay `chaos` at `16.911/16.912ms`, passed `dualstresslong` around `16.866/16.866ms`, and passed no-StateSync `contact` at `16.796/16.882ms`.
  - Follow-up Plan-D refinement: generic actor snapshots are not enough for lifecycle absence, because a host with zero relevant actors must still authoritatively tell the client to prune extras. Empty actor snapshot packets are now sent/applied, moving-hazard extra pruning is delayed rather than immediate, and star candidate `0x010C` has a dedicated host-authored lane. The star lane currently uses host full detection, low-rate client absent deep scan (`30F`), and a short world-state-only fresh wait (`100us` implemented without sub-ms sleep).
  - Latest measured boundary for this semantic lane set: strong-jitter `chaos` passed at `17.160/17.029ms` in `logs/pland-semantic-continue/20260626-053728`, strong-jitter `dualstressmid` passed at `16.968/16.968ms` in `logs/pland-semantic-continue/20260626-053853`, and delay2/jitter2 single-route `chaos` passed at `17.079/16.951ms` in `logs/pland-semantic-continue/20260626-050516`. This is the strongest Plan-D-ish evidence so far, but it is still a measured candidate rather than a proven default. Full spike tracing can perturb the boundary and reintroduce star mismatch (`logs/pland-semantic-continue/20260626-054017`), so the trace path needs separate overhead/isolation work.
  - This is not a correctness proof yet. It deliberately makes moving hazards host-authored for motion/lifecycle, so collision/damage semantics and death-route behavior need targeted validation. The existing `death` route assertion is currently not usable because hzfreeze, p0 without freeze, older re-death candidates, and historical no-rollback baselines can all fail the same route-level check.
  - New `nsmbtinycore` / GPU3DLight cp2 boundary: `nsmbtc-rb0-mr10-lead6-budget2ms-lookupjit-limit2-cp2-confirmedstate-arena-stack-devices-gpu3dlight` passes no-artificial `chaos` at `16.875/16.881ms` with no transient mismatch, so the selected snapshot is not inherently broken. The remaining failure is correction burst handling under artificial delay/jitter.
  - `RollbackMaxResimFrames` is now classified as a correctness boundary, not only a perf knob. With `cp2`, `mr6` can average near the target under sendDelay2/jitter2 but may cap an old prediction mismatch and leave early player-position drift; uncapped resimulation removes the drift but produces `~120ms+` visible correction frames and fails the average gate. `mr8/mr10` and raising max corrections reduce persistent risk but do not remove the transient mismatch under forced delay.
  - Rejected mitigation attempts: startup-only input waits move cost into packet scratch and still predict; confirmed held-future bundles are unsafe because later real inputs can conflict with already-confirmed future frames; skipping intermediate resim checkpoints can improve timing and transient mismatch but is memory/JIT unsafe in this path, reproducing ARM9 prefetch abort around frame `2665`.
  - Persistent shadow was corrected to feed the same local/remote/predicted packet scratch inputs into the worker shadow. This was necessary, but not sufficient: `logs/rb-shadow-scratchfeed/20260626-021556` still averaged `19.699/19.959ms`, host shadow `RunFrame` averaged `34760us`, `ShadowMatch=0`, and Main RAM differed by `5869` bytes. The current "second full NDS per foreground frame" design is too expensive and still divergent.
  - Correction scheduling retakes sharpened the tradeoff. `InputMaxFrameLead=4/2` lowers some max spikes but raises averages to `17.68-17.87ms`. `skipmidcp+lead2` is the cleanest foreground scheduling point so far under forced delay: no transient mismatch/abort in one long chaos run, max `69.8/92.7ms`, over33 `22/24`, but average still `17.871/17.874ms`. Skipping predicted-input lead throttle drops average to `16.851/17.096ms` but causes much larger correction spikes (`136/101ms`). Limiting corrections to one per frame caused a client stall near frame `2109`; pending-only predicted throttle was worse (`17.887/17.898ms`, host max `178.948ms`).
  - Current design implication: foreground selected rollback is good enough as a normal-delay experimental backend but cannot satisfy the active goal under bursty input without either visible spikes or capped-correction state drift. Further byte trimming is unlikely to change this because the expensive term is still foreground restore plus one or more resimulated `RunFrame`s. The current persistent-shadow implementation is not the escape hatch; a usable non-blocking design would need to avoid running a full second emulator every foreground frame, or switch to a semantic selected-state model that avoids full replay on old corrections.
...
- Current design implication: checkpoint byte trimming and same-frame correction scheduling are exhausted for the current foreground architecture. Exact rollback is bounded by restore + resimulation placement; Plan-D can hit the FPS target when the authoritative lanes are semantic and cheap. The immediate Plan-D direction is host-authored moving-hazard freeze, delayed lifecycle pruning, and dedicated world-object lanes with targeted collision/death/item validation, not unsafe process-list relinking or broad page widening. In parallel, continue root classification of deterministic player/global state instead of single-symptom actor patches.
- Follow-up measurement narrowed the Plan-D path:
  ...
  - The current artificial-delay Plan-D boundary is split. `pland-p4-kf30-hazardlife` keeps `chaos` with sendDelay2/jitter2 around `16.7ms` and removes persistent mismatch in that route, but `dualstresslong` still exposes player death/global drift. Reducing player prediction to 0F (`pland-p0-kf30-hazardlife`) makes `dualstresslong` pass at `16.687/16.666ms`, but chaos then fails later on moving-hazard/transition drift. This means player transform prediction, player global/death repair, and hazard lifecycle cannot be controlled by one scalar prediction knob.
  - A dedicated `PlayerStateApplyTrace` exposed an apply-order issue: transition/global words can be applied from a fresh remote sample and then overwritten by game code before the next sample point. Reapplying overwritten globals broadly is unsafe because transition status gets forced and chaos can abort; reapplying only critical powerup/death/counter fields is safer but does not solve the remaining transform/hazard drift. Treating transition status as local validation state also failed because position drift remained.
  - Moving-hazard spawn is not a shortcut. It can instantiate a missing hazard, but without a stable subsequent GUID/map/apply path it lags behind the host and tanks client FPS (`24ms` average in one chaos run). The next hazard work should repair the mapping/apply path for existing/dormant hazards rather than repeatedly spawning new ones.
  - Current design implication: Plan-D remains fast enough, but it is not yet a coherent authoritative-state model. The next serious iteration should split remote player state into independently timed lanes: transform prediction, critical globals/counters, and local transition/render state. For world objects, moving-hazard lifecycle needs a persistent local/remote identity map that survives dormant/respawn boundaries without spawning every mismatch.
...
- Best current selected timing shape is `b1500-arena-lite-changedjit-cp2-cartlite-mr1`: cart runtime subset, changed-page JIT invalidation, `cp2`, and one resimulated frame maximum. It is StateSync-clean in measured `chaos1500` sendDelay2/jitter2 retakes and can average roughly `17.3-17.9ms` with checkpoint save around `0.18ms`, but still produces `50-55ms` visible correction frames. This is a meaningful correctness/cost improvement over full Cart, but not a playable no-spike solution.
- Design implication from the cartlite retake: the selected snapshot can now make checkpoint save cheap again while preserving the cart state needed for lifecycle stability, but foreground rollback still has a hard floor. Even one correction is restore (`~6-7ms`) plus one resimulated `RunFrame` (`~14ms`) plus the normal displayed frame. Further range trimming cannot remove that; the remaining work is correction placement/non-blocking replay or reducing prediction frequency without adding user-visible delay.
- Latest measured boundary after the persistent-shadow rejection: selected actor/global snapshots are currently more practical than a second `NDS` shadow, but the remaining bottleneck is still foreground replay. Adding the actor arena fixed the dropped-star/actor lifecycle mismatch, and changed-page JIT invalidation plus `cp2` can keep StateSync clean with averages below 18ms in one run (`b1500-arena-lite-changedjit-cp2`: `17.895/17.830ms`). However, rollback spikes remain large (`54.745/90.818ms`, over33 `6/5`), so this is not a promotion point.
- Spike-focused throttling confirms the tradeoff. `b1500-arena-lite-changedjit-cp2-lead0` cuts the same-route max to `42.667/48.123ms` and over33 to `3/3`, but the throttle cost raises average to `18.278/18.271ms`. The no-rollback delay4/lead4 baseline passes at `17.042/17.067ms`, proving the machine has enough baseline headroom and that rollback replay/placement is the gap.
- A consume-current bug was fixed: after rollback resimulates through `targetFrame=frame+1`, the normal before-hook path must not continue applying stale old-frame patch/scratch/checkpoint operations. This makes consume-current correctness-shaped again, but it does not beat the non-consume changed-page/cp2 path.
- Added a gated pre-scratch correction drain. Running rollback after local input is recorded but before the current checkpoint/scratch write removes one class of late-correction mismatch without restoring over the current scratch packet. This is the right placement if foreground correction remains in the visible frame.
- The result is a clean tradeoff, not a solution. Pre-scratch interval `1/3/6` variants can keep long `chaos` StateSync clean, but still average above the target and keep `100ms+` visible rollback frames. Interval `4` lowers average but reproduces ARM9 prefetch abort. This shows the mismatch was partly scheduling, while the FPS problem remains same-frame replay cost.
- `RESIM_CONSUME_CURRENT_FRAME` does not solve this selected path when combined with pre-scratch draining; it adds enough replay work that short `chaos1500` worsens to about `19.2ms` average and `130ms` max.
- Added slow checkpoint-save tracing for the normal `SaveRollbackCheckpointIfNeeded` path. Current spike evidence still points to `rollbackMs`, not checkpoint save/prune or packet scratch. Future work should optimize or move rollback replay itself.
...
- Added a persistent-shadow measurement pass before attempting real foreground commit. The probe now records queue lag/drop, enqueue overhead, worker restore/run cost, and comparison status, with CSV fields in the practical suite.
- Current measured worker shape is not viable as a backend yet. `chaos1500` with sendDelay2/jitter2 and the prescratch selected candidate produced host `34.725ms` average and only completed `471/600` queued shadow jobs after a `3s` drain; `47` jobs were dropped, latest lag was `82` frames, max lag was `904`, enqueue avg was `2.306ms`, restore avg was `8.893ms`, and shadow `RunFrame` avg was `40.562ms`. The compare frame was never reached, and the host log emitted repeated ARM7 undefined-instruction lines from the shadow path.
- Design implication from that measurement: the current "persistent" worker is still checkpoint-restore-per-frame plus one `RunFrame`, not a continuously evolving prediction shadow. It loses the intended advantage of preserving execution/JIT history and adds enough CPU contention to tank foreground FPS. Before shadow can be used for correction, it must either run continuously from an early synchronized point with queued effective inputs, or be rejected in favor of the selected lifecycle/Plan-D-ish snapshot path.
...
- 2026-06-24 strong-jitter retake after the GPU3DLight selected snapshot work:
  ...
  - Current selected boundary under `InputDelayFrames=2`, `InputSendDelayFrames=4`, `InputSendJitterFrames=4`: `fullplayer + heap15 + extra2p + GPU3DLight` with `InputMaxFrameLead=0` is the strongest correctness point but not playable (`19.756/19.757ms` with StateSync; `18.111/17.843ms` without StateSync, max up to `83.369ms`). `cp2` stays state-aligned in the measured run but still averages `18.624/18.624ms` without StateSync and has `60ms+` rollback frames. Raising to `lead1/lead2` is correctness-unsafe; the runs hit persistent `playerGlobal=0` and/or ARM9 prefetch aborts.
  - The selected delta-page trace showed why this cannot be solved by only widening pages. A `lead2` diagnostic had byte-level uncovered changes near `0x020890CD`, `0x021B2C00-0x021B43xx`, `0x023260FE`, and `0x0234A15E`; adding those ranges blindly made mismatch earlier (`frame=1140`) instead of fixing it. That means the uncovered set includes host-local/timing/ephemeral fields or fields whose restore order matters, so the next useful state work needs semantic classification of deterministic actor/global state, not page-wide inclusion.
  - Rollback same-frame waits do not rescue `lead2`: `rbwait500` mismatched from frame `1020` and averaged `20.312/20.312ms`; `rbwait1000` mismatched from frame `1260` and averaged `21.267/21.264ms`. The design implication is unchanged but sharper: selected snapshots are viable only if deterministic lifecycle/global state can be separated from local/timing fields, or correction must move to a shadow/non-blocking path. More checkpoint byte trimming and blind extra ranges are not sufficient.
  - Added diagnostics for selected GPU3DLight rollback: `MELONDS_NSML_INPUT_FRAME_LEAD_RESEND_INTERVAL_MS` plus `lead4/lead6`, `resend5`, `resend1`, `noskipjit`, and `mr4` suite candidates. These keep `InputDelayFrames=2` and use `InputSendDelayFrames=4` / `InputSendJitterFrames=4` for the forced-correction check.
  - Selected GPU3DLight did not produce a promotable middle point. `mr2` keeps max frames around `55-63ms` but mismatches player actors and can still ARM9-abort; `mr4/mr6` avoid some contact mismatches but still produce `129-149ms` visible frames, and chaos repeatedly reaches ARM9 prefetch abort around frame `2665-2678`. Removing skip-JIT reset is safer but still too slow and not fully stable.
  - Added `MELONDS_NSML_ROLLBACK_JIT_LOOKUP_RESET_ONLY=1` selected candidates to test the JIT boundary without the unsafe broad skip-reset. `mr6 lookupjit` is the safest measured selected middle point in this pass, with no mismatch/abort on contact or chaos, but it still fails the strong-jitter performance gate (`contact` max `110.705/114.752ms`; `chaos` max `122.719/126.596ms`). `mr4/mr3 lookupjit` are not viable: chaos hits the same host ARM9 prefetch abort at frame `2678`, and `mr3` mismatches `playerActor0Y` on contact. This rejects "just lower max resim" as a solution for GPU3DLight selected rollback.
  - Fixed a consume-current scheduling bug in `RollbackResimulateIfNeeded`: after `MELONDS_NSML_ROLLBACK_RESIM_CONSUME_CURRENT_FRAME` resimulates through `targetFrame=frame+1`, the normal frame skip must be keyed to `targetFrame`, not the old observed `frame`. With that fix, exact-ish consume-current passes strong-jitter contact/chaos (`18.720/18.836ms`, max `46.763/51.848`; chaos `18.652/18.653ms`, max `45.123/48.566`) instead of running an extra normal frame after the consume. It is still slower than the non-consume exact oracle and below the 60fps target, so consume-current remains diagnostic rather than promotable. A follow-up attempt to skip the final resim checkpoint broke chaos at `playerActor0X` frame `990`.
  - Re-measured exact-ish strong-jitter cost with full rollback stats. The current oracle's cost floor is not checkpoint size: save avg is about `1.2ms`, while restore avg is `~5ms`, resimulated `RunFrame` avg is `~14-15ms`, and resim total avg is `~21-22ms`. This is the concrete reason foreground exact rollback stalls around 18-19ms average under forced corrections.
  - Tried correction coalescing on the selected `mr6 lookupjit` GPU3DLight path with rollback input waits. `rbwait500` reduced one contact max below the previous `110ms+` class but still failed at `94.549/97.350ms` and made chaos unsafe with host ARM9 prefetch abort at frame `2665`; `rbwait1000` pushed averages to `18.5-19.2ms` while keeping `99-125ms` rollback spikes. Rollback input wait is therefore not the selected-path solution either.
  - Added selected lifecycle variants around `mr6 lookupjit`: heap scan interval `60/15`, `widearena10000`, and capped `mr4/mr3` follow-ups. Heap60 did not fix the chaos abort. Heap15/widearena can keep chaos running in some retakes and lower the average toward `16.9-17.2ms`, but the replay count stays too high: one widearena stats run recorded `55` corrections and `323` resimulated frames, with resim total max around `105ms`. Capping to `mr4 heap15` avoids mismatch/abort in chaos but costs `18.4ms` average and `116ms` max; capping to `mr3 heap15` breaks contact (`playerActor0X`) and chaos (ARM9 prefetch abort at frame `2678`). The practical boundary is now clear: the selected path needs fewer/older corrections or shadow/non-blocking replay, not only more lifecycle coverage.
  - Tried reducing `InputMaxFrameLead` on the only correctness-preserving selected lifecycle line (`mr4 heap15`). `lead4` moved the chaos abort later but did not remove it (`frame=3285`) and still had `98-107ms` max frames; `lead3` avoided mismatch/abort in that run but fell to `19.3ms` average with `100ms+` max. This confirms frame-lead capping is another wait-vs-replay tradeoff, not a path to stable 60fps.
  - Exact-ish `jitlookup-compiledchunks-skiprestorejit-cp1-maxlead0-leadbudget2ms` is the current forced-jitter correctness oracle: contact passed at `18.411/18.283ms`, max `43.199/46.064`, and chaos passed at `18.530/18.531ms`, max `43.988/45.987`. It is correct enough for comparison, but not a 60fps candidate.
  - Rejected timing shortcuts around that oracle: `rbwait1000` mismatched chaos at frame `990`; leadbudget `4/6ms` worsened average and mismatched chaos; `InputMaxFrameLead=1/2` reduced average but introduced actor mismatches; a new `MELONDS_NSML_INPUT_FRAME_LEAD_THROTTLE_POLL_US` diagnostic showed `poll100` can remain correct but is slower than baseline (`chaos` `18.719/18.720ms`), while `poll250` worsened average and mismatched chaos.
  - Rechecked foreground correction placement. `cp2+resimDelay2` with `rbwait500/750/1000` remains a contact-only improvement and is not chaos-correct under the stronger jitter. cp1 with `resimDelay1` is chaos-correct but slower than the baseline (`18.619/18.623ms`), while cp1 `resimDelay2` mismatches both contact and chaos. Delaying correction therefore trades timing shape for correctness/average cost; it does not solve the 60fps target.
  - Design implication: the project now has a clean correctness/performance split for strong jitter. The exact-ish path is the validation baseline but is about `18.5ms` average, while selected snapshots are not yet pointer/lifecycle-safe under complex correction. The next serious architecture is still shadow/non-blocking correction or a selected lifecycle snapshot that can match the exact oracle, not more checkpoint byte trimming.
- 2026-06-23 exact/tiny-core continuation after StateSync mismatch detection landed:
  - Added bounded trace and summary-only rollback stats so long practical runs can detect visible FPS drops without producing unbounded logs. The suite now checks StateSync mismatch before perf gates, and `dualstressmid` gives a shorter complex-input route for iteration.
  - Re-ran exact oracles under `InputDelayFrames=2`, `InputSendDelayFrames=2`, `InputSendJitterFrames=2`. `coredelta`/`coreframedelta maxresim8` can be correctness oracles, but they stay around `20-21ms` average with `200ms+` rollback frames. This confirms again that full exact foreground rollback is not playable.
  - Retested `tinycoreramdelta` flags and corrected the oracle boundary. The earlier `0x249`/wifi result was not repeat-stable under the same `dualstressmid` delay2/sendDelay2/jitter2 profile. `devices` (`0x27D`) still mismatched, `devices+VRAM` still mismatched, and `devices+full GPU3D` (`0x37D`) was the lightest repeatable exact-ish pass measured in this continuation. It costs about `21.7ms` average, save `~4.3ms`, restore `~24.9ms`, and resim total `~135-142ms`, so it is a correctness boundary, not a playable path.
  - This changes the design boundary: GPU3D full state, not just GPU3D light/Palette/OAM/VRAM, is part of the exact-ish rollback state for complex MvL routes. However, exact foreground correction remains too slow because it still performs restore plus one or more `RunFrame` replays inside the displayed frame.
  - Returned to selected `nsmbtinycore` after that finding. Adding `tinyFlags=0x37D` to actor arena + ARM9 stack + process-list selected ranges moves the selected path closer to correctness. `heap60` can pass some `dualstressmid` delay2/sendDelay2/jitter2 retakes around `20.4-21.6ms`, with save `~3.0-3.3ms`, restore `~5.3-5.7ms`, and resim total `~27-29ms`.
  - Added `MELONDS_NSML_ROLLBACK_NSMB_ACTOR_ARENA_LENGTH` after field traces showed active object bases around `0x021C1F18`, outside the old fixed actor arena `0x021B2600+0x5000`. `widearena10000` can cover that region and passed one retake, but it averaged about `22.0ms`; `heap1` also passed but averaged about `22.4ms`. The correctness/perf tradeoff is now actor lifecycle coverage vs checkpoint save cost.
  - Design implication: page-delta exact restore is not enough, and selected snapshots cannot be made robust by only patching individual examples. The promising line is a generic selected lifecycle snapshot that covers active-object allocation/list membership deterministically while avoiding per-frame heap scans and full exact foreground resim. A shadow/non-blocking correction path remains the alternative if selected lifecycle correctness keeps failing.
- 2026-06-24 GPU3DLight selected snapshot follow-up:
  - Added selected `nsmbtinycore` candidates using `tinyFlags=0x27D`: CPU/timing/device state plus GPU3DLight, but without full GPU3D VertexRAM/PolygonRAM. The selected Main RAM side remains actor arena + ARM9 stack + process-list ranges + delta-discovered ranges + player object length `0xC80`.
  - No-artificial-delay results are encouraging: `devices-gpu3dlight` passed stocktouch/contact/chaos/dualstresslong around `16.8-16.9ms` average, while full GPU3D selected candidates in the previous pass were typically `20ms+`. The shared death-route failure also reproduced with full GPU3D and exact candidates, so it is not evidence against GPU3DLight by itself.
  - Strong-jitter results show the next wall clearly. With `InputDelayFrames=2`, `InputSendDelayFrames=4`, and `InputSendJitterFrames=4`, `InputMaxFrameLead=2` does not rollback smoothly; it waits inside visible frames and produces `PacketBridgeScratchSpike throttleMs` in the `90-120ms` class.
  - Removing or loosening lead throttling trades the problem for prediction/correction cost. `lead999 + rbwait1000/mr2` mismatched player positions; `rb0/mr8/lead999/limit1` stayed aligned on contact/chaos but still spiked to roughly `120-148ms`; `mr4` reduced spikes near the gate but mismatched, and `mr6` produced an ARM9 prefetch abort in chaos.
  - Existing lead-policy knobs are rejected for this path: predicted-frame lead-throttle skipping and applied-frame lead throttling both produced stalls or 30fps-class averages under this jitter profile. Skipping intermediate resim checkpoints also did not reduce the large correction frames enough.
  - Selected-backend correction-placement retakes did not promote either. Post-frame resim shifted cost poorly and made strong-jitter chaos about 35ms average on one side; `RollbackResimulateDelayFrames=2` preserved correctness in the measured routes but kept max rollback frames around `130-156ms`.
  - Follow-up experiments tried to reduce foreground correction cost without raising `InputDelayFrames=2`. `MELONDS_NSML_ROLLBACK_RESIM_SKIP_AUDIO_BUFFER` did not help (`contact` max `164/152ms`, `chaos` max `322/729ms`). Optimistic future-input bundling that confirmed held future inputs lowered one strong-jitter shape but broke no-artificial `chaos` (`playerActor1X` mismatch at frame `990`), so confirmed future-held input is rejected. A speculative future-input variant avoided confirmed future state but still produced stalls/spikes in no-artificial routes. Capping selected rollback to `mr3` lowered spikes but caused mismatch/ARM abort; `mr4 + future4-held` was near the strong-jitter perf gate (`~83-99ms`) but is correctness-unsafe in normal routes. `RESIM_CONSUME_CURRENT_FRAME` on the GPU3DLight selected candidate also failed (`contact` max `150ms`, `chaos` max `180ms`).
  - Design implication: checkpoint bytes and selected range size are no longer the primary blocker for this candidate. GPU3DLight may be a good local/no-artificial selected snapshot shape, but strong-jitter playability requires a different correction placement model: bounded foreground rollback, shadow/non-blocking correction, or a lead policy that avoids both long in-frame waits and long prediction runs.
...
- 2026-06-23 continuation after skip-restored-JIT validation:
  - Added and measured `cp2`, `PRE_PUMP_BEFORE_RESIM`, `RESIM_SKIP_FINAL_CHECKPOINT`, `future4`, and rollback-input-wait variants around `jitlookup-compiledchunks-skiprestorejit-cp1-maxlead0-leadbudget2ms`.
  - `cp2` is worse because the checkpoint is older and rollbackMs grows. `PRE_PUMP_BEFORE_RESIM` did not reduce the relevant correction cost. `RESIM_SKIP_FINAL_CHECKPOINT` did not improve spikes enough and worsened averages.
  - `RollbackInputWaitUs=500` can reduce over33 counts under delay2/jitter2, but it shifts cost into the normal path and leaves average timing around `19.55ms`; `1000us` still failed the spike gate. Small waits are therefore diagnostics, not a final rollback answer.
  - Future-input bundling is now safer: missing future frames are not filled with the current input. The experiment remains rejected, because without real future input it does not improve the exact rollback path and can still raise correction pressure in complex routes.
  - Practical-suite summaries now support `-MaxAverageFrameMs` so the active goal can reject "smooth but 50fps-class" results instead of relying only on max/over33 counts.
  - Added a no-rollback candidate to the practical suite. `norollback-delay4-lead4-bundle8` on chaos with sendDelay2/jitter2 averaged `16.688/16.692ms` but still had max `39.366/39.851` and over33 `7/5` (`logs/codex-goal-norollback-baseline-delay2jitter2/20260623-091310`). Some 33-40ms spikes are therefore normal execution/input-wait floor, not rollback restore.
  - Current conclusion: the best exact-ish oracle is now small enough to expose the real architectural issue. Stable 60fps with delay2 needs either non-blocking/shadow correction or a correct selected lifecycle snapshot, plus attention to the non-rollback spike floor; more checkpoint slimming, final-checkpoint skipping, or micro-waits are not sufficient.
- 2026-06-23 stats/spike-classification continuation:
  - Added rollback-vs-nonrollback spike classification to the practical suite, plus rollback stats-only tracing (`MELONDS_NSML_ROLLBACK_STATS_TRACE`) so checkpoint/restore/resim costs can be measured without full input trace.
  - Rebuilt melonDS and retested the exact-ish `jitlookup-compiledchunks-skiprestorejit` family under delay2/sendDelay2/jitter2 chaos.
  - Measured cost split for cp1: checkpoint save avg `~1.2-1.3ms`, restore avg `~5.5ms`, resim run avg `~13ms`, resim total avg `~20ms`. The visible rollback spike is therefore not a page-delta/checkpoint byte-size problem anymore; it is foreground restore plus one resimulated `RunFrame` plus normal frame pacing.
  - `rbwait1000` nearly removes rollback correction spikes in the measured chaos run (`0/1` rollback spikes, only `0/2` restores/resims), but average remains `18.817/18.818ms`. This is smoother but still below the active goal's 60fps requirement.
  - Rejected timing-placement experiments: adaptive cp2 and post-frame resim both stayed mismatch-free in one chaos run but worsened average FPS. `RESIM_CONSUME_CURRENT_FRAME` is also rejected because it performs an extra resimulated frame before skipping the normal `RunFrame`.
  - Rejected lead/wait variants: `maxLead2`, `maxLead2+rbwait1000`, no lead limit, leadbudget `4/6ms`, `rbwait750`, and `rbwait1000` with 50us input-wait polling. They either increase prediction/correction count or shift cost into remote wait; poll50 worsened average to `20.152/20.155ms`.
  - Follow-up sweep rejected applied-frame-lead throttling and `InputMaxFrameLead=1`. Applied-frame-lead still produced many corrections (`19.681/19.760ms`; with `rbwait1000`, `19.061/19.061ms`), while `maxLead1` landed near the same 18.8-18.9ms wall as the existing wait/lead tradeoffs.
  - `rbwait1000+cp2` is now the best average diagnostic from this pass (`18.430/18.369ms`) but has rollback spikes around `55-59ms`. `rbwait1000+cp3` drops average to `18.209/18.186ms` but increases spikes to about `79ms`, so widening checkpoint intervals is not a playable fix.
  - A short route retake kept `rbwait1000+cp2` mismatch-free on `contact` and `death`, but both routes still failed the performance gate (`18.4ms` and `18.1ms` average respectively). This keeps cp2 as a useful diagnostic but not a promotion candidate.
  - Adding `RollbackResimulateDelayFrames=2` to `rbwait1000+cp2` is the best foreground exact-ish spike-shaping result so far: chaos had only `2/3` rollback spikes with max `41.352/39.752ms` for rollback frames, and no mismatch. It still failed average (`18.097/18.098ms`) and route retakes remained above `17.5ms`, so it is a useful lower-bound diagnostic, not a solution.
  - Lower wait (`rbwait500`) gets chaos average to `18.000/17.998ms` but worsens rollback spikes and contact max. `rbwait250` and maxLead1 variants increase correction pressure. This reinforces that foreground exact rollback is near its practical limit without changing architecture.
  - Updated design conclusion: a foreground exact rollback can now be made mostly correct, but not yet 60fps-stable at delay2. The next serious design should either make correction non-blocking/shadowed, or return to a Plan-D-style selected lifecycle snapshot that avoids foreground `RunFrame` replay while being validated against the exact oracle.
...
- Tried page512 variants with rollback same-frame wait and correction coalescing. `rbwait1000` lowered chaos max to `92.642/88.892ms`, but contact mismatched `playerActor0X` at frame `2010`; `rbwait500` and `rd2` still missed the spike gate or mismatched (`logs/codex-goal-delay2-page512-tune-contact-chaos/20260623-003124`).
- Retested fast/partial JIT reset masks. `pv-maincore` can pass contact on some runs (`86.883/85.725ms` max in `logs/codex-goal-delay2-fastjit-pv-maincore-recheck/20260622-235813`) but chaos still failed badly (`103.406/160.510ms` max), and lookup-clear/coalesce variants did not promote.
...
- Current best read: this is no longer primarily a checkpoint byte-size problem. With page512/cp1 the exact-ish path can keep state aligned in contact/chaos, but each correction still spends roughly `45-60ms` in `rollbackMs`; the visible frame then lands around `70-100ms`.
- Smaller checkpoints, small waits, and future-input bundling do not remove the visible correction cost under delay2 strong jitter. Future input is especially bad on complex input because it creates later corrections when held-input guesses are wrong.
- A viable new rollback method likely needs to avoid doing all restore/resim work inside the visible frame: e.g. background/shadow-state correction with swap, or a pointer-safe selected lifecycle snapshot that avoids cold-JIT re-execution while still being checked against `tinycoreramdelta`.
...
- Follow-up after the JIT reset boundary work: raising `InputDelayFrames` above `2` remains rejected, and simply moving to `RollbackInputWaitUs=250/500` does not solve the spike problem. Under delay2 strong jitter, `skipjit-rbwait250/500-cp1` preserved correctness on chaos/dualstresslong but still failed smoothness and often worsened averages.
- The current best exact-ish practical candidate is `tinycoreramdelta` page256/cp1/maxResim1 with JIT reset skipped and restored Main RAM ranges conservatively invalidated at 16-byte granularity. It passed contact and stayed mismatch-free on chaos/dualstresslong in `logs/rb-skipjit-preciseinv-repeat-dual/20260622-185347`, but chaos and dual still fail the smoothness target (`~18ms` average, max around `93-100ms`, high over33 counts).
- Two earlier attempted shortcuts remain rejected: 512-byte CodeMem bitmask pruning of restored-range invalidation caused contact `playerActor0X` mismatch at frame `2820`, and older larger-page sweeps (`512`/`1024`) mismatched contact. The latest page512/cp1 retake is correctness-closer but still not promotable because chaos remains too spiky; page256 remains the conservative oracle baseline.
- Additional rejected cost cuts: `resimdelay1` and checkpoint interval `2` did not improve the skip-JIT precise-invalidation path, 512-byte code-chunk invalidation still reproduced the frame `2820` contact mismatch, and skipping the final resim checkpoint caused immediate contact/chaos actor-position mismatches. The final resim checkpoint is therefore part of the correctness mechanism, not just redundant overhead.
- Added `jitInv*` rollback trace fields and reprofiled the delay2 `tinycoreramdelta` cp1/skipJIT path. Contact/chaos stayed mismatch-free in `logs/rb-jitinv-instrument-baseline/20260622-194454`, but still failed smoothness. Trace in `logs/rb-jitinv-trace-baseline/20260622-194833` showed full-keyframe restore chains: the first 4MB Main RAM restore can spend about `61-69ms` in restored-range JIT invalidation alone.
- Rechecked `tinycorepreimage` as the lighter exact-style restore path. `maxResim2` is now correctness-unsafe under the current strong-jitter suite (`contact` frame `2010` `playerActor0X`, `chaos` frame `1050` `playerActor1X`). `maxResim3` is the best preimage point found in this pass: contact passed and chaos/dualstresslong stayed mismatch-free, but chaos and dual still failed smoothness (`~18.5-19.9ms` average, max `103-133ms`).
- `tinycorepreimage maxResim3` trace confirms the remaining exact rollback cost is mostly same-frame re-execution: restore is only `~3-6ms`, while three resimulated `RunFrame`s commonly cost `40-70ms` total. Smaller checkpoints alone will not reach stable 60fps under delay2/send6/jitter6; avoiding or moving same-frame resimulation is the next architectural problem.
- Rejected follow-ups in this pass: `rbwait250/500` with preimage maxResim3 worsened averages, `prepump` with maxResim3 mismatched, `skipmidcp` with maxResim3 mismatched contact, and a preimage prune boundary cut broke correctness. These remain diagnostics only.
...
- Plan-D actor lifecycle experiment:
  ...
  - Current interpretation: Plan-D repair cannot be made exact by patching individual examples such as hazard/star. The next version needs a generic active-object lifecycle model, including spawn/despawn/remap/process-list semantics, and must be checked against the exact `tinycoreramdelta` oracle.
  - Star-candidate spawn through `kA2DEActorSpawnActorAddr` is rejected for playability: `starsnapspawn` caused `~100ms` spike clusters and stall. Dormant `0x10C` activation is much lighter but does not solve broad state drift by itself.
  - Moving-hazard dormant activation plus last-known-base restore is a useful diagnostic but not complete. It can remove early `movingHazardX` drift in some runs and keep average frame time near `16.8ms`, but chaos still fails when a host-side `0x53` actor has disappeared from the client process lists around frame `3900`. This is now a process-list/lifecycle problem, not just a coordinate snapshot problem.
  - Exact `tinycoreramdelta` remains the correctness oracle but not the playable path under delay2/send6/jitter6. The latest chaos retake had no mismatch but failed smoothness (`18.214/18.239ms` avg, max `112.616/122.464`); fast JIT reset stayed around `105ms` max and did not solve the resim spike.
...
- Tiny core + Main RAM delta rollback:
  ...
  - It confirms the current tradeoff: exact-ish rollback can remove mismatch under delay2 strong jitter, but same-frame rollback still causes visible spikes. At this stage the best measured candidate was `exact-delay2-tinycoreramdelta-key2-page256-cp2-maxresim1-skiprender`; later cp1/full-reset checks are the current correctness oracle.
...
- Fast JIT reset follow-up:
  ...
  - `exact-delay2-fastjitreset-nodelete0-tinycoreramdelta-key2-page256-cp2-maxresim1-skiprender` preserved correctness in an initial contact/chaos/dualstresslong pass under strong jitter, but still missed the playability target. Contact had max `106/101ms`; chaos/dualstresslong averaged around `18.0-18.15ms` with max `90-100ms` and too many over33 frames. Later retakes demoted fast reset back to diagnostic-only.
...
- 2026-06-22 rollback JIT restore-candidate follow-up:
  - Added `MELONDS_NSML_ROLLBACK_JIT_FAST_RESET_CLEAR_LOOKUP=1` to clear fast lookup tables during rollback fast reset, and `MELONDS_NSML_ROLLBACK_JIT_FAST_RESET_RESTORE_CANDIDATES=1` to move detached blocks into the existing validate-before-reuse `RestoreCandidates` path instead of deleting them immediately.
  - `RestoreCandidates` now use a stronger key containing instruction hash, block address, and CPU number. The old instruction-hash-only key can lose same-instruction blocks at different addresses during mass rollback detachment, which makes candidate reuse order-dependent.
  - This is useful diagnostic infrastructure, but not a promotion path yet. `jitrc-cp1` showed the expected performance direction by lowering cold resim execution in some traces, but repeated strong-jitter runs still mismatched (`playerActor0X` at frame `2010` on contact and frame `990` on chaos in `logs/rb-jitrc-keyed/20260622-170341`).
...
- Exact same-frame resim is not yet a playable endpoint for delay2. The closest exact-ish page512/cp1 result is state-correct on contact/chaos but still too spiky because correction work happens inside the visible frame.
...
- Continue using `tinycoreramdelta` as the correctness oracle. The next practical direction should shift away from more page-size/wait tuning and toward either a generic active-object lifecycle snapshot/apply model validated against that oracle, or a new rollback architecture that avoids doing all correction resimulation inside the visible frame.
