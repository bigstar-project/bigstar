...
- Current design implication: checkpoint byte trimming and same-frame correction scheduling are exhausted for the current foreground architecture. Exact rollback is bounded by restore + resimulation placement; Plan-D can hit the FPS target but needs a semantic authoritative-state model. The immediate Plan-D hazard direction is host-authored moving-hazard freeze with targeted collision/death validation, not unsafe process-list relinking. In parallel, continue root classification of deterministic player/global/star state instead of broad page widening or single-symptom actor patches.
