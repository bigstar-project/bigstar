...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は6,076行（リファクタ開始前21,253行から15,177行、約71.4%減）で、NDS依存のdiagnostic snapshot RAM採取は既存Readerへ、全diagnostic eventの純粋JSON整形とscreenshot／Main RAM dump artifact生成は既存Diagnosticsへ、現役player runtime patch 5群の低レベルRAM書込みは既存Writerへ、test savestate／LocalMP snapshotのI/Oとbarrierは299行の`NsmbTestStateHarness`へ移った。PoCには共有`State G`、診断eventの発火・snapshot選択、NDSからframebuffer・register・Main RAMを受け渡す薄いadapter、テスト用runtime patchの適用条件とreport、各subsystemを呼ぶframe orchestrationが残る。process environmentの直接読取は0箇所になった。brace境界で再計測した現在の`State`定義は43行、`InitFromEnvironment`は182行。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は28行のオーケストレーションまで縮小した。file数や短いfileを増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめ、死んだ実験コードが見つかれば総量も削る。
...
- Next action: 182行の`InitFromEnvironment`と43行の共有`State G`は既にオーケストレーションとして無理のない大きさなので、行数だけを目的に分解しない。次は残る6,076行を数十行単位で刻まず、input／PacketBridge／rollback／MvL／diagnostics orchestrationから数百行規模の責務まとまりを選び、複数の未使用feature監査と合わせて大きめのバッチで処理する。複数の現役consumerを持つまとまりは中規模moduleのまま保ち、一般`STATE_APPLY`、GUIの`STATE_SYNC`、観測trace、自然同期World gateは維持する。高コストなfast／standard実ROMgateは各バッチ末尾に集約する。
- Verification status: Release `melonDS`／config test build、CTest 16件（0.51秒）、変更したPoC／Config／新`NsmbTestStateHarness`自身のClang unused診断0件がpassした。`logs/refactor-state-harness-fast`のpacket loss付き同一実ROM・2-process 1,250-frame fast tierはsemantic goldenをpassした。`logs/refactor-state-harness-standard-{host,client}`では両peerが1,700 framesを完走した（129.00秒、13.18 FPS）。targeted clean-state確認では`logs/refactor-state-harness-adapter/attempt1`でframe 900の2 NDS state各19,228,045 bytesとLocalMP 131,228 bytesを保存し、split側で両NDS stateを正常ロードしたが、`-Fast` sourceはROM menuを実時間で進める前にtest frameだけ先行して`stageGroup=0xffff`だったためgameplay semantic gateには不適格だった。安定frame 3,200のsource再生成は既知の2-instance進行ばらつきで外側5分timeoutへ達したため、成立したfast／standard gameplay gateとは分けて記録する。runner parse、削除対象参照0件、`git diff --check`もpassしている。今回GUI／Rust差分はない。
