...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は9,386行（リファクタ開始前21,253行から11,867行減）で、単一の共有 `State G`、3箇所のprocess environment直接読取、game-state hash送信、テスト用runtime patchがまだ集中している。正確なtop-level境界で測ると、現在の`State`定義は164行、`InitFromEnvironment`は665行。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は28行のオーケストレーションまで縮小した。file数や短いfileを増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめ、死んだ実験コードが見つかれば総量も削る。
...
- Next action: 残る665行の`InitFromEnvironment`と164行の共有`State G`をfeature group単位で監査する。現存するplayer/star/powerup/script/JIT runtime patchはAI収集・smoke・goldenから現役利用されていることを確認済みなので維持する。次はhash logのstream/重複frame管理とgame-state hash送信境界を候補にし、`NsmbAiObservation.cpp`へ広く結合しているAI streamは責務がまとまる境界が得られるまで細分化しない。抽出による読みやすさと未使用実験コードの純減は分けて計測する。
- Verification status: Release `melonDS` build、PoC/GameStateのClang unused診断、CTest 13件（0.30秒）がpassし、game-state contract testは20回連続passした。固定seed・packet loss・4-frame input delay付き2-process実ゲーム回帰は最終codeで`logs/refactor-game-state-trace-runtime-fast`と`logs/refactor-game-state-trace-runtime-rollback`を各1,250 frames完走し、host/client semantic CSVのbyte完全一致、applied input、checkpoint coverage、golden一致を確認した。Diagnostics/heartbeatの20回連続testと実ROM diagnostics tier、Input recorderの実ROM内容一致、Coordinator contract testと同一process実ROMのbarrier/serial/NDS+LocalMP save-load回帰も引き続きpassしている。state sync/apply専用`logs/refactor-state-sync-builders-apply`ではplayer/world/actor snapshot実適用とmoving-hazard GUID map更新を確認済み。送信delay/jitterの実queue経路は`logs/refactor-input-delivery-delay-jitter`、ImitationAI production pathは`logs/refactor-mvl-ai-imitation`、GUIが有効化する`coredelta`は`logs/refactor-remove-rejected-rollback-coredelta`が引き続きpassしている。
