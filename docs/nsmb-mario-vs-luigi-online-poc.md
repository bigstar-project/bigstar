...
- Next action: 182行の`InitFromEnvironment`と43行の共有`State G`は既にオーケストレーションとして無理のない大きさなので、行数だけを目的に分解しない。約250行のstate save/load harnessも現役のclean-state adapter smokeが利用しているため維持する。次は残る6,464行のinput／PacketBridge／rollback／MvL／diagnostics orchestrationを責務・参照元・実行scenario別に再監査し、複数の現役consumerを持つまとまりは中規模moduleのまま保ちつつ、PoC試行時だけの未使用feature groupを優先して縦に削除する。一般`STATE_APPLY`、GUIの`STATE_SYNC`、観測trace、自然同期World gateは維持する。
- Verification status: Release `melonDS`とconfig test targetのbuild、CTest 16件（1.62秒）、変更した`ARM.cpp`／`NsmbNetplayPoC.cpp`／`NsmbNetplayConfig.cpp`本体のClang strict unused診断がpassした。`logs/refactor-remove-net-ready-overrides-fast`のpacket loss付き同一実ROM・2-process 1,250-frame fast tierはsemantic goldenをpassした。`logs/refactor-remove-net-ready-overrides-standard`のsoftware renderer・2-process 3,000-frame standard tierも完走し、host／client各10 PNGを生成した。runner parse、削除対象のrepo全体参照0件、`git diff --check`もpassしている。今回GUI／Rust差分はなく、新規fileもない。
