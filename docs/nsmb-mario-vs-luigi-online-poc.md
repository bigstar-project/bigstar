...
- **Current conclusion:** 最新手動run `logs/nsmb-mvl-manual-local-20260818-002906` の完全なrole別入力とseed `0xC6D26F70` を再生し、恒久差を自動再現できた。`main` 統合後に開始barrierが既定無効になっていた回帰を直して開始前訂正を除いたうえで、残る原因をROM-loop中のgame render process省略へ絞った。NSMBのrender processは表示専用ではなく、少なくとも移動足場の生成状態へ副作用を持つため、中間tickで省略してはいけない。
- 制御試験ではframe 3487のdepth 4訂正を単独で行うと、render省略版はframe 3517でmoving hazard/object状態がlockstepから恒久分岐した。同じ訂正で全中間tickのgame render processを実行すると、host/clientとも比較可能な全game-state fieldがframe 3520までlockstepと一致した。frame 3482/3487/3492の近接3訂正でも差分ゼロだった。
- 完全入力5400-frame stressはhost 346件、client 330件のROM-loop transactionをすべて完了し、cannot-armはゼロだった。60-frame lockstep標本との差は未確定remote inputを表示中のplayer一時差だけで、後続標本と最終frameでは再一致した。これは全イベントの証明ではないが、今回ユーザーが観測した初期からの恒久差は自動再生上で解消した。
- presentationは訂正直後frame 3494でnormal controlと `1128-1917 / 98304` pixelsの差が出たが、6 display frames後の3500では両roleとも全画素一致した。長時間600-frame間隔比較ではhostの600-4800が全点一致、clientは1800/2400の投機中だけ差があり3000以降5400まで再一致した。描画破損の蓄積は検出していないが、訂正直後の短いsprite位置補正は残る。
...
- 最新手動run `logs/nsmb-mvl-manual-local-20260818-132459` は初戦で目視上の恒久ずれを再発しなかった一方、characterと地形、とくにLuigi側の土管X座標が周期的にぶれるちらつきを再現した。これは通常のrollback位置補正やcamera揺れだけではない。同一seed・role別完全入力を毎frame PNG保存した `logs/codex-client-pipe-x-jitter-replay-20260818` では、Luigi側の右端の土管がframe 1126でX=`213..243`（幅30px）、訂正完了直後1127で`211..249`（幅38px）、1128で`208..238`（30px）、次の訂正完了直後1129で`206..241`（35px）、1130で`203..233`（30px）となった。通常camera移動は左端の `-2/-3px` に現れているのに右端だけ逆方向へ伸びるため、複数tickの異なるX位置にあるpipe geometryが一つのdisplay frameへ重複submitされた証拠である。既存image controlでもframe 3487のdepth 4訂正後、frame 3494のlockstep画像には土管がないのにROM-loop画像だけ土管が先行表示され、frame 3500で一致する。訂正中に必要なgame render callbackと、表示へ漏らしてはいけないOAM/GPU submission・render cacheの分離が未完成である。最新手動runは初戦だけでhost 979件、client 986件の訂正を行い、clientはdepth 5/6が146件に対しhostは15件だったため、Luigi側ほど中間renderの重複が目立つ観測とも整合する。
...
- **Current blocker:** 初戦のstate収束候補は残るが、訂正直後の地形を含む表示ちらつきと再戦timeline rebaseは明確なgameplay blockerである。未通過event（死亡/復帰、土管遷移、item）、音声波形、depth 11超fallbackも未検証である。
- **Next action:** 性能最適化より先に、再演算中もgame側render callbackは実行しつつ、各中間tickの3D FIFO/OAM submissionを表示用queueへ蓄積しない境界を作り、訂正後の最終tickだけを提示する。frame 1126-1130のpipe幅・frame 3494/3500画像gate・完全入力stressを同時に通し、単にrender callbackを再び省いて恒久stateを壊す修正は採用しない。その後、再戦時にrollback timelineとpending mismatchをgeneration単位で消去/rebaseし、二戦連続gateを追加する。
