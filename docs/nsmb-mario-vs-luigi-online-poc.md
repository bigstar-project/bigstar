...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は8,713行（リファクタ開始前21,253行から12,540行減）で、単一の共有 `State G`、game-state sample採取、テスト用runtime patchがまだ集中している。process environmentの直接読取は0箇所になった。brace境界で再計測した現在の`State`定義は59行、`InitFromEnvironment`は468行（旧記載の656行は現ソースと矛盾していたため修正済み）。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は28行のオーケストレーションまで縮小した。file数や短いfileを増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめ、死んだ実験コードが見つかれば総量も削る。
...
- Next action: 残る468行の`InitFromEnvironment`と59行の共有`State G`をfeature group単位で監査する。現存するplayer/star/powerup/script/JIT runtime patchはAI収集・smoke・goldenから現役利用されていることを確認済みなので維持する。次はNDS依存のPacketBridge capture/apply/tick orchestrationを今回の同じ中規模moduleへ自然に寄せられるか、またはrepo内で旧`PacketBridge.Enabled/Only`経路を実際に呼ぶlegacy launcher群ごと削除可能かを利用実績から判断する。新しい小fileは増やさず、抽出による読みやすさと未使用実験コードの純減は分けて計測する。
- Verification status: Release `melonDS` build、PoCとPacketBridge runtimeのClang strict unused診断、CTest 16件（0.37秒）がpassし、新しいPacketBridge runtime contract testは初回に加えて20回連続passした。`logs/refactor-packet-bridge-runtime-fast`のpacket loss付き2-process実ゲームfast tierは1,250 framesをpassし、semantic traceとgoldenが一致した。JIT helper初回適用は複数の上位runで両peer同一frame（870または900）を確認した。重い第2試合gateは上記のfixture非決定性により今回pass証拠としては採用しない。直前の`logs/refactor-diagnostic-ring-runtime` diagnostics tier、変更前の`logs/refactor-mvl-runtime-secondgame` 10,000-frame第2試合、AI logのneutral/linear-model byte一致、hash、state sync/apply、送信delay/jitter、GUIの`coredelta`各production回帰も保存している。
