...
- **Current blocker:** P=7の連続frontier、hard wait、depth 7訂正、timeoutは短時間自動gateを通った。残るnetwork blockerは、固定12-entry bundleで回復できる条件しか試しておらず、Slippi型contiguous ACK/未ACK列、packet loss・重複・再順序化・短時間断を未検証な点である。描画・音声・死亡/復帰・土管遷移・item・再戦を含むP=7長時間手動gateも未完である。常時8-9 frame遅延は同期を保ったが単発40-53msとなるため、実用快適域とは扱わない。再戦checkpoint復元の約`0.15秒`停止とstage開始直後hitchも別に残る。
- **Next action:** Bigstarは変更せず、manual scriptのP=7を維持してcontiguous ACK/未ACK再送とloss・再順序化・短時間断を試す。その後D=0/1/2の比較、depth 7訂正区間の画像・音声artifact、複数再戦を含む長時間手動操作を順に通す。
