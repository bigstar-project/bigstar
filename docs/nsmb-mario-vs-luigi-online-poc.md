...
- **Practical control policy:** ユーザーの評価順を受け、P=5を先に製品候補とする判断を撤回した。Bigstar GUI経路は触らず、manual `-SlippiRollback`を`input delay D=2 / prediction horizon P=7 / ROM history H=12`へ変更した。generation別highest-contiguous-confirmed frontierを持ち、7個までは予測、8個目の実行前に停止してnetwork pumpと12-entry bundle再送だけを行う。7秒でexit 73となる。旧`InputMaxFrameLead`はrollback時に使わず、逆順packetでも対象frameより前の最新確定入力を予測値にする。P有効時の事後clampは削除し、範囲外不一致をfatal invariantにした。
...
- **Audio gate:** 最終SDL PCMとSPU startを同一入力lockstep対照へ比較した。9訂正のhostはSPU start `96 -> 104`、sample source欠落0・余分8で波形gate不合格、訂正0のclientは`97 -> 97`で余分・欠落0かつ波形gate合格だった。候補hostの13 startはROM-loop transaction中に発行された。全13件を抑止すると余分8件以外の正しいstartも失うため、単純mute/一律抑止は採用しない。診断有効の候補は実効`59.999/59.999fps`、outer最大`19.188/19.198ms`、25ms超・対戦区間underrunとも0であり、今回の失敗原因は性能やspeaker供給切れではない。
- **Current blocker:** network loss・重複・再順序化・短時間断、再戦、主要event、地形表示は自動gateを通った。現時点の最重要blockerは、Main RAMだけを戻してARM7/SPU timelineを戻さないROM-loopが効果音を再発行する点である。player spriteは恒久差ではないが訂正直後の完全一致でもなく、最大3 frameの位相差を残す。常時8-9 frame人工遅延、再戦checkpoint復元の約`0.15秒`停止、stage開始直後hitchも快適性上の別制約として残る。
- **Next action:** Bigstarは変更せず、sound command/sample sourceをgame tick単位で遅延commit・置換する小さなledger、または音声専用rollback stateを比較する。SPU sourceの余分・欠落0、両role PCM gate、対戦中実効`59.5fps`以上を同時に満たせなければ、ROM-loop方式の製品限界として扱う。その後に実聴と長時間手動gateへ進む。
