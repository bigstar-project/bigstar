...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は20,175行（リファクタ開始前21,253行から1,078行減）で、単一の共有 `State G` に参照中のfieldが782個、381種類の `MELONDS_NSML_*` 設定、ENet、入力同期、rollback、NSMB RAM access、状態同期、診断、テスト用runtime patchがまだ集中している。`InitFromEnvironment` は1,312行、`BeforeRunFrame` は383行、`AfterRunFrame` は371行。
...
- Next action: 残るgame-state受信をhandlerへ分けた直後、同一file内の小関数化を続けず、wire型・codec・受信dispatchを別moduleへ物理移動して`NsmbNetplayPoC.cpp`の行数を明確に減らす。その後、rollback storeを抽出する。各段階でfast/rollback実ゲームgoldenを維持する。
