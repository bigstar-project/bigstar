...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は7,681行（リファクタ開始前21,253行から13,572行減）で、NDS依存のdiagnostic snapshot RAM採取は既存Readerへ、全diagnostic eventの純粋JSON整形は既存Diagnosticsへ、現役player runtime patch 5群の低レベルRAM書込みは既存Writerへ移った。PoCには共有`State G`、診断eventの発火・snapshot選択、テスト用runtime patchの適用条件とreport、各subsystemを呼ぶframe orchestrationが残る。process environmentの直接読取は0箇所になった。brace境界で再計測した現在の`State`定義は43行、`InitFromEnvironment`は182行。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は28行のオーケストレーションまで縮小した。file数や短いfileを増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめ、死んだ実験コードが見つかれば総量も削る。
...
- Next action: 182行の`InitFromEnvironment`と43行の共有`State G`は既にオーケストレーションとして無理のない大きさなので、行数だけを目的に分解しない。現役player runtime patchのWriter境界は完了したため、次はscript remote packet、PacketBridge JIT helper、memory patch、net-random/direct MvL補正をfeature group単位で参照監査する。現役機能は維持し、参照のない旧実験だけを削除する。現役機能でも既存Runtime/Reader/Writerへ責務単位でまとまる場合だけ移し、新しい小fileは増やさない。input recorder/game-state traceのwriter ownershipと、packet handler callbackを必要とするnetwork pump threadは現状の境界を維持する。既存moduleが過大になった場合は、その時点で内部の凝集した責務を中規模fileとして切り出す。
- Verification status: Release `nsmb_game_state_tests`/`melonDS` build、PoC/WriterのClang strict unused診断、CTest 16件（0.71秒）がpassした。`logs/refactor-player-runtime-writer-fast`のpacket loss付き2-process fast tierは1,250 frames、route smoke、semantic goldenをpassした。`logs/refactor-player-runtime-writer-targeted-final`の2-process実ゲームは1,050 framesを11.1秒で完走し、全5 runtime patchの両peer発火、指定RAM値、player-to-star座標一致をextended traceで確認した。Diagnostics contract 20回連続、linear/compact実model、startup/model report、diagnostic snapshot/JSON、memory patch、MvL marker、AI log生成のproduction回帰も引き続き保存している。重い第2試合gateは上記のfixture非決定性によりpass証拠としては採用しない。
