...
- **Current blocker:** 初戦同期、stdout起因の秒単位停止、再戦logical timelineは手動gateまで通過した。再戦checkpoint復元の約`0.15秒`停止とstage開始直後の`40-53ms`級hitchは残り、対戦中の持続的60fps低下とは別だが製品上の無停止要件は未達である。現行手動gateの通信条件はローカルENetへ片道2-3 frameの送信待ちを加えたもので、実WANの輻輳、再順序化、帯域制約、切断は再現していない。土管Xぶれの消失そのもの、未通過event（死亡/復帰、土管遷移、item）、2D OAMちらつき、音声波形も未検証である。特にdepth 11超を事後clampする現状はSlippi相当の安全fallbackではなく、製品prediction上限に達する前の待機へ置き換える必要がある。
- **Next action:** まずoldest contiguous confirmed remote frame基準のhard prediction horizonと安全な待機を実装する。そのうえでlocal input delay 0/1/2を同一人工RTT・jitterでA/Bし、rollback深度分布、cannot-arm、継続state差、outer hitchと操作遅延を測る。再戦checkpoint復元とstage warm-upのhitchは別のperformance課題として計測・削減する。
