...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は8,597行（リファクタ開始前21,253行から12,656行減）で、単一の共有 `State G`、game-state sample採取、テスト用runtime patchがまだ集中している。process environmentの直接読取は0箇所になった。brace境界で再計測した現在の`State`定義は45行、`InitFromEnvironment`は406行（旧記載の656行は現ソースと矛盾していたため修正済み）。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は28行のオーケストレーションまで縮小した。file数や短いfileを増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめ、死んだ実験コードが見つかれば総量も削る。
...
- Next action: 残る406行の`InitFromEnvironment`と45行の共有`State G`をfeature group単位で監査する。input recorderとgame-state traceは既に各専門moduleのwriterがfile lifecycleを所有しており、diagnosticsへ再統合すると責務が逆戻りするため維持する。次は実際の適用状態であるmem patch marker/rangeとnetwork pump thread lifecycleを、既存のHarness/Transport各moduleへcallback依存を増やさず寄せられるか判断する。新しい小fileは増やさず、まとまった既存moduleへの集約、オーケストレーションの可読化、未使用実験コードの純減を分けて計測する。
- Verification status: Release `melonDS` build、PoCとCoordinatorのClang strict unused診断、CTest 16件（0.39秒）がpassし、Coordinator contract testは初回に加えて20回連続passした。`logs/refactor-coordination-bookkeeping`のpacket loss付き2-process fast tierは1,250 framesを完走し、両peerでframe-limit終了とsemantic golden一致を確認した。旧lockstep分岐はこのfast設定では到達しないためcontract testで保護する。直前の`logs/refactor-runtime-patch-log-extended`ではframe 900のplayer powerup `1/2`と各peer一度だけのforce-powerup report、`logs/refactor-mvl-marker-runtime`ではnet-random適用、`logs/refactor-player-life-runtime`では両peer各3件の`player_life_change`、`logs/refactor-ai-config-open-runtime`ではAI log生成とneutral policy評価がpassしている。JIT helper初回適用は複数の上位runで両peer同一frame（870または900）を確認した。重い第2試合gateは上記のfixture非決定性により今回pass証拠としては採用しない。さらに変更前の`logs/refactor-mvl-runtime-secondgame` 10,000-frame第2試合、AI logのneutral/linear-model byte一致、hash、state sync/apply、送信delay/jitter、GUIの`coredelta`各production回帰も保存している。
