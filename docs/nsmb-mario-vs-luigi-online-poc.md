...
- Latest delay-2 exact-ish Main RAM delta experiment:
  ...
  - Latest JIT reset experiments:
    ...
    - `exact-delay2-fastjitreset-nodelete0-tinycoreramdelta-key2-page256-cp2-maxresim1-skiprender` is the current correctness-oriented baseline: contact/chaos/dualstresslong under `InputSendDelayFrames=6` / `InputSendJitterFrames=6` had no persistent mismatch, but still failed playability. Contact: `17.132/16.952ms`, max `106.596/101.831` in `logs/nsmb-mvl-rollback-fastjitreset-nodelete0-contact/20260622-150136`; chaos/dualstresslong: averages around `18.0-18.15ms`, max `90-100ms`, over33 up to `167/162` in `logs/nsmb-mvl-rollback-fastjitreset-nodelete0-chaos-dual/20260622-151614`.
...
- Next actions: do not continue trying to fix this by increasing InputDelay. Use full-reset cp1 `tinycoreramdelta` as the correctness oracle, then either find a pointer-safe way to preserve/reuse enough rollback JIT state across restore, or move to a new rollback architecture that avoids same-frame cold-JIT resim spikes while preserving exact state. Keep fast/skip-JIT-reset variants diagnostic-only until they pass repeated contact/chaos/dual routes.
