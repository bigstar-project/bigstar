...
- Current blocker: with `InputDelayFrames=2`, exact rollback still spikes too much, while the lightweight predict/repair path is fast but cannot reliably know current-frame discontinuities under artificial 6F send delay/jitter. Raising GUI rollback delay above `2` is explicitly not acceptable for this goal.
- Next actions: move beyond repair-only snapshots toward a new delay-2 architecture such as off-main-thread shadow resim, a ROM/actor-level authority model for transition/death/object lifecycle, or minimal MvL actor rollback that can publish exact player/world state without blocking the main frame. Event-heavy routes for star pickup/drop, block break persistence, and 8-coin item identity still need to be added.
