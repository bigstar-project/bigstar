...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は12,632行（リファクタ開始前21,253行から8,621行減）で、単一の共有 `State G`、27箇所の `MELONDS_NSML_*` 設定参照、ENet、入力同期、診断、テスト用runtime patchがまだ集中している。正確なtop-level境界で測ると、現在の`InitFromEnvironment`は772行、`BeforeRunFrame`は269行、`AfterRunFrame`は172行。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は48行のオーケストレーションまで縮小した。file数を増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめる。
...
- Next action: 残る772行の`InitFromEnvironment`と共有`State G`をfeature group単位で監査し、repo内launcher/test/docsから使われないPoC実験を削除する。rollback部は拒否済みbackend削除後にgeneric checkpoint codec・preimage・resimulationへ絞れたため、transport/state syncとの規模を比較し、最も境界が明確な一まとまりを既存moduleまたは一つの中規模moduleへ物理抽出する。数十行ごとの細分化は行わない。
- Verification status: Release `melonDS` build、Clang unused診断、PowerShell変更3本のparser検査、CTest 10件（0.13秒）がpass。固定seed・packet loss付き2-process実ゲーム回帰は`logs/refactor-remove-rejected-rollback-fast`と`logs/refactor-remove-rejected-rollback-rollback`で、savestate/tinycorepreimage双方のhost/client semantic state、applied input、checkpoint coverage、golden一致を確認した。GUIが有効化する`coredelta`も`logs/refactor-remove-rejected-rollback-coredelta`で1,250 framesを完走し、両peerで45 checkpoint save、delta/keyframe生成、host/client gameplay sync、abort/restore integrity errorなしを確認した。
