...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は11,837行（リファクタ開始前21,253行から9,416行減）で、単一の共有 `State G`、27箇所の `MELONDS_NSML_*` 設定参照、入力同期、診断、テスト用runtime patchがまだ集中している。正確なtop-level境界で測ると、現在の`State`定義は363行、`InitFromEnvironment`は603行、`BeforeRunFrame`は269行、`AfterRunFrame`は172行。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は48行のオーケストレーションまで縮小した。file数を増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめる。
...
- Next action: 残る603行の`InitFromEnvironment`と共有`State G`をfeature group単位で監査し、repo内launcher/test/docsから使われないPoC実験を削除する。input prediction、delivery、session、transportとgame-state RAM readerの所有権は既存moduleへ揃ったため、次はlocal/remote input履歴とbarrier orchestration、またはstate sync cache/send/applyのうち、直接参照を閉じられる一まとまりを選ぶ。数十行ごとの細分化は行わない。
- Verification status: Release `melonDS` build、PoC/Reader双方のClang unused診断、CTest 11件（0.17秒）がpass。最新の固定seed・packet loss付き2-process実ゲーム回帰は`logs/refactor-player-probe-reader-fast`と`logs/refactor-player-probe-reader-rollback`で、savestate/tinycorepreimage双方の1,250 frames完走、host/client semantic state、applied input、checkpoint coverage、golden一致を確認した。移動したtile probe/gridを実消費するRuleAI production pathも`logs/refactor-player-probe-rule-ai-direct`で1,250 frames完走し、trace上でground/ahead/left/right地形観測を確認した。送信delay/jitterの実queue経路は`logs/refactor-input-delivery-delay-jitter`、ImitationAI production pathは`logs/refactor-mvl-ai-imitation`がpassしている。GUIが有効化する`coredelta`の`logs/refactor-remove-rejected-rollback-coredelta`も引き続きpassしている。
