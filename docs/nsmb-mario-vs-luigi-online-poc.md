...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は17,106行（リファクタ開始前21,253行から4,147行減）で、単一の共有 `State G`、342箇所の `MELONDS_NSML_*` 設定参照、ENet、入力同期、診断、テスト用runtime patchがまだ集中している。現在の`InitFromEnvironment`は1,393行、`BeforeRunFrame`は613行、`AfterRunFrame`は476行。旧803行の`ReadGameStateSample`は37行のオーケストレーションまで縮小した。file数を増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめる。
