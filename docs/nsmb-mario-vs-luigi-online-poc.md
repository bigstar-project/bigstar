...
- 監査完了: 残るBig Star／moving-hazard補正はGUI・通常fast tierから使われず、旧`-PlanDActorSnapshot`と専用World scenarioだけが有効化している。`MvlMatchSeed=0x00000100`の同一実ROM・2-process 1,100-frame条件で補正あり`logs/refactor-remove-player-state-world`と補正なし`logs/refactor-audit-world-natural`をA/B比較したところ、両方ともBig Star／hazardを各7回比較してX/Y drift 0だった。通常fast tierも補正なしでpassしているため、wire/send/apply/configは未昇格の旧Plan-D補正として削除対象と判断した。一方、game-state traceと`-RequireWorldSnapshotSync` gateは補正なしの自然同期も直接検査できるため維持する。
...
- Next action: 182行の`InitFromEnvironment`と43行の共有`State G`は既にオーケストレーションとして無理のない大きさなので、行数だけを目的に分解しない。旧world item spawn、generic actor snapshot、effect snapshot、`PlayerState`の削除は完了し、Big Star／moving-hazard補正もA/B監査で削除対象と確定した。次は両wire packet、remote store/cache/GUID map、Reader/Writer、network、config、runnerと、役目を失う`-PlanDActorSnapshot`を一括削除する。一般`STATE_APPLY`、GUIの`STATE_SYNC`、観測trace、自然同期World gateは維持する。
- Verification status: Release `melonDS`と関連test target build、変更した主要C++ 7 translation unitのClang strict unused診断、CTest 16件（1.46秒）、Diagnostics contract 20回連続がpassした。今回GUI/Rust差分はない。`logs/refactor-remove-player-state-snapshot-fast`のproduction寄りpacket loss付き2-process 1,250-frame fast tierは約15秒でsemantic goldenをpassした。`logs/refactor-remove-player-state-world`の補正ありWorld tierと`logs/refactor-audit-world-natural`の補正なしA/B tierは、同一実ROM・2-process 1,100 framesでどちらもBig Star／moving-hazard各7回のX/Y drift 0だった。PowerShell runner 3本のparseと`git diff --check`もpassしている。
