...
- 2026-06-26 continuation summary:
  ...
  - Foreground scheduling retakes: tightening `InputMaxFrameLead` to `4` or `2` reduced some max spikes but raised averages (`17.68ms` and `17.87ms`). `skipmidcp+lead2` avoided transient mismatch and aborts in one long chaos run with max `69.8/92.7ms` and over33 `22/24`, but still averaged `17.871/17.874ms`. Disabling predicted-input lead throttle restored average near the target (`16.851/17.096ms`) but increased correction spikes (`136/101ms`, rollback spikes `47/7`). `limit1` smoothed the host but stalled the client around frame `2109`, so it is rejected.
