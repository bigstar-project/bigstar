...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は14,531行（リファクタ開始前21,253行から6,722行減）で、単一の共有 `State G`、136箇所の `MELONDS_NSML_*` 設定参照、ENet、入力同期、診断、テスト用runtime patchがまだ集中している。正確なtop-level境界で測ると、現在の`InitFromEnvironment`は958行、`BeforeRunFrame`は269行、`AfterRunFrame`は355行。旧803行の`ReadGameStateSample`は37行のオーケストレーションまで縮小した。file数を増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめる。
...
- Next action: 残る`InitFromEnvironment`の診断/runtime patch設定を責務がまとまる既存または中規模moduleへ移し、その後`BeforeRunFrame`/`AfterRunFrame`のframe orchestrationを整理する。並行してrepo内launcher/docs参照がない実験feature groupを静的参照・実ゲーム回帰で確認しながら削除する。数十行ごとの細分化は行わない。
- Verification status: Release `melonDS` と9 unit targetのbuild成功。CTest 9件は直近0.13秒でpass。旧binary baseline 2,300 framesはpass。固定seedのfast tierはpacket lossありでもhost/client semantic stateとapplied inputがbyte-for-byte golden一致した。rollback tierもcheckpoint保存・packet loss回復・golden一致を確認した。直近は`logs/refactor-before-frame-phases-fast`と`logs/refactor-before-frame-phases-rollback`で両方初回passした。変更したPoC本体のClang unused診断は0件で、全launcherのPowerShell parser errorも0件。
