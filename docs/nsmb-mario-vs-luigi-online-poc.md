...
- **Current blocker:** raw/logical frame-domain不一致と、片側emulation停止後にframe advantageが`P=7`へ恒久固定される問題はどちらも解消した。残る製品確認は、実Bigstar GUIでcontroller設定dialogを開閉した時の回復速度と見た目である。online設定dialogの協調pauseまたは禁止、既知のrollback音声取り消し不能、player spriteの訂正直後最大3-frame位相差、再戦checkpoint復元の約`0.15秒`停止、stage開始直後hitchは別件として残る。
- **Next action:** 最新Bigstarを独立2プロセスで操作し、controller設定dialog後にoffsetが収束し、訂正depthが7へ張り付かず、critical state一致と通常frame cadenceを維持することを確認する。自動gateではE2E専用pause knobを使う2秒停止の収束assertionを追加し、必要に応じて5～8秒停止へ広げる。7秒経過による切断は要件に含めない。
