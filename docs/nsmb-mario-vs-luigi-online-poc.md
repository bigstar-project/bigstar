...
- Current blocker: with `InputDelayFrames=2`, correctness-oriented rollback is possible through `tinycoreramdelta`, but the required full JIT block reset/recompile and same-frame resim still produce visible spikes/over33 frames under strong jitter. NSMB process-list snapshots are lighter but not correctness-safe across contact/chaos. Raising GUI rollback InputDelay above `2` is explicitly rejected for this goal and must not be used as the fix.
- Next actions: keep `tinycoreramdelta` as the exact-ish oracle/baseline, but move the primary search toward a new architecture that avoids blocking the display frame during exact correction, such as off-main-thread shadow resim with atomic publish, rollback-specific JIT state snapshots/reuse, or a pointer-safe MvL actor/global page-delta that excludes/remaps scheduler/link fields. Continue treating broad actor arena/tail copying and InputDelay increases as rejected.
