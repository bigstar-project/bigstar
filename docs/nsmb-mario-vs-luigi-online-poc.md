...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は9,244行（リファクタ開始前21,253行から12,009行減）で、単一の共有 `State G`、3箇所のprocess environment直接読取、game-state sample採取、テスト用runtime patchがまだ集中している。正確なtop-level境界で測ると、現在の`State`定義は161行、`InitFromEnvironment`は656行。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は28行のオーケストレーションまで縮小した。file数や短いfileを増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめ、死んだ実験コードが見つかれば総量も削る。
...
- Next action: 残る656行の`InitFromEnvironment`と161行の共有`State G`をfeature group単位で監査する。現存するplayer/star/powerup/script/JIT runtime patchはAI収集・smoke・goldenから現役利用されていることを確認済みなので維持する。次は残る成果物保存phaseとAI stream群に、まとまった所有権境界または不要なPoC試行コードがあるかを調べる。diagnostic ringはevent生成とgame RAM採取を合わせて動かせる境界が得られるまで細分化しない。抽出による読みやすさと未使用実験コードの純減は分けて計測する。
- Verification status: Release `melonDS` build、PoC/DiagnosticsのClang unused診断、CTest 13件（0.30秒）がpassした。Diagnostics contract testは20回連続passした。`logs/refactor-diagnostic-event-runtime`の実ROM・2-process・packet loss付きdiagnostics tierはhost/client別event JSONL、watchdog、phase、heartbeat、semantic goldenを検査し、両peer各12 eventを記録した。`logs/refactor-diagnostic-event-runtime-rollback`も1,250 frames、checkpoint coverage、golden一致でpassした。先行するhash移動検証では`logs/refactor-reader-hashes-baseline`と`logs/refactor-reader-hashes-after`の実ROM hash CSVがhost/clientともSHA-256完全一致し、extended mismatch/component全114行も一致している。state sync/apply専用`logs/refactor-state-sync-builders-apply`ではplayer/world/actor snapshot実適用とmoving-hazard GUID map更新を確認済み。送信delay/jitterの実queue経路は`logs/refactor-input-delivery-delay-jitter`、ImitationAI production pathは`logs/refactor-mvl-ai-imitation`、GUIが有効化する`coredelta`は`logs/refactor-remove-rejected-rollback-coredelta`が引き続きpassしている。
