...
- 2026-06-26 continuation summary:
  ...
  - Current interpretation of the new Plan-D line: host-authoritative freeze is the first measured moving-hazard fix that keeps chaos/dualstresslong near 60fps without the process-list relink failure mode. It is not goal-complete yet. The remaining risks are moving-hazard collision/damage semantics under skip-update, the unresolved player/star transient mismatch class, and validation overhead separation from real runtime FPS.
...
- Next actions: keep Plan-D/freeze as the timing baseline and `nsmbtinycore` MR2 as the foreground correctness/cost reference. Validate whether frozen host-authoritative moving hazards still collide/damage correctly under complex manual-like input, fix or replace the invalid `death` route assertion, and continue splitting player/star state into deterministic authoritative lanes instead of widening one-off actor ranges. Current shadow and foreground exact variants are measured boundaries, not promotion candidates.
