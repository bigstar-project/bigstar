...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は13,525行（リファクタ開始前21,253行から7,728行減）で、単一の共有 `State G`、27箇所の `MELONDS_NSML_*` 設定参照、ENet、入力同期、診断、テスト用runtime patchがまだ集中している。正確なtop-level境界で測ると、現在の`InitFromEnvironment`は772行、`BeforeRunFrame`は269行、`AfterRunFrame`は172行。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は48行のオーケストレーションまで縮小した。file数を増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめる。
...
- Next action: 残る772行の`InitFromEnvironment`と共有`State G`をfeature group単位で監査し、repo内launcher/test/docsから使われないPoC実験を削除する。次は約1,300行あるrollback memory backend（NSMB range構築、delta/preimage保存・復元）を、すでにある`NsmbRollbackStore`へ無理に詰め込まず、runtime cacheとNDS依存を一まとまりにできる中規模module境界として設計・抽出する。数十行ごとの細分化は行わない。
- Verification status: Release `melonDS` build、Clang unused診断、CTest 10件（直近0.64秒）がpass。nested rollback config変更後の固定seed・packet loss付き2-process実ゲーム回帰は`logs/refactor-nested-rollback-config-fast`と`logs/refactor-nested-rollback-config-rollback`で、host/client semantic state、applied input、rollback checkpoint coverage、golden一致を確認した。直前のHarnessConfig検証として、`logs/refactor-harness-config-state-save`で両peerがframe 1000の19,228,045-byte stateを保存し、`logs/refactor-harness-config-state-load`でframe 1に復元して1,250 framesを完走、`logs/refactor-harness-config-memory-patch`でも両peerの実RAM同値patchとgameplay syncを確認済み。
