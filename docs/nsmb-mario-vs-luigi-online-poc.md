...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は10,271行（リファクタ開始前21,253行から10,982行減）で、単一の共有 `State G`、6箇所のprocess environment直接読取、barrier orchestration、state syncの送信とremote sample取得、テスト用runtime patchがまだ集中している。正確なtop-level境界で測ると、現在の`State`定義は240行、`InitFromEnvironment`は526行、`BeforeRunFrame`は268行、`AfterRunFrame`は171行。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は48行のオーケストレーションまで縮小した。file数を増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめる。
...
- Next action: 残る526行の`InitFromEnvironment`と共有`State G`をfeature group単位で監査し、repo内launcher/test/docsから使われないPoC実験を削除する。remote applyはsample取得だけの薄いfacadeになったため、次は`SyncGame/Player/World/Effect/MovingHazard/ActorSnapshot`送信群をreader→wire→transportの一まとまりとして既存game-state moduleへ寄せるか、barrier orchestrationを選ぶ。数十行ごとの細分化は行わない。
- Verification status: Release `melonDS` build、PoC/reader/writerのClang unused診断、CTest 11件（0.35秒）がpass。固定seed・packet loss・4-frame input delay付き2-process実ゲーム回帰は`logs/refactor-remote-apply-facade-fast`と`logs/refactor-remote-apply-facade-rollback`で、savestate/tinycorepreimage双方の1,250 frames完走、host/client semantic state、applied input、checkpoint coverage、golden一致を確認した。専用`logs/refactor-remote-apply-facade-apply`は同じ高速条件でplayer globals、world/item、moving hazard、effect、actor snapshotのsync/applyを同時に有効化して1,250 frames完走し、player apply 20行、world apply 24行、actor snapshot apply 12行、moving-hazard GUID map更新1行を確認した。専用apply runへ通常のhost/client完全一致gateを加えると既知の実験機能差が出るため、apply coverageには完走・実apply確認を用い、既定経路の不変性は上記goldenで判定する。診断設定を実消費する`logs/refactor-diagnostics-config-ai-log/ai-play.jsonl`もRuleAI付き実ROM1,250 framesで15行生成され、全JSONL行のparseに成功した。送信delay/jitterの実queue経路は`logs/refactor-input-delivery-delay-jitter`、ImitationAI production pathは`logs/refactor-mvl-ai-imitation`がpassしている。GUIが有効化する`coredelta`の`logs/refactor-remove-rejected-rollback-coredelta`も引き続きpassしている。
