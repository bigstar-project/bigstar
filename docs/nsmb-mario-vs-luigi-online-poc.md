...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は20,079行で、単一の共有 `State G` に参照中のfieldが782個、381種類の `MELONDS_NSML_*` 設定、ENet、入力同期、rollback、NSMB RAM access、状態同期、診断、テスト用runtime patchがまだ集中している。`InitFromEnvironment` は1,312行、`BeforeRunFrame` は383行、`AfterRunFrame` は371行。
...
- Next action: game-state系受信handlerに「通常cleanup」と「既存の再戦cutoffによるcleanup skip」を表す結果型を導入し、ENet event loopをtransport orchestrationだけに縮める。その後、rollback storeを抽出する。各段階でfast/rollback実ゲームgoldenを維持する。
