...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は9,075行（リファクタ開始前21,253行から12,178行減）で、単一の共有 `State G`、game-state sample採取、テスト用runtime patchがまだ集中している。process environmentの直接読取は0箇所になった。brace境界で再計測した現在の`State`定義は111行、`InitFromEnvironment`は468行（旧記載の656行は現ソースと矛盾していたため修正済み）。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は28行のオーケストレーションまで縮小した。file数や短いfileを増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめ、死んだ実験コードが見つかれば総量も削る。
...
- Next action: 残る468行の`InitFromEnvironment`と111行の共有`State G`をfeature group単位で監査する。現存するplayer/star/powerup/script/JIT runtime patchはAI収集・smoke・goldenから現役利用されていることを確認済みなので維持する。次はMvL auto-restart checkpoint群とrollback計測群を、それぞれ既存のsave/restore処理・test境界と一緒に動かせるか確認する。自然な中規模module境界が得られない場合は無理にfileを増やさない。抽出による読みやすさと未使用実験コードの純減は分けて計測する。
- Verification status: Release `melonDS` build、PoCのClang unused診断、CTest 14件（0.43秒）がpassし、Diagnostics contract testはperformance契約を含め20回連続passした。最終コードの`logs/refactor-performance-runtime-final2`に対するpacket loss付きdiagnostics実ゲームtierは1,250 frames、host/client semantic golden、watchdog、phase/event JSONL、10-frame heartbeatをすべてpassし、host/clientともgameplay heartbeatをframe 900/1000/1100/1200の4回、active timingをstartFrame 960・290 samplesで生成した。先行するAI logのneutral policyと実linear-model imitation policyは移動前後のhost/client JSONLがbyte完全一致しており、rollback、hash、state sync/apply、送信delay/jitter、GUIの`coredelta`各production回帰も引き続きpassしている。
