...
- 完了: world、moving hazard、world actor snapshot、world effectの受信decode/state更新もclass別handlerへ分け、`PumpNetworkLocked`を319行から275行へ縮小した。4種すべてで再戦cutoff時のcleanup skip semanticsを`ReceiveDisposition`経由で保持し、fast/rollback実ゲームgolden一致を確認した。world actor snapshot handlerは後続の2026-07-15監査で旧実験ごと削除済みである。
...
- 完了: 上記writer境界を拡張し、sample取得後のworld actor GUID照合、player/star/moving-hazard cache、world item確認・spawn callback、player global/transition、effect、actor snapshotの適用本体を同じ`NsmbGameStateWriter`へ移した。send/apply双方が使うactor cacheは`NsmbGameStateReader`へ一本化し、重複実装は残していない。PoCの5本のremote apply facadeは既存順序のenable判定、network pump、fresh-sample待機、trace前処理とwriter呼出だけになり、この段階で本体をさらに529行削減した。writerはheader/source合計1,236行、readerは2,289行の責務単位で、小fileは追加していない。world itemとgeneric actor snapshotのWriter経路は後続監査で削除済みである。
- 完了: player、world/item、world effect、moving hazard、world actor snapshotのwire packet構築を、RAM読取とactor cacheを所有する既存`NsmbGameStateReader`へ集約した。PoC側の5本のsync関数は従来のenable/frame条件、trace順序、mutex、重複送信判定、transport送信だけを担当する薄いfacadeとなり、ローカルeffect判定2関数と不要になったreader aliasを削除した。PoC本体はさらに161行減り、readerはheader/source合計2,545行の責務単位で、新しいproduction fileは増やしていない。world itemとgeneric actor snapshotのpacket構築は後続監査で削除済みである。
...
- 完了: 旧Plan-Dのworld item生成実験をfeature単位で参照監査した。現行GUIは`STATE_SYNC`をhash/mismatch診断に使うため維持し、Big Starのworld snapshotと独立したmoving-hazard packetも維持した。一方、`Item`/`NeutralItem`/`DroppedStarItem`の3状態を送ってclientで`Actor::spawnActor`を直呼びする経路は、GUI・現行回帰scenarioでは使われず、correctness routeではない旧manual Plan-Dだけが有効化していた。そのため専用env/config、520-byte `WireWorldState`内のitem 3 slotと重複moving-hazard slot、Reader scan、Writerの4-frame grace/apply/callback、instance別12 runtime配列、ARM trampolineと命令emitter、runner引数・専用gateをまとめて削除した。`WireWorldState`はBig Starだけの120 byteとなり、PoC本体は134行減って7,176行になった。AI観測が使うworld item分類定数は現役なので維持し、新しいfileは増やしていない。
...
- Next action: 182行の`InitFromEnvironment`と43行の共有`State G`は既にオーケストレーションとして無理のない大きさなので、行数だけを目的に分解しない。旧world item spawnとgeneric actor snapshotの監査は完了した。次は残るPlan-Dの`PlayerState`、world effect/moving-hazard/Big Star applyをfeature単位で監査する。一般`STATE_APPLY`はstaged retryが現役利用し、GUIの`STATE_SYNC`はhash/mismatch診断に必要なので維持する。観測用lifecycle traceを補正packetと混同せず、参照のない旧補正実験だけを削除する。既存Runtime/Reader/Writerへ責務単位でまとまる場合だけ移し、新しい小fileは増やさない。
- Verification status: Release `melonDS`と関連test target build、PoCのClang strict unused診断、CTest 16件（1.14秒）、Diagnostics contract 20回連続がpassした。今回GUI/Rust差分はない。`logs/refactor-remove-world-actor-snapshot-fast`のproduction寄りpacket loss付き2-process 1,250-frame fast tierはsemantic goldenをpassした。残したBig Star/moving-hazard経路は`logs/refactor-remove-world-actor-snapshot-world-star`の実ROM・2-process 1,100 framesを完走し、各7回の照合でX/Y drift 0、全route gateをpassした。PowerShell runner 3本のparse、削除識別子0件、`git diff --check`もpassしている。linear/compact実model、diagnostic snapshot/JSON、MvL marker、AI log生成のproduction回帰も引き続き保存している。
