...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は13,594行（リファクタ開始前21,253行から7,659行減）で、単一の共有 `State G`、27箇所の `MELONDS_NSML_*` 設定参照、ENet、入力同期、診断、テスト用runtime patchがまだ集中している。正確なtop-level境界で測ると、現在の`InitFromEnvironment`は807行、`BeforeRunFrame`は269行、`AfterRunFrame`は172行。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は48行のオーケストレーションまで縮小した。file数を増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめる。
...
- Next action: 残る807行の`InitFromEnvironment`と共有`State G`をfeature group単位で監査し、repo内launcher/test/docsから使われないPoC実験を削除する。次はrollback runtime、transport、state syncのうち、runtime stateとの境界が明確で既存の中規模moduleへまとめられる部分から物理抽出する。数十行ごとの細分化は行わない。
- Verification status: Release `melonDS` build、Clang unused診断、CTest 10件（0.17秒）、config test単体（0.01秒）がpass。固定seed・packet loss付き2-process実ゲーム回帰は`logs/refactor-harness-config-fast`と`logs/refactor-harness-config-rollback`で、host/client semantic state、applied input、rollback checkpoint coverage、golden一致を確認した。さらに`logs/refactor-harness-config-state-save`で両peerがframe 1000の19,228,045-byte stateを保存し、`logs/refactor-harness-config-state-load`で両peerがframe 1に同stateを復元して1,250 framesを完走した。`logs/refactor-harness-config-memory-patch`では直前の実RAM dumpをsourceに、両peerがframe 1000でoffset 0の1 byteを同値patchし、production hookログとhost/client gameplay syncを確認した。
