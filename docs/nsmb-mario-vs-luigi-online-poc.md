...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は9,795行（リファクタ開始前21,253行から11,458行減）で、単一の共有 `State G`、3箇所のprocess environment直接読取、barrier orchestration、game-state hash送信、テスト用runtime patchがまだ集中している。正確なtop-level境界で測ると、現在の`State`定義は187行、`InitFromEnvironment`は526行、`BeforeRunFrame`は268行、`AfterRunFrame`は171行。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は48行のオーケストレーションまで縮小した。file数を増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめる。
...
- Next action: 残る526行の`InitFromEnvironment`と187行の共有`State G`をfeature group単位で監査する。現存するplayer/star/powerup/script/JIT runtime patchはAI収集・smoke・goldenから現役利用されていることを確認済みなので維持し、次はframe/start/serial/state-load barrier orchestrationか`SyncGameState`のhash比較/送信境界を責務単位で整理する。数十行ごとの細分化は行わない。
- Verification status: Release `melonDS` build、PoC/diagnostics/reader/writerのClang unused診断、CTest 11件（0.25秒）がpass。固定seed・packet loss・4-frame input delay付き2-process実ゲーム回帰は`logs/refactor-hang-diagnostics-fast`と`logs/refactor-hang-diagnostics-rollback`で、savestate/tinycorepreimage双方の1,250 frames完走、host/client semantic state、applied input、checkpoint coverage、golden一致を確認した。新しいdiagnostics tierの`logs/refactor-hang-diagnostics-tier`も同条件でgolden一致し、host/client別のwatchdog JSONL 100行前後とphase JSONL 28,000行以上、先頭/末尾schemaとroleを検査した。さらに`logs/refactor-hang-diagnostics-enabled`のwatchdog 197行・phase 57,150行は全行JSON parse済みで、ARM abort、fatal error、log open失敗はない。state sync/apply専用`logs/refactor-state-sync-builders-apply`ではplayer/world/actor snapshot実適用とmoving-hazard GUID map更新を確認済み。送信delay/jitterの実queue経路は`logs/refactor-input-delivery-delay-jitter`、ImitationAI production pathは`logs/refactor-mvl-ai-imitation`、GUIが有効化する`coredelta`は`logs/refactor-remove-rejected-rollback-coredelta`が引き続きpassしている。
