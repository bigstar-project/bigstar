...
- Verification status: Release `melonDS` build、CTest 16件（0.49秒）、`ARM.cpp`自身のClang unused診断0件がpassした（既存header `DSi_AES.h`の未使用inline警告は差分外）。`logs/refactor-remove-lower-net-experiments-fast`のpacket loss付き同一実ROM・2-process 1,250-frame fast tierは16.6秒でsemantic goldenをpassした。`logs/refactor-remove-lower-net-experiments-standard-{host,client}`では両peerが1,700 framesを完走し、frame 1,500以降の`PacketBridge.Only`現役lower APIを200 frames直接検査した（133.60秒、12.72 FPS）。日常反復はfast tier、約2分強のstandard splitはCPUコア内のPacketBridge変更時など意味のある節目だけに使う。runner parse、削除対象のrepo全体参照0件、`git diff --check`もpassしている。今回GUI／Rust差分はなく、新規fileもない。
