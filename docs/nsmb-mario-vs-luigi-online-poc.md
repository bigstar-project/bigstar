...
- **Current blocker:** network loss・重複・再順序化・短時間断、再戦、主要event、地形表示は自動gateを通った。現時点の最重要blockerは、Main RAMだけを戻すROM-loopでは、訂正前にARM7/SPUへ反映済みの誤予測音を後着commandだけで取り消せない点である。正確な音声には最大予測幅の出力遅延とARM7 sound driver/SPUの整合したrollbackが必要だが、安全な部分状態境界と性能は未証明である。player spriteは恒久差ではないが訂正直後の完全一致でもなく、最大3 frameの位相差を残す。常時8-9 frame人工遅延、再戦checkpoint復元の約`0.15秒`停止、stage開始直後hitchも快適性上の別制約として残る。
- **Next action:** Bigstarは変更しない。Slippi型ROM-loopを主方針にするなら、音声artifactを既知制約として知覚的に緩和する案と、7 frame分のaudio遅延＋ARM7/SPU rollbackを性能・同期gateからやり直す高リスク案を分離する。後者は小規模ledgerの延長ではなくfull-machine rollbackへ近づくため、実装前に保存対象と1 frame当たりコストを測る。音声の余分・欠落0とPCM gateを満たすまでは音声込み完成とは扱わない。
