...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は8,994行（リファクタ開始前21,253行から12,259行減）で、単一の共有 `State G`、game-state sample採取、テスト用runtime patchがまだ集中している。process environmentの直接読取は0箇所になった。brace境界で再計測した現在の`State`定義は91行、`InitFromEnvironment`は468行（旧記載の656行は現ソースと矛盾していたため修正済み）。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は28行のオーケストレーションまで縮小した。file数や短いfileを増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめ、死んだ実験コードが見つかれば総量も削る。
...
- Next action: 残る468行の`InitFromEnvironment`と91行の共有`State G`をfeature group単位で監査する。現存するplayer/star/powerup/script/JIT runtime patchはAI収集・smoke・goldenから現役利用されていることを確認済みなので維持する。次はNDS savestateとstage遷移に結合したMvL auto-restart checkpoint群を、既存のsave/restore処理と同じ中規模境界へまとめられるか確認する。自然な境界が得られない場合は無理にfileを増やさない。抽出による読みやすさと未使用実験コードの純減は分けて計測する。
- Verification status: Release `melonDS` build、PoCのClang strict unused診断、CTest 14件（0.34秒）がpassし、RollbackStore contract testは統計契約を含め20回連続passした。最終コードの`logs/refactor-rollback-statistics-final`に対するpacket loss付きrollback実ゲームtierは2-process・1,250 frames・tinycorepreimage checkpoint coverage・host/client semantic/input goldenをすべてpassした。frame 960/1080/1200のproduction統計でcheckpointSaves=120/240/360と保存量・時間を記録し、integrity errorはない。直前のdiagnostics tier、AI logのneutral/linear-model byte一致、hash、state sync/apply、送信delay/jitter、GUIの`coredelta`各production回帰も引き続きpassしている。
