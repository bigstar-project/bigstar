...
- Current blocker: with `InputDelayFrames=2`, exact rollback still spikes too much, while the lightweight predict/repair path is fast but still timing-sensitive. Actor lifecycle repair is promising but not complete; `chaos` can still expose stale moving-hazard samples, persistent `playerGlobal` windows, or a single large active-frame spike.
- Next actions: keep the delay-2 path and improve root behavior rather than raising InputDelay. The next concrete work is to make moving-hazard/world samples available before the first active gameplay comparison without long waits, trace the remaining `playerGlobal` windows around `chaos` frames `1320-1410`, and investigate the `chaos` spike around frames `1800-1830` with phase tracing. Broader alternatives remain off-main-thread shadow resim or a more explicit ROM/actor-level authority model if this repair path plateaus.
