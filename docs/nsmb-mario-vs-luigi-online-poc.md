...
- Current blocker: with `InputDelayFrames=2`, exact-ish rollback can preserve state but misses playable smoothness because same-frame correction still creates `90ms+` spikes. Plan-D-style selected repair is fast enough but still drifts in active object lifecycle and player/global transitions under complex inputs.
- Next actions: do not increase InputDelay above `2`. Keep full-reset/skip-JIT `tinycoreramdelta` as correctness oracles, but stop treating smaller checkpoints alone as the main solution. The next practical direction should either build a real generic active-object lifecycle snapshot/apply model validated against the exact oracle, or prototype a new rollback architecture that avoids doing all correction resimulation inside the visible frame.
