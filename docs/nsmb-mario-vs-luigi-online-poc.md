...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は14,516行（リファクタ開始前21,253行から6,737行減）で、単一の共有 `State G`、136箇所の `MELONDS_NSML_*` 設定参照、ENet、入力同期、診断、テスト用runtime patchがまだ集中している。現在の`InitFromEnvironment`は1,105行、`BeforeRunFrame`は549行、`AfterRunFrame`は355行。旧803行の`ReadGameStateSample`は37行のオーケストレーションまで縮小した。file数を増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめる。
...
- Verification status: Release `melonDS` と9 unit targetのbuild成功。CTest 9件は直近0.15秒でpass。旧binary baseline 2,300 framesはpass。固定seedのfast tierはpacket lossありでもhost/client semantic stateとapplied inputがbyte-for-byte golden一致した。rollback tierもcheckpoint保存・packet loss回復・golden一致を確認した。直近は`logs/refactor-diagnostics-config-fast`と`logs/refactor-diagnostics-config-rollback`で両方初回passした。変更したPoC本体のClang unused診断は0件で、全launcherのPowerShell parser errorも0件。
