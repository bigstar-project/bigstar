...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は14,113行（リファクタ開始前21,253行から7,140行減）で、単一の共有 `State G`、119箇所の `MELONDS_NSML_*` 設定参照、ENet、入力同期、診断、テスト用runtime patchがまだ集中している。正確なtop-level境界で測ると、現在の`InitFromEnvironment`は939行、`BeforeRunFrame`は269行、`AfterRunFrame`は172行。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は48行のオーケストレーションまで縮小した。file数を増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめる。
...
- Next action: 残る939行の`InitFromEnvironment`と共有`State G`をfeature group単位で監査し、repo内launcher/test/docsから使われないPoC実験を削除する。そのうえでrollback runtime、transport、state syncなど依存がまとまる境界を選び、中規模moduleとして物理抽出する。数十行ごとの細分化は行わない。
- Verification status: Release `melonDS` と9 unit targetのbuild成功。CTest 9件は直近0.41秒でpass。旧binary baseline 2,300 framesはpass。固定seedのfast tierはpacket lossありでもhost/client semantic stateとapplied inputがbyte-for-byte golden一致した。rollback tierもcheckpoint保存・packet loss回復・golden一致を確認した。直近は`logs/refactor-runtime-diagnostics-config-fast`と`logs/refactor-runtime-diagnostics-config-rollback`で両方初回passした。変更したPoC本体のClang unused診断は0件で、全launcherのPowerShell parser errorも0件。
