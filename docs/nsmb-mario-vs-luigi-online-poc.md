...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は9,590行（リファクタ開始前21,253行から11,663行減）で、単一の共有 `State G`、3箇所のprocess environment直接読取、game-state hash送信、テスト用runtime patchがまだ集中している。正確なtop-level境界で測ると、現在の`State`定義は178行、`InitFromEnvironment`は674行。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は48行のオーケストレーションまで縮小した。file数や短いfileを増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめ、死んだ実験コードが見つかれば総量も削る。
...
- Next action: 残る674行の`InitFromEnvironment`と178行の共有`State G`をfeature group単位で監査する。現存するplayer/star/powerup/script/JIT runtime patchはAI収集・smoke・goldenから現役利用されていることを確認済みなので維持し、次はgame-state hash送信境界か成果物/trace runtimeを責務単位で整理する。数十行ごとの細分化は行わず、抽出による読みやすさと未使用実験コードの純減を分けて計測する。
- Verification status: Release `melonDS` build、PoC/coordinator/diagnostics/reader/writerのClang unused診断、CTest 12件（0.25秒）がpass。Coordinator contract testは20回連続passし、同一process実ROMでは`logs/refactor-coordinator-barrier`と`logs/refactor-coordinator-serial`が各300 frames完走、`logs/refactor-coordinator-state-save`/`state-load`でNDS 2台と共有LocalMPのsave/loadを確認した。固定seed・packet loss・4-frame input delay付き2-process実ゲーム回帰は`logs/refactor-coordinator-fast`と`logs/refactor-coordinator-rollback`で各1,250 frames完走し、host/client semantic state、applied input、checkpoint coverage、golden一致を確認した。diagnostics tierの`logs/refactor-hang-diagnostics-tier`も同条件でgolden一致し、host/client別のwatchdog/phase JSONL schemaとroleを検査済み。state sync/apply専用`logs/refactor-state-sync-builders-apply`ではplayer/world/actor snapshot実適用とmoving-hazard GUID map更新を確認済み。送信delay/jitterの実queue経路は`logs/refactor-input-delivery-delay-jitter`、ImitationAI production pathは`logs/refactor-mvl-ai-imitation`、GUIが有効化する`coredelta`は`logs/refactor-remove-rejected-rollback-coredelta`が引き続きpassしている。
