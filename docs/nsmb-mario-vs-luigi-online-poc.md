...
- Current rollback work is using `tinycoreramdelta` as the exact-ish correctness baseline: tiny core state plus Main RAM page delta. The older `tinycorepreimage`/repair-only paths remain diagnostic references, but they are no longer the primary correctness candidate for the active delay2 goal.
