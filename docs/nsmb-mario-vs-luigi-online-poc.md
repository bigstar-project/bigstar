...
- Next action: 182行の`InitFromEnvironment`と43行の共有`State G`は既にオーケストレーションとして無理のない大きさなので、行数だけを目的に分解しない。旧world item spawnの監査は完了した。次は残るPlan-Dの`STATE_APPLY`、`PlayerState`、world effect/moving-hazard/generic actor snapshotを、GUI・回帰scenario・過去の設計記録まで含めてfeature単位で監査する。GUIが使う`STATE_SYNC`のhash/mismatch診断と実際に回帰で使う観測経路を混同せず、参照のない旧補正実験だけを削除する。既存Runtime/Reader/Writerへ責務単位でまとまる場合だけ移し、新しい小fileは増やさない。input recorder/game-state traceのwriter ownershipと、packet handler callbackを必要とするnetwork pump threadは現状の境界を維持する。
- Verification status: Release config/MvL runtime/diagnostics test targetsと`melonDS` build、PoCのClang strict unused診断、CTest 16件（1.57秒）、Diagnostics contract 20回連続がpassした。GUIは直前の共通baselineで`corepack pnpm run ci`、Vitest 89件、Playwright 2件、Tauriの`cargo fmt`、`cargo clippy-all`、Rust unit test 49件をpass済みで、今回GUI/Rust差分はない。production寄りfast tierは`logs/refactor-remove-world-item-spawn-fast`でpacket loss付き2-process 1,250 framesとsemantic goldenをpassした。削除対象に隣接するBig Star world snapshotは`logs/refactor-remove-world-item-spawn-world-star-final`で実ROM・2-process 1,250 framesを完走し、12回のstar照合と12回のmoving-hazard照合でX/Y drift 0、全route gateをpassした。linear/compact実model、startup/model report、diagnostic snapshot/JSON、MvL marker、AI log生成のproduction回帰も引き続き保存している。重い第2試合gateは上記のfixture非決定性によりpass証拠としては採用しない。
