...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は20,070行で、単一の共有 `State G` に参照中のfieldが782個、381種類の `MELONDS_NSML_*` 設定、ENet、入力同期、rollback、NSMB RAM access、状態同期、診断、テスト用runtime patchがまだ集中している。`InitFromEnvironment` は1,312行、`BeforeRunFrame` は383行、`AfterRunFrame` は371行。
...
- Next action: match seed/start-readyのwire codecをpure moduleへ抽出してmalformed packetをunit test化し、次にresend/accept transitionをENet非依存のsession policyへ分離する。抽出の各段階でfast/rollback実ゲームgoldenを維持する。
- Verification status: Release `melonDS` と4 unit targetのbuild成功。CTest 4件は合計約0.05-0.22秒でpass。旧binary baseline 2,300 framesはpass。固定seedのfast tierはpacket lossありでもhost/client semantic stateとapplied inputがbyte-for-byte golden一致した。rollback tierも約14-15秒でcheckpoint保存・packet loss回復・golden一致を確認した。typed bootstrap/connection/input/rollback config、unused/旧runtime patch削除、input timeline/protocol/delivery抽出後もpassし、Clang unused診断は0件。
