...
- 2026-06-23 latest selected-snapshot continuation:
  ...
  - Log storage fix: `run-nsmb-mvl-lan-route-smoke.ps1` now stores per-run host/client ROMs as hardlinks by default, matching `codex/rule-based-mvl-ai`. Use `-CopyRomToLog` only when a run really needs standalone ROM copies in the log directory.
