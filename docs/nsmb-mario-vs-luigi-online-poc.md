...
- Next action: 182行の`InitFromEnvironment`と43行の共有`State G`は既にオーケストレーションとして無理のない大きさなので、行数だけを目的に分解しない。約250行のstate save/load harnessも現役のclean-state adapter smokeが利用しているため維持する。次は残る6,408行のinput／PacketBridge／rollback／MvL／diagnostics orchestrationを責務・参照元・実行scenario別に再監査し、複数の現役consumerを持つまとまりは中規模moduleのまま保ちつつ、PoC試行時だけの未使用feature groupを優先して縦に削除する。一般`STATE_APPLY`、GUIの`STATE_SYNC`、観測trace、自然同期World gateは維持する。
- Verification status: Release `melonDS` build、CTest 16件（0.49秒）、`ARM.cpp`自身のClang unused診断0件がpassした（既存header `DSi_AES.h`の未使用inline警告は差分外）。`logs/refactor-remove-stage-start-experiments-fast`のpacket loss付き同一実ROM・2-process 1,250-frame fast tierは15.7秒でsemantic goldenをpassした。`logs/refactor-remove-stage-start-experiments-standard-{host,client}-retry`では両peerが1,800 framesを完走し、frame 1,500以降の`PacketBridge.Only`実経路を直接検査した。描画・音声・screenshotを止めるstandard-split高速化案も測定したが11.73 FPSで、従来条件の12.60 FPSを改善しなかったためcodeには残していない。日常反復はfast tier、約2.5分のstandard splitは意味のある節目だけに使う。runner parse、削除対象のrepo全体参照0件、`git diff --check`もpassしている。今回GUI／Rust差分はなく、新規fileもない。
