...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は8,612行（リファクタ開始前21,253行から12,641行減）で、単一の共有 `State G`、game-state sample採取、テスト用runtime patchがまだ集中している。process environmentの直接読取は0箇所になった。brace境界で再計測した現在の`State`定義は57行、`InitFromEnvironment`は406行（旧記載の656行は現ソースと矛盾していたため修正済み）。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は28行のオーケストレーションまで縮小した。file数や短いfileを増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめ、死んだ実験コードが見つかれば総量も削る。
...
- Next action: 残る406行の`InitFromEnvironment`と57行の共有`State G`をfeature group単位で監査する。input recorderとgame-state traceは既に各専門moduleのwriterがfile lifecycleを所有しており、diagnosticsへ再統合すると責務が逆戻りするため維持する。次はPoCに残るruntime patchの一回性marker群とnetwork pump thread lifecycleを、既存の中規模moduleへ自然に寄せられるか利用実績とreset境界から判断する。新しい小fileは増やさず、まとまった既存moduleへの集約、オーケストレーションの可読化、未使用実験コードの純減を分けて計測する。
- Verification status: Release `melonDS` build、PoCのClang strict unused診断、CTest 16件（0.37秒）がpassし、Diagnostics runtime testは初回に加えて20回連続passした。`logs/refactor-player-life-runtime`のpacket loss付き2-process diagnostics tierは1,250 frames、semantic golden、watchdog/heartbeat/event schemaをpassし、両peerで`diagnostic_started` 1件、`player_life_change` 3件、`start_ready` 3件、`player_position_anomaly` 5件を生成した。直前の`logs/refactor-ai-config-open-runtime`では両peerのAI log生成とneutral policy評価、`logs/refactor-packet-bridge-runtime-fast`ではpacket loss付きfast tierとgolden一致もpassしている。JIT helper初回適用は複数の上位runで両peer同一frame（870または900）を確認した。重い第2試合gateは上記のfixture非決定性により今回pass証拠としては採用しない。さらに変更前の`logs/refactor-mvl-runtime-secondgame` 10,000-frame第2試合、AI logのneutral/linear-model byte一致、hash、state sync/apply、送信delay/jitter、GUIの`coredelta`各production回帰も保存している。
