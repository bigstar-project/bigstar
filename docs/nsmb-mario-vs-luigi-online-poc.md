...
- **Current conclusion:** 中間catch-up tickでもNSMBのgame render callbackを実行し、そのtickがGPU3Dへ生成したsceneだけをFIFO順序付きbegin/end markerで破棄する境界を実装した。行列、geometry test、FIFO処理、render callbackのMain RAM副作用は通常どおり通し、最終tickだけが頂点・polygonを表示sceneへ残す。自動再生ではLuigi側の土管X重複を消し、state収束と60fps相当を維持した。長時間手動runの約1.2秒停止はruntime traceの非同期化で解消し、初戦同期は手動gateを通過した。再戦では外側frameと復元後のgeneration-local logical frameを混用してdepth 12117とする原因を修正し、2回の再戦を含む自動gateと、その後のユーザー手動2再戦gateで継続ずれなしを確認した。再戦ロード時の約0.15秒停止とstage開始直後hitchは残る。
...
- **Bigstar delay classification:** rollback toggleが設定する`InputDelayFrames=2`は`MELONDS_NSML_DELAY=2`へ渡り、local inputを `logicalFrame + 2` 番で保存・送信し、現在frameには2 frame前の履歴を適用する。開始2 frameはneutralでprimeされ、表示だけを遅らせるbufferはないため、意味はTangoよりSlippiに近い。ただしBigstarの現行rollback起動設定はまだ`coredelta` backendであり、この分類だけでGUI経路全体がROM-loop候補だとは扱わない。
- **Practical control policy:** 最初の製品候補は`input delay D=2 / prediction horizon P=5 / ROM history H=12`とする。直近の成功した手動runはD=0・人工片道2-3 frame遅延で、host/clientとも最大log depth 4、最大5 replay tickだったため、現時点で支持できる連続投機量は5 tickまでである。Slippi相当のP=7は目標だが、P=6、次に7の性能・描画・音声gateを通った時だけ昇格する。現行logのdepthは通常`P-1`になるため、用語を混同しない。
- **Horizon implementation audit:** 現行`InputMaxFrameLead`はfuture send frameと最大受信frameを比較するため、途中のpacket穴を安全に検出できない。さらにpredictionのlast-confirmed値はpacket到着順で上書きされるので、逆順到着した古い入力を新しい予測へ使い得る。generation別のhighest-contiguous-confirmed frontier、対象frameより前の最新確定入力、`P+1`個目を実行する前のhard waitへ置き換える。depth 11超を最近のframeへ事後clampする経路は削除し、gateを越えた不一致はfatal invariantとして扱う。
...
- **Current blocker:** 初戦同期、stdout起因の秒単位停止、再戦logical timelineは手動gateまで通過した。再戦checkpoint復元の約`0.15秒`停止とstage開始直後の`40-53ms`級hitchは残り、対戦中の持続的60fps低下とは別だが製品上の無停止要件は未達である。現行手動gateの通信条件はローカルENetへ片道2-3 frameの送信待ちを加えたもので、実WANの輻輳、再順序化、帯域制約、切断は再現していない。土管Xぶれの消失そのもの、未通過event（死亡/復帰、土管遷移、item）、2D OAMちらつき、音声波形も未検証である。安全面では最大受信frame基準のlead throttle、arrival-order依存prediction、depth 11超の事後clampがreal-WAN契約を満たさない。
- **Next action:** generation別contiguous remote frontierとframe順predictionを実装し、`D=2/P=5/H=12`で`P+1`個目の実行前に停止、未ACK入力を再送、7秒で理由付き切断する。ROM不要の穴あき・逆順・loss・timeout testを先に通し、その後software rendererの実ROMでD=0/1/2、P=5/6/7を段階測定する。P=5のcorrectness・60fps・描画・音声gateが通るまではBigstarをROM-loopへ切り替えない。再戦checkpoint復元とstage warm-upのhitchは別のperformance課題として計測・削減する。
