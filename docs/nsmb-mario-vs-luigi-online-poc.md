...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は8,284行（リファクタ開始前21,253行から12,969行減）で、単一の共有 `State G`、NDS依存のdiagnostic snapshot/event採取、テスト用runtime patchがまだ集中している。process environmentの直接読取は0箇所になった。brace境界で再計測した現在の`State`定義は43行、`InitFromEnvironment`は405行（旧記載の656行は現ソースと矛盾していたため修正済み）。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は28行のオーケストレーションまで縮小した。file数や短いfileを増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめ、死んだ実験コードが見つかれば総量も削る。
...
- Next action: 残る405行の`InitFromEnvironment`と43行の共有`State G`をfeature group単位で監査する。input recorderとgame-state traceは既に各専門moduleのwriterがfile lifecycleを所有しており、diagnosticsへ再統合すると責務が逆戻りするため維持する。network pump threadは手動・split実行とrollback設計で現役であり、現状のTransportへ移すにはPoCのpacket処理を呼び戻すcallbackが必要になるため維持する。`StateSyncRuntime`は既にgame-state moduleが所有しているためfield移動は不要だった。次は`InitFromEnvironment`のstartup report/model/transport副作用phaseと、NDSに依存してPoCに残したdiagnostic snapshot採取・event生成の境界を監査する。新しい小fileは増やさず、まとまった既存moduleへの集約、オーケストレーションの可読化、未使用実験コードの純減を分けて計測する。
- Verification status: Release `melonDS` build、PoCとDiagnosticsのClang strict unused診断、CTest 16件（0.55秒）がpassし、Diagnostics contract testは初回に加えて20回連続passした。`logs/refactor-diagnostic-json-formatting`のpacket loss付き2-process diagnostics tierは1,250 frames、semantic golden、watchdog/heartbeat/event schemaをpassし、両peerで`player_life_change` 3件、`player_position_anomaly` 5件を生成した。移動前後のringなし`player_life_change`はhost/client各2件がbyte完全一致した。直前の`logs/refactor-mem-patch-config-fast`はfast golden、`logs/refactor-mem-patch-config-targeted`は両peer各1回のmemory patch、`logs/refactor-runtime-patch-log-extended`はplayer powerup `1/2`と各peer一度だけのreport、`logs/refactor-mvl-marker-runtime`はnet-random適用、`logs/refactor-ai-config-open-runtime`はAI log生成とneutral policy評価をpassしている。JIT helper初回適用は複数の上位runで両peer同一frame（870または900）を確認した。重い第2試合gateは上記のfixture非決定性により今回pass証拠としては採用しない。さらに変更前の`logs/refactor-mvl-runtime-secondgame` 10,000-frame第2試合、AI logのneutral/linear-model byte一致、hash、state sync/apply、送信delay/jitter、GUIの`coredelta`各production回帰も保存している。
