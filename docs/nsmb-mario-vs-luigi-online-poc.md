...
- Next action: 182行の`InitFromEnvironment`と43行の共有`State G`は既にオーケストレーションとして無理のない大きさなので、行数だけを目的に分解しない。旧world item spawn、generic actor snapshot、effect snapshot、`PlayerState`、Big Star／moving-hazard補正の削除は完了した。次は残る6,648行を責務・参照元・実行scenario別に再計測し、複数の現役consumerを持つまとまりは中規模moduleのまま維持しつつ、PoC試行時だけの未使用feature groupを優先して縦に削除する。一般`STATE_APPLY`、GUIの`STATE_SYNC`、観測trace、自然同期World gateは維持する。
