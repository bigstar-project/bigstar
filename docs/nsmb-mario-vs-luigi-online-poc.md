...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は14,129行（リファクタ開始前21,253行から7,124行減）で、単一の共有 `State G`、136箇所の `MELONDS_NSML_*` 設定参照、ENet、入力同期、診断、テスト用runtime patchがまだ集中している。正確なtop-level境界で測ると、現在の`InitFromEnvironment`は955行、`BeforeRunFrame`は269行、`AfterRunFrame`は172行。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は48行のオーケストレーションまで縮小した。file数を増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめる。
...
- 完了: hash/screenshot/RAM dump、game-state trace、diagnostics/event JSONL、AI play/observation logのpath・interval・frame範囲・filterをtyped `DiagnosticsConfig`へ抽出し、既存の`NsmbNetplayConfig.{h,cpp}`へ集約した。event pathだけでも有効、明示disableが最優先、diagnostics pathから既定event filenameを派生する優先順位と、ring容量・interval・stage filter・object数のclampをROM不要characterization testで固定した。既存event fileを消す副作用とRAM dump range構文解析はPoC側に残し、新しいfileは増やしていない。config module全体はheader+sourceで1,028行の中規模まとまりになった。
...
- Next action: 残る955行の`InitFromEnvironment`と共有`State G`をfeature group単位で監査し、repo内launcher/test/docsから使われないPoC実験を削除する。そのうえでrollback runtime、transport、state syncなど依存がまとまる境界を選び、中規模moduleとして物理抽出する。数十行ごとの細分化は行わない。
- Verification status: Release `melonDS` と9 unit targetのbuild成功。CTest 9件は直近0.36秒でpass。旧binary baseline 2,300 framesはpass。固定seedのfast tierはpacket lossありでもhost/client semantic stateとapplied inputがbyte-for-byte golden一致した。rollback tierもcheckpoint保存・packet loss回復・golden一致を確認した。直近は`logs/refactor-game-state-trace-final-fast`と`logs/refactor-game-state-trace-final-rollback`で両方初回passした。変更したPoC本体のClang unused診断は0件で、全launcherのPowerShell parser errorも0件。
