...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は20,119行で、単一の共有 `State G` に参照中のfieldが782個、381種類の `MELONDS_NSML_*` 設定、ENet、入力同期、rollback、NSMB RAM access、状態同期、診断、テスト用runtime patchがまだ集中している。`InitFromEnvironment` は1,312行、`BeforeRunFrame` は383行、`AfterRunFrame` は371行。
...
- Next action: world/hazard/actor/effect/game-state受信をhandlerへ分け、導入済みの`ReceiveDisposition`で再戦cutoff semanticsを保持しながらENet event loopをtransport orchestrationだけに縮める。その後、rollback storeを抽出する。各段階でfast/rollback実ゲームgoldenを維持する。
