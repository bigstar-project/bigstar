...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は7,754行（リファクタ開始前21,253行から13,499行減）で、NDS依存のdiagnostic snapshot RAM採取は既存Readerへ、diagnostic eventの純粋JSON整形は既存Diagnosticsへ移った。PoCには共有`State G`、診断eventの発火・snapshot選択、テスト用runtime patch、各subsystemを呼ぶframe orchestrationが残る。process environmentの直接読取は0箇所になった。brace境界で再計測した現在の`State`定義は43行、`InitFromEnvironment`は182行。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は28行のオーケストレーションまで縮小した。file数や短いfileを増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめ、死んだ実験コードが見つかれば総量も削る。
...
- Next action: 182行の`InitFromEnvironment`と43行の共有`State G`は既にオーケストレーションとして無理のない大きさなので、行数だけを目的に分解しない。診断eventでは`player_life_change`だけがNDS依存のnearby-hazard採取とJSON整形をPoCに残すため、hazard採取を既存Reader、純粋整形を既存Diagnosticsへ一まとまりで移せるか監査する。その後はruntime patchをfeature group単位で参照監査し、現役機能は維持、参照のない旧実験だけを削除する。input recorder/game-state traceのwriter ownershipと、packet handler callbackを必要とするnetwork pump threadは現状の境界を維持する。新しい小fileは増やさず、既存の中規模moduleへの集約、frame orchestrationの可読化、未使用実験コードの純減を別々に評価する。
- Verification status: Release `melonDS` build、PoC/DiagnosticsのClang strict unused診断、CTest 16件（0.56秒）がpassし、Diagnostics contract testは20回連続passした。`logs/refactor-diagnostic-event-formatters`のpacket loss付き2-process diagnostics tierは1,250 frames、semantic golden、watchdog/heartbeat/event schemaをpassした。変更前`logs/refactor-diagnostic-snapshot-reader`とのevent比較では12 eventの種別・frame列が両peerで一致し、保存directoryとnetwork thread schedulingで変わる最終送受信frame・queue件数だけを正規化するとhost/client各約3.50 MBのJSONLがbyte完全一致した。linear/compact実model、startup/model report、diagnostic snapshot/JSON、memory patch、runtime patch、MvL marker、AI log生成のproduction回帰も引き続き保存している。重い第2試合gateは上記のfixture非決定性によりpass証拠としては採用しない。
