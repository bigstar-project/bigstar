...
## NsmbNetplayPoC リファクタ調査 - 2026-07-15

- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は8,159行（リファクタ開始前21,253行から13,094行減）で、単一の共有 `State G`、NDS依存のdiagnostic snapshot/event採取、テスト用runtime patchがまだ集中している。process environmentの直接読取は0箇所になった。brace境界で再計測した現在の`State`定義は43行、`InitFromEnvironment`は280行。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は28行のオーケストレーションまで縮小した。file数や短いfileを増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめ、死んだ実験コードが見つかれば総量も削る。
...
- 完了: PacketBridgeの有効化、pre-game、wait/throttle、tick/net-ready/GameLocalPlayerID強制、pump上限、local input delay/neutralize、送信delay/jitter設定をtyped `PacketBridgeConfig`へ抽出し、既存の`NsmbNetplayConfig.{h,cpp}`へ集約した。既定値、`SEND_ALL`の反転、負値の0 clamp、tick-aheadとpump eventの上下限、lead値をclampしない既存仕様をROM不要characterization testで固定した。pump event上限64もconfig側を唯一の定義元にし、新しいfileは増やしていない。
...
- 完了: 上記`MvlConfig`の棚卸し後、repo内scenario/GUIから有効化されず汎用launcherが手動引数を中継するだけだった旧direct boot load-game-state-machine分岐6種とcourse-select factory注入を削除した。通常の`Game::loadLevel` direct bootは維持し、env 9個、State field 10個、約250行のVSConnect RAM patch/ARM trampoline、launcher引数とログ項目をまとめて除去した。Release/9 CTest/unused診断/fast goldenがpassし、rollback初回は既知のwindow=20 checkpoint不足が自然発火したが、同一条件の`logs/refactor-unused-direct-boot-rollback-retry`でcheckpoint coverageとgolden一致がpassした。
...
- 完了: NSML/game/player/world/hazard/actor/effectのwire型、kind定数、配列上限、全size static assertionを、現在の`NsmbNetplayProtocol.h`内の`WireProtocol`へまとめて物理移動した。PoC本体は249行減り、全wire packetのABI sizeとfast/rollback実ゲームgoldenを維持した。
...
- 完了: repo内launcher/test参照が0件で既定無効だった旧initial-player-spawn座標補正と、既存camera-slot mirror launcherが公開していなかったX/Y/width/height直接override分岐を削除した。通常のentrance spawn正規化は残し、env 5個、State field 10個、helper/hookを含む84行を除去した。この時点で残したcamera-slot mirrorも後続の参照監査で未使用と確認し、上記runtime patch 6群の削除へ統合した。Release/9 CTest/unused診断/fast goldenがpassし、rollbackは初回に既知のwindow=20 checkpoint不足が自然発火したが、同一条件の再実行`logs/refactor-unused-spawn-camera-rollback-retry`ではcheckpoint coverageとgolden一致がpassした。
...
- 完了: force death/powerup/inventory/starとscript remote packetの共有`State`に残っていた5配列は、patchの適用可否ではなく同じreportを一度だけ出すための診断markerだった。これらを既存`Diagnostics::Runtime`の型付き`RuntimePatchLogKind`へ集約し、実際の挙動制御である`MemPatchApplied`とは分離した。新しいproduction fileやtest targetは増やさず、diagnostics moduleはheader/source合計1,230行の一まとまり、共有`State`は48行になった。ROM不要contract testは種類/instance分離、重複抑止、invalid kind、script再戦resetと同じ個別resetを固定して20回連続passした。`logs/refactor-runtime-patch-log-extended`のpacket loss付き2-process実ゲームは1,000 framesを完走し、両peerともframe 900のextended traceでpowerup `1/2`のRAM反映を確認し、force-powerup reportは各1件だけだった。
...
- Current blocker: リファクタ自体のblockerはなし。既存rollback精度の既知課題として、20-frame送信遅延で実resimulationを強制するとclientは21-frame resimを行う一方、window=20のhostはcheckpoint不足になる。window=32でもframe 1050の`playerActor0Y`がhost/client不一致になるため、通常のrefactor regression tierはcheckpoint保存・packet loss回復のcoverageに留める。複数試合fixtureもbootstrap checkpoint成立frameが実行ごとに842/860へ揺れ、同じ入力でもresultへ入るrunと入らないrunがある。今回の再実行では両peerが5,000 framesを完走したがresultへ入らず、最終比較は既知系統のframe 1610 `movingHazardX` 1-step差で失敗したため、現時点ではJIT restore policyの実ゲームgateとして安定採用できない。fast goldenはstable ROM hashも検査するため、ROM patch変更時はsemantic差分確認と明示更新が必要。
- Next action: 残る280行の`InitFromEnvironment`と43行の共有`State G`をfeature group単位で監査する。input recorderとgame-state traceは既に各専門moduleのwriterがfile lifecycleを所有しており、diagnosticsへ再統合すると責務が逆戻りするため維持する。network pump threadは手動・split実行とrollback設計で現役であり、現状のTransportへ移すにはPoCのpacket処理を呼び戻すcallbackが必要になるため維持する。`StateSyncRuntime`は既にgame-state moduleが所有しているためfield移動は不要だった。startup reportの純粋整形は完了したため、次はmodel load、typed config適用、transport初期化という副作用phaseの境界と、NDSに依存してPoCに残したdiagnostic snapshot採取・event生成の境界を監査する。新しい小fileは増やさず、まとまった既存moduleへの集約、オーケストレーションの可読化、未使用実験コードの純減を分けて計測する。
- Verification status: Release `melonDS` build、PoCとDiagnosticsのClang strict unused診断、CTest 16件（0.47秒）がpassし、Diagnostics contract testは初回に加えて20回連続passした。`logs/refactor-startup-report-formatting`のpacket loss付き2-process diagnostics tierは1,250 frames、semantic golden、watchdog/heartbeat/event schemaをpassした。変更前後のstartup report 4行はtimestampとlog directoryだけを正規化するとbyte完全一致した。直前の`logs/refactor-diagnostic-json-formatting`ではringなし`player_life_change`のhost/client各2件もbyte完全一致している。`logs/refactor-mem-patch-config-fast`はfast golden、`logs/refactor-mem-patch-config-targeted`は両peer各1回のmemory patch、`logs/refactor-runtime-patch-log-extended`はplayer powerup `1/2`と各peer一度だけのreport、`logs/refactor-mvl-marker-runtime`はnet-random適用、`logs/refactor-ai-config-open-runtime`はAI log生成とneutral policy評価をpassしている。JIT helper初回適用は複数の上位runで両peer同一frame（870または900）を確認した。重い第2試合gateは上記のfixture非決定性により今回pass証拠としては採用しない。さらに変更前の`logs/refactor-mvl-runtime-secondgame` 10,000-frame第2試合、AI logのneutral/linear-model byte一致、hash、state sync/apply、送信delay/jitter、GUIの`coredelta`各production回帰も保存している。
