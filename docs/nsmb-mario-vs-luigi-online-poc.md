...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は10,110行（リファクタ開始前21,253行から11,143行減）で、単一の共有 `State G`、3箇所のprocess environment直接読取、barrier orchestration、game-state hash送信、テスト用runtime patchがまだ集中している。正確なtop-level境界で測ると、現在の`State`定義は240行、`InitFromEnvironment`は526行、`BeforeRunFrame`は268行、`AfterRunFrame`は171行。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は48行のオーケストレーションまで縮小した。file数を増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめる。
...
- 完了: local/remote hash、送信済みframe marker、player/star actor cache、world item 3種のspawn確認、moving hazard/actor snapshotのGUID対応表、world trace markerを、同じ`NsmbGameState`の`StateSyncRuntime`へ集約した。再戦時の約50行の直接初期化を`ResetForRestart(instance)`へ置き換え、PoC本体を68行、`State`を34行縮小した。再戦時に全peer共通のlocal/remote履歴は消す一方、対象外instanceのactor cacheと累積mismatch/trace markerは保持する従来契約をROM不要testで固定した。game-state moduleはheader/source合計1,685行の一まとまりで、新しいproduction fileは増やしていない。
...
- 完了: game-state traceの通常/extended CSV headerと、400 field超の`GameStateSample` row整形を、既存の`NsmbGameState.{h,cpp}`へ物理移動した。module全体はcodec/hash/trace formattingをまとめた1,402行となり、新しいfileは増やしていない。`TraceGameState`は449行から48行、PoC本体は419行減った。ROM不要testで通常/extended双方のheader-row列数、先頭・末尾・4 hash列、streamの10進復帰を固定し、実ゲームgoldenでもproduction trace経路を確認した。
...
- Next action: 残る526行の`InitFromEnvironment`と共有`State G`をfeature group単位で監査し、repo内launcher/test/docsから使われないPoC実験を削除する。state syncの低レベルRAM read/writeと5種packet構築はreader/writerへ寄ったため、次は`SyncGameState`のhash比較/送信境界かbarrier orchestrationを責務単位で整理する。数十行ごとの細分化は行わない。
- Verification status: Release `melonDS` build、PoC/reader/writerのClang unused診断、CTest 11件（0.32秒）がpass。固定seed・packet loss・4-frame input delay付き2-process実ゲーム回帰は`logs/refactor-state-sync-builders-fast`と`logs/refactor-state-sync-builders-rollback`で、savestate/tinycorepreimage双方の1,250 frames完走、host/client semantic state、applied input、checkpoint coverage、golden一致を確認した。専用`logs/refactor-state-sync-builders-apply`は同じ高速条件でplayer globals、world/item、moving hazard、effect、actor snapshotのsync/applyを同時に有効化して1,250 frames完走し、player apply 19行、world apply 24行、actor snapshot apply 12行、moving-hazard GUID map更新1行を確認した。短いfixture中にeffect対象slotは出現しなかったため、effectは有効設定とpacket builder呼出までのcoverageであり、実effect適用の長時間scenarioは今後も上位回帰で補う。専用apply runへ通常のhost/client完全一致gateを加えると既知の実験機能差が出るため、apply coverageには完走・実apply確認を用い、既定経路の不変性は上記goldenで判定する。診断設定を実消費する`logs/refactor-diagnostics-config-ai-log/ai-play.jsonl`もRuleAI付き実ROM1,250 framesで15行生成され、全JSONL行のparseに成功した。送信delay/jitterの実queue経路は`logs/refactor-input-delivery-delay-jitter`、ImitationAI production pathは`logs/refactor-mvl-ai-imitation`がpassしている。GUIが有効化する`coredelta`の`logs/refactor-remove-rejected-rollback-coredelta`も引き続きpassしている。
