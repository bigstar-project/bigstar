...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は16,867行（リファクタ開始前21,253行から4,386行減）で、単一の共有 `State G`、342箇所の `MELONDS_NSML_*` 設定参照、ENet、入力同期、診断、テスト用runtime patchがまだ集中している。現在の`InitFromEnvironment`は1,378行、`BeforeRunFrame`は612行、`AfterRunFrame`は476行。旧803行の`ReadGameStateSample`は37行のオーケストレーションまで縮小した。file数を増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめる。
...
- Verification status: Release `melonDS` と9 unit targetのbuild成功。CTest 9件は合計約0.05-0.28秒でpass。旧binary baseline 2,300 framesはpass。固定seedのfast tierはpacket lossありでもhost/client semantic stateとapplied inputがbyte-for-byte golden一致した。rollback tierも約14-15秒でcheckpoint保存・packet loss回復・golden一致を確認した。直近は`logs/refactor-unused-direct-boot-fast`と`logs/refactor-unused-direct-boot-rollback-retry`で再検証済み。Clang unused診断はPoC本体・config・reader・rollback storeとも0件。
