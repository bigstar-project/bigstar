...
- **Next action:** 次は最新Bigstarを使った実GUIの人間操作で、現実のWAN対戦と再戦を確認する。音声artifactはユーザー指示により後回しだが既知制約として残し、音声込み完成とは扱わない。再開時は知覚的緩和と、7 frame分のaudio遅延＋ARM7/SPU rollbackを性能・同期gateからやり直す高リスク案を分離する。
