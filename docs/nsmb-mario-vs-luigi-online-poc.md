...
- **Other remaining blockers:** 冒頭の時間軸異常は修正・回帰検証済み。通常側と訂正側で同じ在庫入力を消費するfixtureもmanual-local経路で成立し、両playerの放出frameが無訂正対照と一致した。更新版同士の実WAN対戦は未実施。online設定dialogの協調pauseまたは禁止、rollback音声取り消し不能、player spriteの訂正直後位相差、再戦checkpoint復元の停止、stage開始直後hitchは別件として残る。
- **Follow-up:** 実WAN確認では同期の再発有無を確認する。今回のplayer恒久ずれと、上の別件の表示・音声・性能課題を区別する。
