...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は11,602行（リファクタ開始前21,253行から9,651行減）で、単一の共有 `State G`、27箇所の `MELONDS_NSML_*` 設定参照、barrier orchestration、state sync send/apply、テスト用runtime patchがまだ集中している。正確なtop-level境界で測ると、現在の`State`定義は286行、`InitFromEnvironment`は559行、`BeforeRunFrame`は269行、`AfterRunFrame`は171行。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は48行のオーケストレーションまで縮小した。file数を増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめる。
...
- Next action: 残る559行の`InitFromEnvironment`と共有`State G`をfeature group単位で監査し、repo内launcher/test/docsから使われないPoC実験を削除する。input系とremote state storeの所有権は既存moduleへ揃ったため、次はstate syncのsend/apply cacheまたはbarrier orchestrationから、まとまった責務を選ぶ。数十行ごとの細分化は行わない。
- Verification status: Release `melonDS` build、Clang unused診断、CTest 11件（0.41秒）がpass。最新の固定seed・packet loss・4-frame input delay付き2-process実ゲーム回帰は`logs/refactor-remote-state-store-fast`と`logs/refactor-remote-state-store-rollback`で、savestate/tinycorepreimage双方の1,250 frames完走、host/client semantic state、applied input、checkpoint coverage、golden一致を確認した。診断設定を実消費する`logs/refactor-diagnostics-config-ai-log/ai-play.jsonl`もRuleAI付き実ROM1,250 framesで15行生成され、全JSONL行のparseに成功した。送信delay/jitterの実queue経路は`logs/refactor-input-delivery-delay-jitter`、ImitationAI production pathは`logs/refactor-mvl-ai-imitation`がpassしている。GUIが有効化する`coredelta`の`logs/refactor-remove-rejected-rollback-coredelta`も引き続きpassしている。
