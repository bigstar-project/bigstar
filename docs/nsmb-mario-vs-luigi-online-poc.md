...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は1,920行（リファクタ開始前21,253行から19,333行、約91.0%減）で、NDS依存のdiagnostic event／snapshot／life／heartbeat／screenshot adapterはheader/source合計689行の`NsmbGameplayDiagnostics`へ、現役MvL runtime patchは380行の`NsmbMvlGameHooks`へ、MvL match lifecycleは588行の`NsmbMvlLifecycle`へ、game/world state sample・apply・sync・traceは439行の`NsmbGameStateSync`へ、test savestate／LocalMP snapshotは299行の`NsmbTestStateHarness`へ、rollback checkpoint／restore／resimulation／統計は758行の`NsmbRollbackRuntime`へ、PacketBridge transport／JIT integrationは811行のPacketBridge subsystemへ、network/input sessionは1,143行の`NsmbNetplaySession`へ移った。PoCには38行の共有`State G`、AI input／observation adapter、subsystem間を接続する薄いhook、初期化とframe orchestrationが残る。process environmentの直接読取は0箇所で、`InitFromEnvironment`174行は無理に分割しない。最終的なPoC本体は1,200〜1,800行を目安に、初期化とframe処理順を追える司令塔として残す。
...
- Next action: 次は残る1,920行のうち、物理的には別fileでもPoCへテキストincludeされて`G.AI`／`G.Diagnostics`／`G.ImitationAI`／roleを直接参照する4,049行の`NsmbAiObservation.cpp`と、PoC内のRule／Imitation入力adapter約100行を整理する。AI subsystemへ明示Contextを導入して匿名global依存を外し、通常の独立translation unitにできるかを優先する。無理な細分化はせず、観測serialization・feature抽出・推論adapterの自然な境界を監査して、PoC本体を1,200〜1,800行の終了帯へ入れる。高コストなfast／standard実ROMgateは引き続きバッチ末尾に集約する。
- Verification status: Release `melonDS` build、CTest 16件（3.14秒）、変更したPoC／`NsmbGameplayDiagnostics`のClang strict unused診断がpassした。`logs/refactor-gameplay-diagnostics-fast`の同一実ROM・2-process 1,250-frame fast tierはLAN route smokeとsemantic goldenをpassした。standard初回は高CPU負荷により接続まで約176秒を要し、host 1,560／client 1,501 frameで180秒process上限に達したが、hang／stderr／diagnostic phase停止はなかった。同一契約を上限300秒で再実行した`logs/refactor-gameplay-diagnostics-standard-retry-{host,client}`では両peerが1,700 framesを完走した（211.92秒、8.02 FPS）。性能値は外部負荷の影響が大きいため比較判定に使わず、semantic gate成功を採用した。今回GUI／Rust差分はない。
