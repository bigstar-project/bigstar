...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は12,341行（リファクタ開始前21,253行から8,912行減）で、単一の共有 `State G`、27箇所の `MELONDS_NSML_*` 設定参照、ENet、入力同期、診断、テスト用runtime patchがまだ集中している。正確なtop-level境界で測ると、現在の`State`定義は381行、`InitFromEnvironment`は619行、`BeforeRunFrame`は269行、`AfterRunFrame`は172行。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は48行のオーケストレーションまで縮小した。file数を増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめる。
...
- Next action: 残る619行の`InitFromEnvironment`と共有`State G`をfeature group単位で監査し、repo内launcher/test/docsから使われないPoC実験を削除する。core設定の境界が揃ったため、ENet transportとinput送受信、generic rollback、state sync apply/send、診断の規模と依存を再測定し、最も境界が明確な一まとまりを既存moduleまたは一つの中規模moduleへ物理抽出する。数十行ごとの細分化は行わない。
- Verification status: Release `melonDS` build、Clang unused診断、CTest 10件（0.13秒）がpass。最新の固定seed・packet loss付き2-process実ゲーム回帰は`logs/refactor-core-config-direct-fast`と`logs/refactor-core-config-direct-rollback`で、savestate/tinycorepreimage双方の1,250 frames完走、host/client semantic state、applied input、checkpoint coverage、golden一致を確認した。RuleAI production pathの`logs/refactor-mvl-ai-rule-direct`とImitationAI production pathの`logs/refactor-mvl-ai-imitation`も直前の設定整理後にpassしている。GUIが有効化する`coredelta`の`logs/refactor-remove-rejected-rollback-coredelta`も引き続きpassしている。
