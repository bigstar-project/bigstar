...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は8,650行（リファクタ開始前21,253行から12,603行減）で、単一の共有 `State G`、game-state sample採取、テスト用runtime patchがまだ集中している。process environmentの直接読取は0箇所になった。brace境界で再計測した現在の`State`定義は59行、`InitFromEnvironment`は406行（旧記載の656行は現ソースと矛盾していたため修正済み）。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は28行のオーケストレーションまで縮小した。file数や短いfileを増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめ、死んだ実験コードが見つかれば総量も削る。
...
- Next action: 残る406行の`InitFromEnvironment`と59行の共有`State G`をfeature group単位で監査する。現存するplayer/star/powerup/script/JIT runtime patchはAI収集・smoke・goldenから現役利用されていることを確認済みなので維持する。次はgame-state trace、input recorder、diagnostic event/watchdogなど、PoCに残るresource初期化を既存の所有moduleへ自然に寄せられるかを確認する。新しい小fileは増やさず、まとまった既存moduleへの集約、オーケストレーションの可読化、未使用実験コードの純減を分けて計測する。
- Verification status: Release `melonDS` build、PoCのClang strict unused診断、CTest 16件（0.39秒）がpassし、AI observation runtime testは初回に加えて20回連続passした。`logs/refactor-ai-config-open-runtime`の2-process実ゲームは1,250 framesを完走し、両peerのAI log生成、semantic state比較、neutral policy評価がpassした。直前の`logs/refactor-packet-bridge-runtime-fast`ではpacket loss付きfast tierとgolden一致もpassしている。JIT helper初回適用は複数の上位runで両peer同一frame（870または900）を確認した。重い第2試合gateは上記のfixture非決定性により今回pass証拠としては採用しない。さらに`logs/refactor-diagnostic-ring-runtime` diagnostics tier、変更前の`logs/refactor-mvl-runtime-secondgame` 10,000-frame第2試合、AI logのneutral/linear-model byte一致、hash、state sync/apply、送信delay/jitter、GUIの`coredelta`各production回帰も保存している。
