...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は4,897行（リファクタ開始前21,253行から16,356行、約77.0%減）で、NDS依存のdiagnostic snapshot RAM採取は既存Readerへ、diagnostic eventのJSON整形・artifact生成・before-frame phase計測は既存Diagnosticsへ、現役MvL runtime patchは380行の`NsmbMvlGameHooks`へ、test savestate／LocalMP snapshotは299行の`NsmbTestStateHarness`へ、rollback checkpoint／restore／resimulation／統計は758行の`NsmbRollbackRuntime`へ移った。PoCには共有`State G`、診断eventの発火・snapshot選択、NDS resourceを渡す薄いadapter、MvL lifecycle、network/input、PacketBridge/JITとframe orchestrationが残る。process environmentの直接読取は0箇所で、`State`定義43行と`InitFromEnvironment`182行は無理に分割しない。最終的なPoC本体は1,200〜1,800行を目安に、初期化とframe処理順を追える司令塔として残す。
...
- Next action: 次は残る4,897行から、PacketBridge/JIT integrationまたはnetwork/input sessionの700〜1,200行級の境界を選び、関連する未使用実験分岐と合わせて1バッチで処理する。182行の`InitFromEnvironment`と43行の共有`State G`、rollbackとMvL／PacketBridgeを接続する薄いadapterは行数だけを目的に分解せず、一般`STATE_APPLY`、GUIの`STATE_SYNC`、観測trace、自然同期World gateは維持する。高コストなfast／standard実ROMgateは引き続き各バッチ末尾に各1回だけ実行する。
- Verification status: Release `melonDS`とrollback test target build、CTest 16件（0.55秒）、変更したPoC／`NsmbRollbackRuntime`／`NsmbRollbackStore`のClang strict unused診断がpassした。`logs/refactor-rollback-runtime-fast`の同一実ROM・2-process 1,250-frame fast tierはLAN route smokeとsemantic goldenをpassし、`logs/refactor-rollback-runtime-standard-{host,client}`では両peerが1,700 framesを完走した（144.47秒、11.77 FPS）。今回GUI／Rust差分はない。
