...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は17,187行（リファクタ開始前21,253行から4,066行減）で、単一の共有 `State G`、376種類の `MELONDS_NSML_*` 設定、ENet、入力同期、診断、テスト用runtime patchがまだ集中している。現在の`InitFromEnvironment`は1,439行、`BeforeRunFrame`は613行、`AfterRunFrame`は476行。旧803行の`ReadGameStateSample`は37行のオーケストレーションまで縮小した。
...
- Next action: `BeforeRunFrame`/`AfterRunFrame`のframe orchestrationと、診断/runtime patch群を責務単位の中規模moduleへ整理する。並行してrepo内launcher/docs参照がない実験feature groupを静的参照・実ゲーム回帰で確認しながら削除する。
- Verification status: Release `melonDS` と9 unit targetのbuild成功。CTest 9件は合計約0.05-0.28秒でpass。旧binary baseline 2,300 framesはpass。固定seedのfast tierはpacket lossありでもhost/client semantic stateとapplied inputがbyte-for-byte golden一致した。rollback tierも約14-15秒でcheckpoint保存・packet loss回復・golden一致を確認した。直近は`logs/refactor-unused-spawn-camera-fast`と`logs/refactor-unused-spawn-camera-rollback-retry`で再検証済み。Clang unused診断はPoC本体・reader・rollback storeとも0件。
