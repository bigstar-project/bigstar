...
- 2026-06-23 current rollback goal continuation after the main merge:
  ...
  - Isolated `tinycoreramdelta` tiny-core flags. Base `0x241` with full Main RAM delta is not enough under complex input. Adding one extra device state and raising `RollbackMaxResimFrames` to `8` makes StateSync pass on `dualstressmid`; the best measured point so far is `exact-delay2-tinycoreramdelta-wifi-maxresim8-skiprender`, which passed at `18.490/18.449ms`, max `179.892/186.715ms`, save avg `~1.3ms`, restore avg `~20ms`, and resim total avg `~121ms`.
  - Rejected follow-ups in this pass: `maxresim6` is correctness-unsafe, full GPU/device flags are correct but slower (`~21-22ms`), `RollbackSkipJitReset` does not improve timing here, `InputMaxFrameLead=2` reduces correction count but drops steady timing to about `25ms`, and `RollbackResimulateDelayFrames=1/2` only mildly reshapes spikes.
  - Current interpretation: lightweight checkpointing is feasible (`~1.3ms` save with tiny-core + Main RAM delta), but the active goal is still blocked by foreground restore + resimulated `RunFrame`, not by snapshot byte size. The next useful direction is either a correct selected lifecycle snapshot validated against the `tinycoreramdelta-wifi-maxresim8` oracle, or a non-blocking/shadow correction architecture. Do not raise `InputDelayFrames` above `2`.
