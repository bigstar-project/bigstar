...
- Current blocker: with `InputDelayFrames=2`, correctness-oriented rollback is now possible through `tinycoreramdelta`, but it still produces visible spikes/over33 frames under strong jitter. NSMB process-list snapshots are lighter but not correctness-safe across contact/chaos. Raising GUI rollback InputDelay above `2` is explicitly rejected for this goal.
- Next actions: keep `tinycoreramdelta` as the exact-ish oracle/baseline and optimize away main-frame JIT/reset/resim spikes, or move to a different architecture such as off-main-thread shadow resim with atomic publish. Continue treating broad actor arena/tail copying as rejected.
