...
- Next action: 次は残る3,416行から、MvL actor lifecycle／world state sync／game-state mismatch処理の凝集した境界を監査し、Reader／Writer／Diagnosticsとの重複や未使用の旧PoC診断分岐をまとめて整理する。174行の`InitFromEnvironment`、38行の共有`State G`、rollback／PacketBridge／AI／Sessionを接続する薄いadapterと公開frame orchestrationは、行数だけを目的に分解しない。高コストなfast／standard実ROMgateは引き続き各バッチ末尾に各1回だけ実行する。
- Verification status: Release `melonDS`と`nsmb_session_policy_tests` build、CTest 16件（0.52秒）、変更したPoC／`NsmbNetplaySession`／ProtocolのClang strict unused診断がpassした。`logs/refactor-netplay-session-fast`の同一実ROM・2-process 1,250-frame fast tierはLAN route smokeとsemantic goldenをpassし、`logs/refactor-netplay-session-standard-{host,client}`では両peerが1,700 framesを完走した（129.55秒、13.12 FPS）。今回GUI／Rust差分はない。
