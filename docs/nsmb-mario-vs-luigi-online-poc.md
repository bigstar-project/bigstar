...
- Current blocker: with `InputDelayFrames=2`, exact rollback can be correct but still stalls/spikes because even a two-frame resim often costs more than one display frame. NSMB process-list snapshots can make checkpoint/restore very cheap, but raw actor/global RAM range restore is not yet correctness-safe across contact/chaos and broad tail copies can reintroduce stalls.
- Next actions: do not raise GUI rollback InputDelay above `2`. Stop treating broad actor arena/tail copying as the main path. Continue toward pointer-safe field/page snapshots or a different rollback architecture that can correct contact/object lifecycle state without restoring pointer-heavy actor internals on the main frame.
