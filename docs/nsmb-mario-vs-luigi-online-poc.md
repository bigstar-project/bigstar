...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は13,757行（リファクタ開始前21,253行から7,496行減）で、単一の共有 `State G`、87箇所の `MELONDS_NSML_*` 設定参照、ENet、入力同期、診断、テスト用runtime patchがまだ集中している。正確なtop-level境界で測ると、現在の`InitFromEnvironment`は911行、`BeforeRunFrame`は269行、`AfterRunFrame`は172行。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は48行のオーケストレーションまで縮小した。file数を増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめる。
...
- Verification status: Release `melonDS` と10 unit targetのbuild成功。CTest 10件は直近約0.2秒でpassし、追加したRuleAI contract test単体は約0.05秒。旧binary baseline 2,300 framesはpass。直近の固定seed・packet loss付き回帰は`logs/refactor-rule-ai-module-fast`と`logs/refactor-rule-ai-module-rollback`で、host/client semantic state、applied input、rollback checkpoint coverage、golden一致を確認した。`logs/refactor-rule-ai-state-module`はRuleAI有効化後に実ゲーム1,100 framesを完走し、起動設定ログ1件と実入力判断trace 8件を確認した。変更したPoC本体のClang unused診断は0件。
