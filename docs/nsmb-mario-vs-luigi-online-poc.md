...
- 2026-06-23 latest continuation after rebuilding melonDS:
  ...
  - Current blocker: checkpoint size is not the limiting factor. With exact-ish state, any real correction still performs restore/JIT lookup-reset plus one or more cold-ish `RunFrame` re-executions inside the visible frame, producing `70-130ms` spikes even when average frame time is near `18ms`.
  - Next actions: keep `jitlookup-cp1-maxlead0-leadbudget2ms` as the current correctness oracle for local/no-artificial-delay rollback, but do not promote it for jitter. Continue toward either pointer-safe JIT reuse that does not mismatch, or a new architecture that avoids doing exact correction resimulation inside the displayed frame.
