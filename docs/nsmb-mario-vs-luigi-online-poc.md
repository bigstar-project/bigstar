...
- 2026-06-26 continuation summary:
  - Latest Plan-D/semantic-lane retake after the FPS-trace cleanup: `MELONDS_NSML_FPS_SPIKE_TRACE` is now lightweight and detailed phase tracing is gated separately by `MELONDS_NSML_PERF_SPIKE_PHASE_TRACE`, so FPS-spike detection can stay enabled without the heavier checkpoint/scratch phase logging.
  - Star candidate `0x010C` now uses active-only cached sampling for normal world-state sync, keeps the absent/deep scan on a separate low-rate timer, and prunes extra local star candidates at most once per deep-scan interval. This fixed the `0x10C/0x1120` duplicate seen when returning the deep scan to `30F`, without the severe `51fps` slowdown caused by pruning every world-state packet.
  - Moving-hazard sync now treats the local freeze skip-update flag as a host-only bit for hash/diff purposes and allows a bounded stale fresh sample (`MELONDS_NSML_WORLD_STATE_MOVING_HAZARD_MAX_STALE_FRESH_FRAMES=4`) rather than dropping the update. Local-view object `0x010B` and fine camera/layout words `CAF20/CAF40` are still logged but excluded from the gameplay `Basic` hash.
  - Current measured target remains `pland-p0-kf30-hz-pfresh1500-gates-worddiff-hzfreeze`, with `InputDelayFrames=2`, `InputSendDelayFrames=4`, `InputSendJitterFrames=4`, StateSync, and lightweight FPS-spike tracing. Retakes after the extra-star prune interval fix passed: `dualstressmid` `16.986/16.835ms`, max `45.946/28.924`, over33 `4/0`; `chaos` `17.286/17.165ms`, max `49.483/27.703`, over33 `5/0` (`logs/pland-semantic-continue/20260626-074951`); `contact` `16.899/16.898ms`, max `32.790/33.309`, over33 `0/1` (`logs/pland-semantic-continue/20260626-075221`). All three had `TransientMismatchCount=0`.
  - Fixed-seed retakes after discarding the moving-hazard spawn experiment passed the previously suspect seeds on current HEAD: `chaos` with seed `practical-pland-p0-kf30-hz-pfresh1500-gates-worddiff-hzfreeze-chaos-20260626-080023` passed at `17.113/17.112ms` (`logs/pland-semantic-fixed/20260626-080820`), and `dualstresslong` with seed `practical-pland-p0-kf30-hz-pfresh1500-gates-worddiff-hzfreeze-dualstresslong-20260626-075450` passed at `17.127/17.065ms` (`logs/pland-semantic-fixed/20260626-080947`). Targeted `MELONDS_NSML_WORLD_STATE_SPAWN_MOVING_HAZARD=1` on this candidate was rejected because it reintroduced `0x010C` duplicate mismatch in chaos.
  - Rejected or scoped findings from this pass: star-candidate spawn and actor-snapshot spawn remain rejected because they stall; moving-hazard prediction bias `+1/-1` is rejected; star candidate deep scan `30F` without dedicated extra prune can duplicate `0x010C`, while `20F` produced a slow/persistent-mismatch run. `luigistar`/`mariostarleft` route assertions currently fail because the scripted star pickup path no longer reaches the current fixed-star position, while host/client stay mutually aligned; those routes need repair before they can reject the rollback candidate.
  - Plan-D moving-hazard lifecycle trace narrowed the `3900/3960` chaos failure. In `logs/pland-lifecycle-trace/20260626-034414`, host destroyed GUID39 at frame `3891` but kept GUID27 alive, while client destroyed both GUID39 and GUID27 by frame `3893`; after that the client had no live local hazard to apply the host sample to. This means the failure is not only missing object bytes. The client-side game update can prematurely destroy a host-authoritative hazard and remove process-list ownership.
  - Direct process-list relink was tested as a diagnostic and rejected. It can temporarily hide the missing-hazard mismatch, but the manual relink path can create self-looped process nodes (`exec:021B4724->021B4724`, `render:021B4734->021B4734`) and causes FPS collapse/abort risk. It remains an env-gated diagnostic only, not a promotion path.
  - Added a host-authoritative moving-hazard freeze candidate: after applying a remote moving-hazard sample, clear stale link fields and set the local skip-update bit so the client game update does not immediately destroy or move that hazard independently. Candidate `pland-p0-kf30-hz-pfresh1500-gates-worddiff-hzfreeze` passed artificial-delay `chaos` at `16.911/16.912ms`, max `46.189/29.587`, host over33 `4`, client over33 `0` in `logs/pland-semantic-continue/20260626-035856`.
  - Stronger jitter exposed two follow-up lifecycle holes and both now have a measured mitigation. Immediate pruning of extra moving hazards was rejected because it worsened chaos/dualstressmid and caused `18ms+` averages in `logs/pland-semantic-continue/20260626-042529`. Delayed extra-hazard pruning (`90F`) kept the useful behavior: strong-jitter `dualstressmid` passed at `16.861/16.836ms` in `logs/pland-semantic-continue/20260626-043042`.
  - Star candidate `0x010C` is now a dedicated host-authored world-state lane instead of relying only on the generic 32-entry actor snapshot. Empty actor snapshots are also sent/applied so host absence can prune client-only actors. A naive per-frame full scan fixed StateSync but was initially too slow with full spike tracing; the current candidate uses host full detection, client low-rate absent deep scan (`30F`), and a short world-state-only fresh wait (`100us` without sub-ms sleep). With that, strong-jitter `chaos` passed at `17.160/17.029ms`, max `47.592/31.081`, host over33 `4`, client over33 `0` in `logs/pland-semantic-continue/20260626-053728`; strong-jitter `dualstressmid` passed at `16.968/16.968ms` in `logs/pland-semantic-continue/20260626-053853`.
  - Spike tracing itself can still perturb the strong-jitter chaos timing/state boundary: `logs/pland-semantic-continue/20260626-054017` averaged `17.207/17.207ms` with max `44.304/28.727`, but reintroduced a star candidate mismatch. Treat spike-trace runs as heavier validation until this is isolated; trace-off StateSync is currently the cleaner gameplay signal.
  - Regression checks with GUI-standard delay plus artificial send delay/jitter also passed after the star lane change: `chaos` single-route retake passed at `17.079/16.951ms` in `logs/pland-semantic-continue/20260626-050516`, `dualstresslong` passed at `16.966/16.966ms`, and `stocktouch` passed at `16.852/16.833ms` in `logs/pland-semantic-continue/20260626-050041`. One three-route batch showed a `chaos` perf outlier (`18.19ms`), so single-route retakes remain the cleaner timing signal.
  - The same freeze line also passed `dualstresslong` with StateSync at `16.866/16.866ms` in `logs/pland-semantic-continue/20260626-040023`, passed `stocktouch` with StateSync in `logs/pland-semantic-continue/20260626-040715`, and passed `contact` without StateSync at `16.796/16.882ms`, max `27.995/27.428` in `logs/pland-semantic-continue/20260626-040558`. `contact` with StateSync still averaged `17.7-17.9ms`, which is currently interpreted as validation overhead rather than runtime play cost because the no-StateSync route is smooth.
  - The current `death` route is still not a valid discriminator for this line. It fails the route-level death assertion on hzfreeze, on p0 without freeze, and on the older `pland-p4-kf30-hz-dynpred-redeath` comparison; this matches earlier evidence that even no-rollback baselines can fail the route assertion. The route should be fixed or replaced before using it to reject rollback candidates.
  - Current interpretation of the new Plan-D line: host-authoritative freeze plus delayed hazard pruning and the dedicated star-candidate lane are the first measured Plan-D-ish repair set that keeps both strong-jitter chaos and dualstressmid near 60fps without persistent StateSync mismatch. It is not goal-complete yet. Remaining risks are moving-hazard collision/damage semantics under skip-update, player transition/global edge cases under more manual-like routes, the invalid `death` route assertion, and separating StateSync validation overhead from real runtime FPS.
  - New selected `nsmbtinycore` / GPU3DLight retake after rebuilding: `nsmbtc-rb0-mr10-lead6-budget2ms-lookupjit-limit2-cp2-confirmedstate-arena-stack-devices-gpu3dlight` passes no-artificial `chaos` at `16.875/16.881ms`, max `89.489/47.178ms`, over33 `6/2`, rollback spikes `2/1`, and no transient mismatch (`logs/rb-continue-cp2-mr10-nodelay/20260626-020613`). This confirms the selected snapshot can be correct and near 60fps when remote input does not arrive in delayed bursts, but rollback frames are still visible.
  - The same line under artificial `InputSendDelayFrames=2` / `InputSendJitterFrames=2` remains outside the goal. `mr6/cp2` improves average (`16.808/17.050ms`) but can mismatch early because `RollbackMaxResimFrames=6` caps old prediction corrections. Removing the cap (`RollbackMaxResimFrames=0`) removes transient mismatch but returns to about `17.66ms` average and `119/134ms` max rollback frames (`logs/rb-continue-cp2-uncapped/20260626-015209`). `mr8/mr10` and `limit4` keep persistent StateSync clean but still show a 30-frame transient `playerActor0Y` mismatch and fail average gates under the forced delay.
  - Diagnostic shortcuts were rejected in this pass. Startup-only input waiting (`MELONDS_NSML_ROLLBACK_STARTUP_INPUT_WAIT_*`) waits inside packet scratch, adds `~16-20ms` scratch spikes, and still predicts. Held future input bundles (`MELONDS_NSML_INPUT_BUNDLE_FUTURE=6`, assume-held) remove some transient mismatch but introduce persistent `playerGlobal` mismatch and worse averages. Skipping intermediate resim checkpoints lowered average to `17.242/17.272ms` with no transient mismatch, but hit host ARM9 prefetch abort at frame `2665`, so it is unsafe.
  - Shadow scratch-feed retake: persistent shadow jobs now carry the foreground local/remote/predicted input and write packet scratch before `shadow->RunFrame()`. This removed one obvious divergence source, but `logs/rb-shadow-scratchfeed/20260626-021556` still failed as a backend: host avg `19.699ms`, host over33 `193`, shadow `RunFrame` avg `34760us`, `ShadowMatch=0`, and `5869` differing Main RAM bytes. The current second-NDS shadow is therefore CPU-bound/divergent even when fed the same scratch input.
  - Foreground scheduling retakes: tightening `InputMaxFrameLead` to `4` or `2` reduced some max spikes but raised averages (`17.68ms` and `17.87ms`). `skipmidcp+lead2` avoided transient mismatch and aborts in one long chaos run with max `69.8/92.7ms` and over33 `22/24`, but still averaged `17.871/17.874ms`. Disabling predicted-input lead throttle restored average near the target (`16.851/17.096ms`) but increased correction spikes (`136/101ms`, rollback spikes `47/7`). `limit1` smoothed the host but stalled the client around frame `2109`. A pending-only throttle variant was worse (`17.887/17.898ms`, host max `178.948ms`). These are diagnostic, not promotable.
  - Current interpretation: the selected `nsmbtinycore` shape is viable for normal/no-artificial delay2 play, but artificial delay exposes the same architectural wall: correctness requires correcting old predictions, and foreground restore + replay creates visible spikes. `MaxResimFrames` capping is not correctness-safe, while lead/throttle tuning only trades average FPS against correction spikes. The current second-NDS shadow is not viable either; the next useful work is a different non-blocking model that avoids running a full second emulator per foreground frame, or a semantic selected-state model that avoids full foreground replay.
...
- Latest Plan-D follow-up:
  ...
  - Player prediction is now a split boundary, not a single winner. `pland-p4-kf30-hazardlife` is still the strongest measured chaos shape under `InputSendDelayFrames=2` / `InputSendJitterFrames=2` (`16.698/16.666ms`, max `44.146/30.377ms`, no persistent mismatch in `logs/rb-p4hz/20260625-052011`), but it fails artificial `dualstresslong` with player-global/death drift around `3420/3540` (`logs/rb-p4hz-dual/20260625-052145`).
  - Reducing player prediction to 0F with the same hazard lifecycle (`pland-p0-kf30-hazardlife`) flips the result: artificial `dualstresslong` passed at `16.687/16.666ms`, max `41.392/32.100ms`, with only transient `significantActiveObjects` (`logs/rb-pred-hazardlife/20260625-060801`), while artificial chaos still fails later on moving-hazard/transition drift around `3900/3960`.
  - A lightweight `PlayerStateApplyTrace` showed that some player globals, especially `Game::playerVSPipeState`/transition status, can be applied and then overwritten again within the same frame. Broad reapply of overwritten globals can remove some death/global drift, but it makes chaos unstable or can trigger ARM aborts; critical-only reapply avoids that instability but does not solve transition/position drift. These variants are diagnostic, not promotable.
  - Moving-hazard spawn is also rejected for now. `pland-p0-kf30-hzspawn` created the missing hazard but left it lagging behind the host and pushed the client to `24.228ms` average with `296` over33 frames in chaos (`logs/rb-p0-hzspawn/20260625-061520`).
- Rejected in the latest pass:
  - `pland-p1-kf30-hazardlife` and `pland-p2-kf30-hazardlife` did not bridge the p0/p4 split. `p1` produced a `275ms` class spike in chaos and failed dualstresslong performance; `p2` still mismatched player position/star lifecycle.
  - `pland-p4-kf30-hz-reglobal`, `pland-p4-kf30-hz-recrit`, and transition-local variants are not promoted. They are useful evidence that player global apply order matters, but they do not produce mismatch-free chaos and dualstresslong together.
...
- Next actions: keep Plan-D/freeze/star-lane as the timing baseline and `nsmbtinycore` MR2 as the foreground correctness/cost reference. Validate whether frozen host-authoritative moving hazards still collide/damage correctly under complex manual-like input, add/repair routes that stress player contact, death/respawn, and item/star movement without false route assertions, and continue splitting player/global state into deterministic authoritative lanes instead of widening one-off actor ranges. Current shadow and foreground exact variants are measured boundaries, not promotion candidates.
...
- Latest continuation after the shadow/Plan-D split:
  - Added a rollback-only NDS cart runtime subset (`tinyFlags` bit `0x400`) after retakes showed that adding full Cart state (`0x259`) can stabilize some star/object lifecycle drift but costs about `1.2ms` checkpoint save time. The new subset saves cart slot/interface/encryption/transfer runtime state and retail SRAM command/status fields, but skips ROM checksum, full SRAM bytes, GBA cart state, and save-file writeback during rollback restore.
  - New best selected foreground shape is `b1500-arena-lite-changedjit-cp2-cartlite-mr1`: `nsmbtinycore`, actor arena + process-list + full safe player object ranges, changed-page JIT invalidation, `cp2`, cart runtime subset, and `RollbackMaxResimFrames=1`. It is StateSync-clean in the measured `chaos1500` sendDelay2/jitter2 retakes and can average about `17.29-17.90ms`, with save avg back down to about `0.17-0.18ms`. It is still not goal-complete: visible rollback spikes remain around `50-55ms` and over33 counts stay around `5-7`.
  - Rejected adjacent timing knobs for the cartlite line: `RESIM_CONSUME_CURRENT_FRAME` worsened rollback count/average, and `RollbackInputWaitUs=250/500` shifted cost into the client or raised rollback count. `MaxResimFrames=2/4` remains slower/spikier than `mr1`. This confirms the current selected path is now limited by foreground restore + one resimulated `RunFrame` plus the normal displayed frame, not checkpoint save size.
  - Cart bit split result: `0x249 + Cart(0x10)` can pass one strict suite run but is not repeat-stable because full Cart save raises average to `18.2ms+`; `0x249 + SPU(0x4)` and `0x249 + Mic/SPI/RTC(0x20)` did not promote. The cart runtime subset keeps the useful cart timing state without the full SRAM/checksum cost, but does not eliminate foreground correction spikes.
  - Continuous persistent-shadow was measured and rejected for now. It eliminates queue drops, but the shadow `RunFrame` still costs about `36ms` per frame and the shadow state diverged from foreground by thousands of Main RAM bytes, so it is not a usable non-blocking correction backend on this implementation.
  - The current practical path is Plan-D-ish selected Main RAM: `nsmbtinycore` + actor arena + process-list + full player object safe ranges, with heap/stack scan removed. The actor arena fixes the previous star/lifecycle mismatch that appeared when a dropped star/actor moved differently between host and client.
  - Added a `hidden-basic drift` filter for StateSync: if only the non-wire `Basic` hash differs while `playerGlobal`, `wifiCandidate`, `renderCandidate`, and comparable wire sample fields match, the run is logged as hidden diagnostic drift rather than fatal gameplay mismatch. This avoids false positives from local-only diagnostic fields.
  - Replaced broad JIT range invalidation with changed-page invalidation for the best selected path. `b1750-arena-lite-changedjit` stays StateSync-clean on `chaos1500` sendDelay2/jitter2 and improves the earlier arena-lite average from about `19.05ms` to best retakes around `18.1-18.2ms`, but still has visible rollback spikes.
  - Fixed `RESIM_CONSUME_CURRENT_FRAME` scheduling: after a consume rollback advances to `targetFrame=frame+1`, `BeforeRunFrame` now stops applying stale old-frame patch/scratch/checkpoint work before EmuThread skips the already-consumed frame. This removes the consume-path mismatch, but consume is still slower than non-consume in current selected runs.
  - Current best measured selected candidate is `b1500-arena-lite-changedjit-cp2`: `chaos1500` sendDelay2/jitter2 passed StateSync with `17.895/17.830ms`, but still had max `54.745/90.818ms` and over33 `6/5`. A stricter spike-focused variant `b1500-arena-lite-changedjit-cp2-lead0` reduced max to `42.667/48.123ms` and over33 to `3/3`, but averaged `18.278/18.271ms`. This is progress, not goal-complete.
  - `norollback-delay4-lead4-bundle8` remains the GUI rollback-off baseline: the same `chaos1500` sendDelay2/jitter2 run passed at `17.042/17.067ms` with host over33 `0` and client over33 `1`. The machine can run the scenario; the remaining gap is rollback correction placement/resimulation cost.
- Persistent-shadow feasibility instrumentation was added after the pre-scratch sweep. `MELONDS_NSML_PERSISTENT_SHADOW_PROBE` now reports enqueue cost, worker restore/run cost, queue lag/drop, compare status, and the practical suite exports those fields to `summary.csv`.
- First `shadowprobe` retake on `chaos1500` with sendDelay2/jitter2 was negative for the current implementation: host avg `34.725ms`, client avg `22.759ms`, host shadow `600` enqueued / `471` completed / `47` dropped, latest lag `82`, max lag `904`, enqueue avg `2.306ms`, restore avg `8.893ms`, run avg `40.562ms`, and no final compare. Host stdout also spammed `undefined ARM7 instruction 363F013F @ 037FBC28` while the shadow worker ran. Log: `logs/rollback-shadowprobe-20260624/20260624-183818`.
- Interpretation of that run: the current worker is not yet a usable non-blocking backend. It still restores a tiny-core+MainRAM checkpoint for each queued frame and runs one shadow frame, so it does not preserve enough continuous execution/JIT history and cannot keep up with foreground time. The ARM7 undefined-instruction spam also suggests missing/unsafe shadow state or shared host-state/thread interaction that must be fixed before using shadow output for correction.
- Follow-up correction-placement experiment: added gated pre-scratch rollback draining (`MELONDS_NSML_ROLLBACK_PRE_SCRATCH_RESIM`) plus an interval knob (`MELONDS_NSML_ROLLBACK_PRE_SCRATCH_RESIM_INTERVAL`). This pumps network and runs pending rollback after local input is recorded but before the current checkpoint/scratch write, so a same-hook correction can be applied without restoring over the current scratch packet.
- `pre-scratch` fixed the important correctness symptom from the previous `critical2` no-wait run. The earlier long `chaos` mismatch at frame `990` disappeared with `nsmbtc-rb0-mr6-lead6-budget2ms-lookupjit-limit1-cp12-adaptive2-prescratch-confirmedstate-arena-stack-devices-gpu3dlight`, but performance failed (`18.268/18.279ms`, max `113.334/129.075ms`).
- Prescratch interval sweeps did not reach the goal. Interval `3` kept StateSync clean but still failed long `chaos` (`18.415/18.427ms`, max `108.747/128.206ms`). Interval `4` lowered average (`17.587/17.834ms`) but hit host ARM9 prefetch abort at frame `2665`; interval `6` was still too slow (`18.444/18.449ms`). `mr5`, `adaptive3`, and `RESIM_CONSUME_CURRENT_FRAME` combinations were also rejected.
- Spike trace confirmed the current bottleneck is still rollback/resim time, not packet scratch or steady checkpoint save. `BeforeHookPhaseSpike` lines are dominated by `rollbackMs`; an apparent checkpoint spike did not reproduce after adding checkpoint-save detail tracing.
...
- 2026-06-24 latest selected-snapshot redo after the stash-loss report:
  ...
  - `b1750-fullstar-core249` is better than the previous star/lifecycle mismatch shape, but it is not goal-complete: chaos/sendDelay2/jitter2 stays around `18.1ms` average with rollback spikes around `60-65ms`, and a retake still had a short `playerActor0Y/playerGlobal` transient. Lower-budget or timing-shape variants (`b1500/b1600/b1700`, `lead1`, `poll100`, `resimdelay1`) either desynced persistent star/player/global state or stalled badly.
  - Rejected widening blindly: adding dropped-item object `0x001F` or increasing player object length to `0x1000` made correctness or spikes worse. Continue with root correction-placement/lifecycle work, not InputDelay increases and not one-off actor widening.
- 2026-06-24 latest strong-jitter retake:
  - Restored the lost rollback-suite/local-triage changes after the user's stash accident. `run-nsmb-mvl-local-triage.ps1` now mirrors the route-smoke hardlink behavior: host/client ROMs are hardlinked into log dirs by default and `-CopyRomToLog` is available only when standalone ROM copies are required.
  - Added selected `nsmbtinycore` diagnostics for process-list metadata ranges, arbitrary extra address ranges, and full player object snapshots inside the otherwise safe-object-field path. These are controlled by `MELONDS_NSML_ROLLBACK_NSMB_PROCESS_LIST_METADATA_RANGES`, `MELONDS_NSML_ROLLBACK_NSMB_EXTRA_RANGES`, and `MELONDS_NSML_ROLLBACK_NSMB_FULL_PLAYER_OBJECT_WITH_SAFE_RANGES`.
  - Strong-jitter retake of the selected `fullplayer + heap15 + extra2p + GPU3DLight` path shows the current boundary. `InputMaxFrameLead=0` is the strongest correctness point: no `playerGlobal=0` StateSync mismatch in the measured 4200F chaos run, but still too slow (`19.756/19.757ms` with StateSync; `18.111/17.843ms` without StateSync, max up to `83.369ms`). `cp2` remains state-aligned in the measured run but only reaches `18.624/18.624ms` without StateSync and still has `60ms+` rollback frames. `lead1/lead2` are correctness-unsafe: `lead1` mismatched `playerGlobal=0` from frame `1440`; `lead2` reached `playerGlobal=0` and ARM9 prefetch abort in retakes.
  - Added a trace-only extension so `MELONDS_NSML_ROLLBACK_DELTA_PAGE_TRACE=1` can inspect selected `nsmbtinycore` coverage against the previous checkpoint without changing the saved checkpoint format. This exposed byte-level uncovered changes around `0x020890CD`, `0x021B2C00-0x021B43xx`, `0x023260FE`, and `0x0234A15E` in a `lead2` run, even when page-level coverage looked complete. Blindly adding these as extra ranges did not fix correctness and made the first persistent mismatch earlier (`frame=1140`), so the leak set contains timing/local or otherwise unsafe fields, not just missing deterministic game state.
  - `lead2` rollback input waits are also rejected for this selected path. `rbwait500` still mismatched from frame `1020` and averaged `20.312/20.312ms`; `rbwait1000` still mismatched from frame `1260` and averaged `21.267/21.264ms`. This reinforces that the next useful work is not more wait/coalescing or blind range widening, but separating deterministic actor/global state from local/timing fields or moving correction off the displayed frame.
  - Added `MELONDS_NSML_INPUT_FRAME_LEAD_RESEND_INTERVAL_MS` and selected `nsmbtinycore` GPU3DLight candidates for `lead4/lead6`, `resend5`, `resend1`, `noskipjit`, and `mr4` boundary checks. These are diagnostic candidates, not promoted defaults.
  - Under `InputDelayFrames=2`, `InputSendDelayFrames=4`, `InputSendJitterFrames=4`, selected GPU3DLight remains split: `mr2` keeps spikes lower (`~55-63ms`) but mismatches player actors and can still abort; `mr4/mr6` can avoid persistent mismatch on contact but still spikes around `129-149ms`, and chaos commonly hits ARM9 prefetch abort around frame `2665-2678`. Removing `MELONDS_NSML_ROLLBACK_SKIP_JIT_RESET` reduces one unsafe axis but does not make the selected path playable.
  - Rechecked selected GPU3DLight with `MELONDS_NSML_ROLLBACK_JIT_LOOKUP_RESET_ONLY=1` instead of broad skip-JIT-reset. `mr6 lookupjit` avoided mismatch/abort in contact and chaos in the measured retake, but still failed performance (`contact` `17.856/17.857ms`, max `110.705/114.752`; `chaos` `18.414/18.410ms`, max `122.719/126.596`). Lowering to `mr4/mr3` reduced some averages but reintroduced chaos ARM9 prefetch abort at frame `2678`, and `mr3` also mismatched `playerActor0Y` on contact. This confirms the selected GPU3DLight/JIT boundary is still a correctness-vs-spike tradeoff, not a promotion path.
  - Fixed a `RESIM_CONSUME_CURRENT_FRAME` bookkeeping bug: the resim path advances to `targetFrame=frame+1`, but the normal `RunFrame` skip marker was recorded against the old `frame`, so EmuThread re-read `NumFrames=targetFrame` and could still run an extra frame. The skip marker now uses `targetFrame`. After rebuilding, `jitlookup-compiledchunks-skiprestorejit-consume-cp1-maxlead0-leadbudget2ms` passed strong-jitter contact/chaos with no transient mismatch (`contact` `18.720/18.836ms`, max `46.763/51.848`; `chaos` `18.652/18.653ms`, max `45.123/48.566`). This is a correctness/timing-shape improvement over the broken consume path, but still 53-54fps-class and not goal-complete. `RESIM_SKIP_FINAL_CHECKPOINT` remains rejected because chaos mismatched `playerActor0X` at frame `990`.
  - Took a full rollback-stats retake for the exact-ish oracle under the same strong jitter. The useful cost split is restore avg `~5.0-5.4ms`, resimulated `RunFrame` avg `~14.5-15.0ms`, resim total avg `~21-22ms`, and checkpoint save avg `~1.2ms`. This confirms again that the active cost is same-frame restore plus a full resimulated `RunFrame`, not checkpoint bytes.
  - Tried selected GPU3DLight `mr6 lookupjit` with rollback input waits as a correction-coalescing diagnostic. `rbwait500` slightly lowered contact max (`94.549/97.350ms`) but still failed the spike gate and hit host ARM9 prefetch abort in chaos at frame `2665`; `rbwait1000` worsened averages to about `18.5ms` contact and `19.2ms` chaos. These waits are rejected for the selected lookupjit path.
  - Tested selected lifecycle coverage additions on `mr6 lookupjit`: heap scan interval 60/15 and `widearena10000`. `heap60` still hit the same chaos ARM9 prefetch abort at frame `2678`. `heap15` and `widearena10000` can avoid abort in a retake and get averages near `16.9-17.2ms`, but they still produce `105-119ms` rollback frames because a single correction replays many frames (`resimFrames` around `323` over `55` corrections in one widearena run). Lowering to `mr4 heap15` keeps chaos state-aligned but averages `18.408/18.407ms` with max `116.723/115.515`; lowering to `mr3 heap15` reintroduces `playerActor0X` mismatch on contact and ARM9 abort in chaos. This rejects "selected lifecycle + lower maxResim" as the current solution.
  - Tried capping selected `mr4 heap15` frame lead to reduce multi-frame correction windows. `lead4` still hit host ARM9 prefetch abort in chaos at frame `3285` (`18.373/18.457ms`, max `98.560/107.602`), while `lead3` stayed aligned in the measured chaos run but averaged `19.321/19.326ms` with `100ms+` rollback max. Lowering frame lead therefore trades replay spikes for waits/instability and does not solve the delay2 rollback target.
  - Rechecked the exact-ish oracle under the same stronger jitter. `jitlookup-compiledchunks-skiprestorejit-cp1-maxlead0-leadbudget2ms` passed contact and chaos without persistent mismatch (`18.411/18.283ms`, max `43.199/46.064`; chaos `18.530/18.531ms`, max `43.988/45.987`). This is now the best correctness reference for forced jitter, but it is still roughly 54fps-class, not a 60fps solution.
  - Rejected exact-ish timing shortcuts in this pass: `rbwait1000` mismatched chaos at frame `990`; leadbudget `4/6ms` worsened average to `19-21ms` and mismatched chaos; `InputMaxFrameLead=1/2` improved average to `~17.8-18.4ms` but caused transient/persistent actor mismatches. A new diagnostic `MELONDS_NSML_INPUT_FRAME_LEAD_THROTTLE_POLL_US` showed that `poll100` can stay correct but is slower than the baseline (`chaos` `18.719/18.720ms`), while `poll250` worsened average and mismatched chaos.
  - Rechecked correction placement under the same forced jitter. `cp2+resimDelay2+rbwait500/750/1000` can pass contact, but all chaos runs mismatched (`playerActor0X` at frame `990` for rbwait500/1000, `playerActor1X` at frame `1410` for rbwait750). Keeping cp1 and adding only `resimDelay1` stayed correct on contact/chaos, but was slower (`contact` `19.028/19.029ms`, chaos `18.619/18.623ms`) than the cp1 oracle; `resimDelay2` mismatched contact and chaos.
  - Current interpretation: for strong jitter, exact-ish rollback is the trustworthy oracle and selected GPU3DLight is still correctness-unsafe. The remaining gap is architectural: foreground exact correction is now correct enough but averages above 16.7ms, while selected snapshots are fast only when the correction/lifecycle boundary is easy. Continue toward shadow/non-blocking correction or a pointer-safe selected lifecycle snapshot validated against the exact-ish oracle.
- 2026-06-23 latest selected-snapshot continuation:
  - Rechecked the older Plan-D-ish `nsmbtinycore` process-list selected-range path after the exact rollback lower-bound work. The useful direction is now `nsmbtinycore` + process-list ranges + delta-discovered ranges + `playerObjectLength=0xC80` + `RollbackSkipJitReset=1`, not `tinycoreramdelta` foreground exact restore.
  - Added practical-suite candidates around that path, especially `nsmbtc-rb1000-mr2-limit1`: `InputDelayFrames=2`, `InputMaxFrameLead=2`, `RollbackInputWaitUs=1000`, `RollbackMaxResimFrames=2`, `MELONDS_NSML_ROLLBACK_MAX_CORRECTIONS_PER_FRAME=1`, `RollbackCheckpointInterval=1`, `RollbackTinyCoreFlags=0x241`.
  - This candidate passed normal/no-artificial `chaos` at `17.072/17.080ms`, with zero rollback-derived spikes/restores in `logs/codex-goal-resume-nsmbtinycore-limit1/20260623-111627`. The remaining `49/46ms` max frames were classified as non-rollback spikes.
  - The same candidate passed normal `contact`, `death`, and `dualstresslong` in `logs/codex-goal-resume-nsmbtinycore-limit1-routes/20260623-112604`: averages `16.704-16.751ms`, no persistent mismatch, no rollback-derived spike above the 40ms gate. `dualstresslong` still reported two transient fields (`movingHazardX;playerActor1X`) that settled.
  - Under artificial `InputSendJitterFrames=2`, the candidate stayed mismatch-free and averaged `16.713/16.712ms`, but still had a client rollback spike at `41.647ms` in `logs/codex-goal-resume-nsmbtinycore-limit1-jitter/20260623-113032`. Under fixed `InputSendDelayFrames=2`, it averaged `17.148/17.148ms` with one client rollback max `40.587ms` in `logs/codex-goal-resume-nsmbtinycore-limit1-delay2/20260623-112108`. Fixed `InputSendDelayFrames=3` is still too much and fails at `19.088/19.092ms` with many rollback spikes.
  - Cost split is now much better than the exact foreground oracle: selected snapshot save avg is about `0.15-0.18ms`, restore about `3.3-3.7ms`, and one resimulated `RunFrame` about `12-15ms`. The remaining single-correction spike is the visible-frame placement cost, not checkpoint bytes.
  - Rejected in this pass: `rbwait500` had scheduler-scale spikes; `maxresim1` caused actor position mismatch; `skipresimcp` caused mismatch; `skipmidcp`, `rbwait1500`, and `RESIM_CONSUME_CURRENT_FRAME` did not improve the spike profile; a temporary SPU output-discard hook worsened timing and was removed.
  - Current status: this is the strongest path seen so far for actual GUI-standard delay2 rollback because it keeps normal routes near 60fps and state-aligned while avoiding the exact path's 18-19ms steady cost. It is still not goal-complete: forced delay/jitter can still produce ~41ms rollback spikes, and constant 3F send delay causes rollback storms.
  - Next actions: promote only as an experimental candidate, not as final. Continue with selected snapshot/process-list lifecycle correctness, plus frame-placement/coalescing that avoids more than one correction per visible frame without hiding failure by raising InputDelay.
  - Log storage fix: `run-nsmb-mvl-lan-route-smoke.ps1` and `run-nsmb-mvl-local-triage.ps1` now store per-run host/client ROMs as hardlinks by default, matching `codex/rule-based-mvl-ai`. Use `-CopyRomToLog` only when a run really needs standalone ROM copies in the log directory.
- 2026-06-23 current rollback goal continuation after the main merge:
  - Added lower-overhead validation knobs for the practical suite: bounded FPS/spike trace output, rollback stats summary-only mode, `dualstressmid`, persistent StateSync mismatch gating before perf gating, and final active-frame fields including `maxConsecutiveSlowFrames`, rollback spike count, and rollback max frame time. This catches the "average looks OK but a visible correction frame tanks" failure mode.
  - Rechecked full/exact correctness oracles under `InputDelayFrames=2`, `InputSendDelayFrames=2`, and `InputSendJitterFrames=2`. `coredelta` and `coreframedelta maxresim8` can pass StateSync, but still average about `20-21ms` with `200ms+` correction spikes. The old fast selected-range `nsmbtinycore` paths can be near `16.8ms`, but repeat runs still show ARM9 aborts or `playerGlobal`/actor mismatch, so they are not safe.
  - Retested `tinycoreramdelta` tiny-core flags under repeated `dualstressmid` runs. The earlier `wifi`-only pass was not stable enough to be the correctness oracle. `devices` still mismatched; adding full GPU3D state (`RollbackTinyCoreFlags=0x37D`) was the lightest repeatable exact-ish boundary found in this pass. It passed at about `21.7ms` average with save avg `~4.3ms`, restore avg `~24.9ms`, and resim total avg `~135-142ms`. This is correct-ish but not playable.
  - Returned to the selected `nsmbtinycore` line with the new GPU3D finding. `nsmbtc-rb1000-mr2-limit1-arena-stack-heap60-gpu3d` (`tinyFlags=0x37D`, actor arena + ARM9 stack + process list + heap scan interval 60) can pass some `dualstressmid` delay2/sendDelay2/jitter2 runs at about `20.4-21.6ms`, with save avg `~3.0-3.3ms`, restore avg `~5.3-5.7ms`, and resim total avg `~27-29ms`. It is much lighter than exact Main RAM restore but still not 60fps-stable and remains seed-sensitive.
  - Added `MELONDS_NSML_ROLLBACK_NSMB_ACTOR_ARENA_LENGTH` so the selected actor arena can cover late active-object bases seen around `0x021C1F18` without per-frame heap scanning. `widearena10000` passed one `dualstressmid` retake but averaged `22.0ms`; `widearena17000` was slower and not stable in a later seed. Heap scan interval 1 also passed but averaged `22.4ms`, so the current best selected tradeoff remains heap60/GPU3D, not a promoted solution.
  - Rejected follow-ups in this pass: `maxresim6` for exact is correctness-unsafe; full exact GPU/device flags are too slow; `InputMaxFrameLead=2` lowers exact correction count but shifts cost into steady waits; selected `cp2` lowers average to `~18ms` but mismatches around frame `1020`; selected heap1 and widearena are correctness aids but worsen average FPS.
  - Current interpretation: the active goal is blocked by two different walls. Exact-ish rollback can be made correct only around `21ms+` or with large correction spikes. Selected snapshots can make restore/resim cheap enough, but correctness depends on actor/process lifecycle coverage and still averages `20ms+` once GPU3D state is included. Do not raise `InputDelayFrames` above `2`; continue with selected lifecycle correctness and/or a non-blocking/shadow correction architecture.
- 2026-06-24 GPU3D-light selected snapshot retake:
  - Restored the lost practical-suite candidate additions after the user's stash accident and added `nsmbtc` candidates that use `tinyFlags=0x27D` (`devices + GPU3DLight`, no full VertexRAM/PolygonRAM) plus actor arena, ARM9 stack, process-list ranges, delta-discovered ranges, and `playerObjectLength=0xC80`.
  - No-artificial-delay route check: `nsmbtc-rb1000-mr2-limit1-arena-stack-devices-gpu3dlight` passed `stocktouch`, `contact`, `chaos`, and `dualstresslong` at about `16.8-16.9ms` average, with max frames around `41-44ms`. The `death` route failed for the same reason as full GPU3D and exact candidates: both peers did not hit the route's required player-1 death check, so this failure is treated as route-script coverage, not a GPU3DLight-specific desync.
  - This is the first measured selected candidate where GPU3DLight is enough for several complex no-artificial routes while avoiding the full GPU3D `20ms+` steady cost. It does not prove full correctness; it only narrows the next useful line.
  - Under stronger artificial `InputSendDelayFrames=4` / `InputSendJitterFrames=4`, the old `InputMaxFrameLead=2` profile remains unplayable because frame-lead throttle dominates visible frames (`PacketBridgeScratchSpike throttleMs` commonly around `90-120ms`). This is not a checkpoint-byte problem.
  - Raising only `InputMaxFrameLead` is also not a clean fix. `lead999` with `rbwait1000/mr2` caused player-position mismatches; `rb0/mr8/lead999` stayed mismatch-free on contact/chaos but still had rollback-frame max around `120-148ms`. `mr4` lowered max to roughly `85-91ms` but mismatched contact; `mr6` still spiked over `110ms` and produced an ARM9 prefetch abort in chaos.
  - Existing knobs `MELONDS_NSML_ROLLBACK_SKIP_PREDICTED_FRAME_LEAD_THROTTLE` and `MELONDS_NSML_ROLLBACK_THROTTLE_APPLIED_FRAME_LEAD` did not promote. They reduced some waiting shape but created stalls or average frame times around `33ms`, so they are rejected for this path.
  - `MELONDS_NSML_ROLLBACK_RESIM_SKIP_INTERMEDIATE_CHECKPOINTS=1` did not improve the `rb0/mr8/lead999/limit1` spike profile. The remaining large frames are mostly foreground resimulation work, not intermediate checkpoint saving.
  - Selected-backend post-frame and delayed-resim experiments were also rejected. `rb0/mr8/lead999/limit1 + RESIM_POST_FRAME` worsened strong-jitter chaos badly, including one side around `35ms` average, and `RollbackResimulateDelayFrames=2` kept the same `130-156ms` class max rollback frames.
  - Additional follow-up rejected narrow resim-side and input-bundle shortcuts. Skipping only `SPU.BufferAudio()` during resim did not reduce spikes. Held future-input bundles are unsafe when treated as confirmed input: they looked closer under strong jitter but caused no-artificial `chaos` mismatch. A speculative future-bundle variant still stalled/spiked. `mr3` capped rollback is correctness-unsafe, while `mr4 + future4-held` is near the strong-jitter spike gate but also breaks normal routes, so it is not promotable. `RESIM_CONSUME_CURRENT_FRAME` also failed to improve the GPU3DLight selected path.
  - Current interpretation: GPU3DLight selected snapshots are promising for local/no-artificial delay2 play, but the strong-jitter wall is now frame-lead policy plus foreground resimulation placement. The next useful work is not more range widening; it is either a rollback-aware pacing policy that avoids long throttle waits without letting prediction run too far, or a non-blocking/shadow correction architecture.
- 2026-06-23 follow-up after the JIT-boundary commit:
  - Added fast-lookup usage tracking variants in the ARM JIT: used-span, used-chunk, and compiled-chunk reset. Used-span/used-chunk are rejected for now because lookup-hit tracking raises average frame time; compiled-chunk tracking is useful as a lighter correctness aid.
  - Added restore phase timing (`restoreCoreMaxUs`, `restoreCopyMaxUs`, `restoreJITMaxUs`) to the FPS spike trace. This confirmed the exact-ish delay2 spike was dominated by restored-range JIT invalidation, not checkpoint copy: one representative run showed core restore about `3.3ms`, Main RAM copy about `0.25ms`, and JIT invalidation about `72ms`.
  - Added `MELONDS_NSML_ROLLBACK_SKIP_RESTORED_JIT_INVALIDATION` as a diagnostic/promotion candidate for `tinycoreramdelta` when paired with `MELONDS_NSML_ROLLBACK_JIT_LOOKUP_RESET_ONLY=1` and compiled-chunk lookup reset. This keeps the tiny-core restore's lookup reset but skips the broad post-copy MainRAM/WRAM/ITCM invalidation pass.
  - New best exact-ish candidate: `jitlookup-compiledchunks-skiprestorejit-cp1-maxlead0-leadbudget2ms`. It passed no-artificial-delay chaos (`17.641/17.640ms`, max `41.451/40.562`) in `logs/codex-goal-skiprestorejit-no-artificial/20260623-083040` and stayed mismatch-free under delay2/jitter2 (`18.762/18.764ms`, max `43.823/54.127`, over33 `4/4`) in `logs/codex-goal-skiprestorejit-delay2/20260623-083205`.
  - This is a real improvement over the previous `100ms+` correction spikes, but it is not complete. With the JIT invalidation removed from the hot restore path, the remaining spikes are now mostly the unavoidable same-frame correction shape: restore `~3-7ms`, one resimulated `RunFrame` around `12-24ms`, checkpoint save around `1-2ms`, plus the normal visible frame.
  - Rejected follow-ups from this pass: code-chunk invalidation did not help this backend; 4ms/6ms frame-lead budgets reduce some correction clustering but worsen average FPS; `InputMaxFrameLead=2` restores average but causes many more corrections/over33 frames; `RollbackMaxCorrectionsPerFrame=1` and `RESIM_CONSUME_CURRENT_FRAME` both worsened the spike profile.
  - Current blocker: exact-ish state alignment is now practical enough for local/no-artificial and no longer has the old JIT invalidation cliff, but delay2 jitter still produces visible `33-55ms` correction frames. The next useful work is frame placement/coalescing or a shadow-state architecture, not more checkpoint byte slimming.
- 2026-06-23 continuation after skip-restored-JIT validation:
  - Rebuilt melonDS and tested additional `jitlookup-compiledchunks-skiprestorejit` variants under `InputDelayFrames=2`, `InputSendDelayFrames=2`, and `InputSendJitterFrames=2`.
  - Rejected `cp2`, `RESIM_SKIP_FINAL_CHECKPOINT`, and `PRE_PUMP_BEFORE_RESIM`: all stayed mismatch-free in some runs but worsened average timing or over33 spike counts compared with `cp1`.
  - `RollbackInputWaitUs=500` reduced correction spikes enough for the old suite gate (`19.555/19.553ms`, max `46.369/50.638`, over33 `1/3`) in `logs/codex-goal-skiprestorejit-rbwait500-delay2/20260623-085955`, but it is not promotable because the average frame time is about 19.5ms. `RollbackInputWaitUs=1000` also failed spikes (`logs/codex-goal-skiprestorejit-rbwait1000-delay2/20260623-090139`).
  - Future-input bundling is rejected for now. The sender no longer fills missing future bundle entries with the current input, because that created false "confirmed" future inputs on complex movement and increased rollback corrections. After the safety fix, `future4` still did not improve the candidate (`logs/codex-goal-safe-future4-delay2/20260623-090444`).
  - Practical rollback suite now has `-MaxAverageFrameMs` so a run with low over33 counts but 19ms-class average cannot be marked as a performance pass for the active goal.
  - Added a no-rollback practical-suite candidate for baseline timing. `norollback-delay4-lead4-bundle8` on the same chaos route with sendDelay2/jitter2 averaged `16.688/16.692ms` with max `39.366/39.851` and over33 `7/5` in `logs/codex-goal-norollback-baseline-delay2jitter2/20260623-091310`. This shows there is a small normal execution/input-wait spike floor even without rollback.
  - Current blocker remains structural: after removing the restored-range JIT invalidation cliff, exact rollback still pays restore plus a resimulated `RunFrame` inside the displayed frame, on top of the normal 33-40ms spike floor. Small waits can trade rollback spikes for worse average FPS, but they do not reach stable 60fps.
- 2026-06-23 stats/spike-classification continuation:
  - Practical rollback suite now records `NSMB PerfSpike` as rollback vs non-rollback spikes, `PacketBridgeScratchSpike` maxima, and low-cost rollback stats via `MELONDS_NSML_ROLLBACK_STATS_TRACE`. The smoke wrapper exposes `-RollbackStatsTrace`, and the suite passes through `-MaxRollbackFrameMs`.
  - Rebuilt melonDS and retested chaos with `InputDelayFrames=2`, `InputSendDelayFrames=2`, `InputSendJitterFrames=2`.
  - No-rollback baseline `norollback-delay4-lead4-bundle8` passed at `16.699/16.695ms`, max `38.254/50.055`, rollback spikes `0/0` in `logs/codex-goal-stats-rollback/20260623-093149`. This confirms the harness has a small non-rollback spike floor.
  - Current exact-ish cp1 oracle `jitlookup-compiledchunks-skiprestorejit-cp1-maxlead0-leadbudget2ms` stayed mismatch-free but failed performance at `19.330/19.393ms`, max `61.585/49.396`, rollback spikes `15/9`. Stats show checkpoint save avg only `~1.2-1.3ms`, restore avg `~5.5ms`, and resim total avg `~20ms`, so checkpoint bytes are not the dominant cost.
  - `cp2` and adaptive cp2 remain rejected. Constant cp2 lowers average only slightly but increases older correction cost; adaptive cp2 raised average to `19.616/19.699ms`.
  - Post-frame resim placement (`MELONDS_NSML_ROLLBACK_RESIM_POST_FRAME`) was implemented as an experiment but rejected for promotion: it stayed mismatch-free yet averaged `19.841/19.914ms`. The frame-limit wait did not hide enough correction work.
  - Current best smoothness tradeoff in this pass is `jitlookup-compiledchunks-skiprestorejit-rbwait1000-cp1-maxlead0-leadbudget2ms`: `18.817/18.818ms`, max `44.157/41.895`, rollback spikes `0/1`, restores `0/2` in `logs/codex-goal-rbwait-stats/20260623-094602`. It is much smoother but still about 53fps-class and therefore not complete.
  - Rejected in this pass: `consume-current-frame`, `maxLead2`, `maxLead2+rbwait1000`, leadbudget `4/6ms`, no lead limit, `rbwait750`, and `rbwait1000` with `MELONDS_NSML_INPUT_WAIT_POLL_US=50`. They either increased correction frequency, increased remote waits, or worsened average FPS. The poll50 check failed at `20.152/20.155ms` in `logs/codex-goal-rbwait-poll50-stats/20260623-100803`.
  - Follow-up throttle/checkpoint sweep:
    `MELONDS_NSML_ROLLBACK_THROTTLE_APPLIED_FRAME_LEAD=1` was rejected because it reduced neither waits nor corrections enough (`19.681/19.760ms`; with `rbwait1000`, `19.061/19.061ms`) in `logs/codex-goal-appliedlead-stats/20260623-101336`.
    `InputMaxFrameLead=1` was also not a useful middle point: `18.909/18.838ms`, and `rbwait1000+maxLead1` stayed around `18.823/18.828ms` in `logs/codex-goal-maxlead1-stats/20260623-101745`.
    `rbwait1000+cp2` is the best average found in this continuation (`18.430/18.369ms`) but still has rollback spikes up to `54.964/59.310ms`; `rbwait1000+cp3` lowers average to `18.209/18.186ms` but spikes worsen to `78.756/79.050ms`. This confirms the checkpoint-interval tradeoff: less saving helps average a little, but older correction restore/resim is not playable.
    `rbwait1000+cp2` also stayed mismatch-free on `contact` and `death`, but still failed performance (`contact` `18.413/18.467ms`, max `47.700/45.611`; `death` `18.062/18.134ms`, max `41.640/41.590`) in `logs/codex-goal-rbwait-cp2-routes/20260623-102603`.
  - `rbwait1000+cp2+resimDelay2` is the closest exact-ish foreground candidate so far on spike count: chaos `18.097/18.098ms`, max `41.352/44.726`, rollback spikes `2/3`, no transient mismatch in `logs/codex-goal-rbwait-cp2-resimdelay-stats/20260623-103038`. It also stayed mismatch-free on contact/death (`logs/codex-goal-rbwait-cp2-rd2-routes/20260623-103338`) but still averaged `17.918/17.916ms` and `18.205/18.103ms`.
  - `rbwait500+cp2+resimDelay2` lowers chaos average to `18.000/17.998ms` but raises spikes (`47.866/44.863`) and contact still hits `55.821/53.607ms`. `rbwait250` and `maxLead1` variants are rejected: they increase correction count and spike severity. The current exact foreground path is therefore close to a diagnostic lower bound but still not a 60fps/no-spike solution.
  - Current blocker is now clearer: the exact path can be state-correct under this chaos run, but playable delay2 rollback still needs either fewer foreground corrections or a non-blocking/shadow correction architecture. Further checkpoint slimming alone is unlikely to reach 60fps.
- 2026-06-23 latest continuation after rebuilding melonDS:
  - Fixed a test-wrapper trap: `run-nsmb-mvl-split-local-input-smoke.ps1` no longer silently forces `MELONDS_NSML_ROLLBACK_SKIP_JIT_RESET=1` for `nsmbtinycore` / `tinycorepreimage`. That forced skip made some "force JIT" candidates look lighter but correctness-unsafe.
  - New correctness-oriented exact-ish candidate family is `tinycoreramdelta` page256/cp1/maxResim1 with selective JIT lookup/reset variants. `jitlookup-cp1-maxlead0-leadbudget2ms` passed `chaos` with no artificial send delay (`17.743/17.743ms`, max `52.965/41.046`, over33 `2/1`) in `logs/codex-goal-jitlookup-no-artificial-delay/20260623-074901`.
  - The same exact-ish path is still not playable once rollback corrections are forced by artificial latency. `jitlookup-cp1-maxlead0-leadbudget2ms` failed `InputSendDelayFrames=1` / `InputSendJitterFrames=1` (`18.076/18.091ms`, max `105.997/105.396`) and delay2/jitter2 (`18.420/18.481ms`, max `134.510/91.339`). It stayed mismatch-free in these runs, but visible spikes remain.
  - Fast-but-risky JIT reuse remains rejected. `skipjit-rbwait500-cp1-maxlead0-leadbudget2ms` reduced over33 to `7/7` and max to `73.574/79.162`, but mismatched `playerActor0X` at frame `990`. Adding local fast-lookup invalidation moved the mismatch later (`playerActor0X` at frame `2670`) but did not eliminate it.
  - Restore-candidate fast reset (`jitrc-dirty-cp1-maxlead0-leadbudget2ms`) is close but not promotable: delay2/jitter2 stayed around `18.668/18.657ms` with max `98.695/95.628`, and no-artificial-delay chaos still mismatched `playerActor1X` at frame `990`.
  - Rejected follow-ups: rollback same-frame input waits of 2-4ms worsened averages, `RESIM_SKIP_FINAL_CHECKPOINT` caused mismatch or worse spikes, `InputMaxFrameLead=0/1` reduces correction count but cannot remove correction spikes, `RESIM_CONSUME_CURRENT_FRAME` worsened spikes because it turns the correction into two resimulated frames, and 100us frame-lead sleep did not improve the spike profile.
  - Current blocker: checkpoint size is not the limiting factor. With exact-ish state, any real correction still performs restore/JIT lookup-reset plus one or more cold-ish `RunFrame` re-executions inside the visible frame. The newer skip-restored-JIT path reduces this from `100ms+` spikes to roughly `33-55ms` class correction frames, but still misses stable 60fps under delay2 jitter.
  - Next actions: keep `jitlookup-compiledchunks-skiprestorejit-cp1-maxlead0-leadbudget2ms` as the current exact-ish correctness oracle, but do not promote it for jitter. Continue toward either a shadow/non-blocking correction architecture or a pointer-safe selected lifecycle snapshot that can be validated against this oracle.
- Previous 2026-06-23 local/no-artificial selected-range candidate: `nsmbtinycore-delay2-maxlead2-proclist-deltapages-forcejit-noheap-playerc80-maxresim2-skiprender`.
  - Method: NSMB selected process-list/global snapshot, delta-discovered static ranges, tiny core state, player object length `0xC80`, full conservative restored-range JIT invalidation, `RollbackMaxResimFrames=2`, GUI-standard `InputDelayFrames=2` / `InputMaxFrameLead=2`.
  - GUI-standard no-artificial-delay verification passed `contact`, `chaos`, `death`, and `secondgame`. Timing: contact `16.695/16.703ms`, max `41.179/44.838`, over33 `3/4`; chaos `16.684/16.682ms`, max `39.462/40.730`, over33 `2/3`, with one transient `playerActor1X` sample settled within 30F; death `16.721/16.734ms`, max `37.789/38.961`, over33 `3/3`; secondgame `16.774/16.769ms`, max `57.933/58.551`, over33 `4/5`.
  - `secondgame` harness failure is resolved. The cause was `generate-nsmb-mvl-stable-roms.ps1` treating normal `cargo` stderr output as a terminating PowerShell error while regenerating the MvL ROM pair for `MvlLives=3`.
  - `maxresim2` is stronger than `maxresim1`: `maxresim1` could pass normal routes but FPS-trace overhead exposed a `secondgame` player-position mismatch; `maxresim2` passed the same FPS-trace `secondgame` run.
  - Artificial `InputSendDelayFrames=2` / `InputSendJitterFrames=2` with GUI-standard `InputMaxFrameLead=2` is still not playable: it has no persistent mismatch in the throttled path, but fails smoothness because `PacketBridgeScratchSpike` / input frame-lead throttling waits dominate. An experimental `MELONDS_NSML_ROLLBACK_SKIP_PREDICTED_FRAME_LEAD_THROTTLE=1` removed those waits and brought averages back near `17ms`, but caused early mismatch or heavy rollback correction spikes; it is env-gated and not promotable.
  - Heap-scan expansion (`heap60` / `heap15`) did not solve the traced drift and slightly increases checkpoint cost, so the current baseline remains process-list plus delta-discovered ranges without heap scan.
  - Current interpretation: selected NSMB snapshot plus full restored-range JIT invalidation is now good for local/no-artificial-delay GUI-standard play, but WAN/jitter playability is still blocked by the tradeoff between MaxLead throttling and frequent same-frame rollback correction. Do not increase InputDelay above `2`; do not promote prediction-throttle skip or correction-limit shortcuts.
  - Next actions: keep `maxresim2` as the local baseline, then investigate a rollback-aware frame pacing/lead policy that avoids long in-frame waits without triggering correction storms. Re-run manual GUI and longer complex-input routes after that, including `secondgame`, contact-heavy movement, death/respawn, and star loss.
- Latest exact-ish delay2 retake: `skipjit-page512-cp1` is currently the closest measured candidate. It passed contact and stayed mismatch-free on chaos under `InputSendDelayFrames=6` / `InputSendJitterFrames=6`, but chaos still failed smoothness (`18.156/18.315ms` avg, max `101.526/91.335`, over33 `61/52`) in `logs/codex-goal-delay2-codechunk-contact-chaos/20260623-002325`.
- Follow-up tuning did not promote it. `skipjit-page512-cp1-rbwait1000` reduced chaos max to `92.642/88.892ms` but mismatched contact at frame `2010`; `rbwait500` and correction coalescing still failed spike or correctness gates in `logs/codex-goal-delay2-page512-tune-contact-chaos/20260623-003124`.
- Optimistic future-input bundling (`MELONDS_NSML_INPUT_BUNDLE_FUTURE=4`) was tested without increasing local input delay, but it worsened correction/spike pressure on complex input and is rejected for now.
- Current interpretation: checkpoint size is no longer the main bottleneck. The remaining exact-ish path spends `45-60ms` class time in rollback correction inside the visible frame, so a new direction should avoid same-frame cold-JIT re-execution or build a pointer-safe selected lifecycle snapshot validated against the exact oracle.
...
- Latest delay-2 Plan-D-style snapshot/repair experiments:
  ...
  - Retake under the hard delay2 rule confirms the split: Plan-D-style predict/repair is fast enough but not correct enough. `predictrepair-delay2-player-world-actorsnap32-lifecycle-prune-transition90-pred0` passed `dualstresslong` at `16.899/16.901ms` avg but failed `contact` (`movingHazardX` at frame `990`) and `chaos` (persistent `playerGlobal`) in `logs/codex-goal-delay2-predictrepair-retake/20260622-204429`.
  - Hazard lifecycle variants did not solve the root issue. Combining moving-hazard spawn and link clear kept `dualstresslong` fast (`16.768/16.769ms`, max `42.998/30.520`) but still mismatched `contact` (`movingHazardX` at frame `990`) and caused a `chaos` spike cluster around remote hazard spawn (`~100-130ms`) in `logs/codex-goal-delay2-predictrepair-spawnclear/20260622-205934`.
  - Fresh-sample waiting is rejected: `predictrepair-delay2-player-world-actorsnap32-lifecycle-prune-transition90-fresh-pred0` still failed `contact`/`chaos` and stalled `dualstresslong` in `logs/codex-goal-delay2-predictrepair-fresh/20260622-211049`. This behaves like adding hidden wait, not like a robust rollback fix.
  - Current Plan-D interpretation: selected actor/global repair must become a real active-object lifecycle model, not a list of per-symptom patches. The latest `chaos` failures include host-only `0x10C` and moving-hazard process-list/lifecycle drift, so moving hazard/star fixes alone cannot make this exact.
  - 2026-06-22 follow-up: star candidate trampoline spawn is rejected. `predictrepair-delay2-player-world-actorsnap32-lifecycle-prune-transition90-starsnapspawn-pred0` produced `~100ms` spike clusters and stall in `logs/codex-goal-delay2-starsnapspawn/20260622-212815`.
  - Dormant-slot activation is lighter than trampoline spawn but not sufficient. `predictrepair-delay2-player-world-actorsnap32-lifecycle-prune-transition90-staractivate-pred0` avoided the huge spawn stall, but still failed `contact`/`chaos`; adding moving-hazard dormant activation made `dualstresslong` pass once (`logs/codex-goal-delay2-activate/20260622-214613`) while `chaos` still failed when a host-side `0x53` moving hazard was absent on client.
  - Last-known moving-hazard base restore fixed the early `movingHazardX` 2F drift in some runs and kept timing practical (`~16.8ms` avg, max `~47-51ms`), but it is not robust: chaos still fails when the hazard drops out of the client process list near frame `3900` (`logs/codex-goal-delay2-restorebase-immediate/20260622-220623`, `logs/codex-goal-delay2-restorebase-bestbase/20260622-220852`, `logs/codex-goal-delay2-restorebase-hazardpred0/20260622-221100`). This points to missing process-list/node lifecycle, not just transform fields.
  - Exact rollback retake confirms the other side of the tradeoff: `exact-delay2-tinycoreramdelta-key2-page256-cp2-maxresim1-skiprender` stayed mismatch-free on chaos but failed smoothness (`18.214/18.239ms` avg, max `112.616/122.464`, over33 `68/63`) in `logs/codex-goal-delay2-exact-probe/20260622-215224`. Fast JIT reset did not solve this (`logs/codex-goal-delay2-exact-fastjit-probe/20260622-215407`, max `~105ms`).
- Latest delay-2 exact rollback rechecks under strong jitter:
  ...
  - 2026-06-22 retake: `tinycoreramdelta` page256/cp1/maxResim1/skipJIT with conservative 16-byte restored-range JIT invalidation stayed mismatch-free on contact/chaos, but still failed smoothness (`logs/rb-jitinv-instrument-baseline/20260622-194454`: contact `18.118/18.068ms`, max `99.846/106.378`; chaos `18.840/18.922ms`, max `93.853/107.992`).
  - Added `jitInv*` rollback trace fields. The trace run showed why `tinycoreramdelta` spikes: restore chains often include a 4MB Main RAM keyframe, and JIT invalidation alone can take about `61-69ms` on the first full restore (`logs/rb-jitinv-trace-baseline/20260622-194833`).
  - Rechecked `tinycorepreimage` under the same delay2/send6/jitter6 condition. `maxResim2` is fast-ish but incorrect on the current branch (`contact` mismatched `playerActor0X` at frame `2010`; `chaos` mismatched `playerActor1X` at frame `1050`). `maxResim3` fixed contact and stayed mismatch-free on chaos/dualstresslong, but still failed smoothness: contact passed at `17.080/16.900ms`, max `70.610/62.507`; chaos failed at `18.485/18.459ms`, max `107.706/132.871`; dualstresslong failed at `19.517/19.851ms`, max `103.049/106.487`.
  - `tinycorepreimage maxResim3` trace confirms the checkpoint side is already light: restore is roughly `3-6ms`, checkpoint save around `1.5-8ms`, while three resimulated `RunFrame`s are commonly `40-70ms` total. This means the remaining exact rollback problem is same-frame re-execution cost, not snapshot byte size.
  - Rejected follow-ups: `rbwait250/500` with maxResim3 worsened averages, `prepump` with maxResim3 caused mismatch, `skipmidcp` with maxResim3 caused contact mismatch, and an attempted preimage prune boundary cut broke correctness. Those paths are not promotion candidates.
- Latest delay-2 NSMB process-list range snapshot experiments:
  - `nsmbtinycore-delay2-proclist-arena-stack-noheap-maxresim2-skiprender` is not promotable. It can keep checkpoint save around `0.14-0.19ms` and restore around `3ms`, but selected range restore is unsafe: trace runs showed host `RunFrame` spikes around `90-102ms`, a client ARM9 prefetch abort at frame `1054`, and practical chaos at `92/66ms` average in `logs/codex-goal-nsmbtinycore-delay2-arena-chaos-send6-jitter6-20260622/20260622-112137`.
  - Range isolation showed `ARM9 stack` and raw `actorArena` broad copies can create pathological resim cost. The most promising lightweight base is process-list object ranges without heap scan or actor arena.
  - Added `MELONDS_NSML_ROLLBACK_NSMB_PLAYER_OBJECT_LENGTH` to tune player actor snapshot length. With `player=0xC80`, `nsmbtinycore-delay2-proclist-noheap-playerc80-maxresim2-skiprender` passed a 1500F strong-jitter chaos trace with `16.667ms` average, max `22.634/26.959ms`, `over33=0`, and resim totals up to about `26ms` in `logs/codex-goal-nsmbtinycore-playerlen-sweep-1500-jitter6-20260622/playerC80`.
  - The same candidate is still not a solution. A practical suite run with strong jitter passed `dualstresslong` (`16.666/16.666ms`, max `25.514/29.069ms`) but `contact` mismatched at frame `990` (`playerActor0Y`) and `chaos` stalled in `logs/codex-goal-nsmbtinycore-proclist-playerc80-suite-send6-jitter6-20260622/20260622-120710`.
  - Added `MELONDS_NSML_ROLLBACK_NSMB_MOVING_HAZARD_OBJECT_LENGTH` for diagnosis, but broad moving-hazard tail copying is rejected: `hazard=0x500` made contact average `59/94ms` with max `129/154ms` in `logs/codex-goal-nsmbtinycore-proclist-playerc80-hazard500-contact-send6-jitter6-20260622/20260622-121436`.
- Latest delay-2 exact-ish Main RAM delta experiment:
  - Added experimental backend `tinycoreramdelta`: `DoRollbackTinyCoreSavestate` for CPU/timer/core state plus full/delta Main RAM page ranges. This is a new path between full `coredelta` correctness and NSMB selected-range lightness.
  - `arm9ram` showed that Main RAM alone is not sufficient: it kept contact timing near `16.7ms` but mismatched moving-hazard X at frame `990` in `logs/codex-goal-arm9ram-contact-send6-jitter6-20260622/20260622-124807`.
  - `coredelta` / `coreframedelta` remain correctness oracles but not playable under delay2 strong jitter: contact had no mismatch, but max spikes were `~217-248ms` in `logs/codex-goal-coredelta-contact-fieldcompare-send6-jitter6-20260622/20260622-124933` and `logs/codex-goal-coreframedelta-contact-send6-jitter6-20260622/20260622-125108`.
  - `tinycoreramdelta` with keyframe interval `2`, page `256`, and `RollbackMaxResimFrames=1` is the current best correctness-oriented delay2 candidate: contact/dualstresslong/chaos produced no persistent mismatch under `InputSendDelayFrames=6` / `InputSendJitterFrames=6`, but still failed the spike gate. Contact: `17.407/17.360ms`, max `93.251/92.990`, over33 `17/14` in `logs/codex-goal-tinycoreramdelta-key2-cp2-maxresim1-contact-send6-jitter6-20260622/20260622-132043`. Dualstresslong/chaos: no mismatch, but averages `18.1-18.3ms`, max up to `163ms`, over33 `78-167` in `logs/codex-goal-tinycoreramdelta-key2-cp2-maxresim1-dual-chaos-send6-jitter6-20260622/20260622-132339`.
  - `tinycoreramdelta` must not skip JIT reset when restoring Main RAM. Enabling `RollbackSkipJitReset` made contact fast (`~51ms` max) but caused persistent actor position mismatch in `logs/codex-goal-tinycoreramdelta-key2-maxresim1-skipjit-contact-send6-jitter6-20260622/20260622-131518`.
  - Rechecked the JIT-reset cost boundary under the hard delay2 rule:
    - Adding forced JIT invalidation for restored Main RAM plus tiny-core Shared WRAM/ARM7 WRAM/ITCM did not make `RollbackSkipJitReset` safe. `exact-delay2-tinycoreramdelta-key2-page256-cp2-maxresim1-skipjit-skiprender` still mismatched `playerActor0X` at frame `2010` in `logs/nsmb-mvl-rollback-jit-forceinv16-contact/20260622-134436`, though it kept averages near `16.9-17.0ms`.
    - `MELONDS_NSML_ROLLBACK_JIT_MEMORY_RESET_ONLY=1` also mismatched `playerActor0X` at frame `2010` in `logs/nsmb-mvl-rollback-jit-memreset16-contact/20260622-134604`.
    - A new experimental `MELONDS_NSML_ROLLBACK_JIT_FAST_RESET=1` path detaches JIT blocks immediately and defers metadata deletion. It preserved contact correctness but was slower than normal full reset: default delete budget hit `19.223/19.235ms` average and `611ms` max in `logs/nsmb-mvl-rollback-fastjitreset-contact/20260622-135249`; delete budget `0` still hit `18.813/18.695ms` average and `139/142ms` max in `logs/nsmb-mvl-rollback-fastjitreset-nodelete-contact/20260622-135459`. This is diagnostic, not promotable.
    - Same-frame wait without raising InputDelay did not solve spikes: `RollbackInputWaitUs=500` on the best full-reset candidate kept correctness but failed at `17.520/17.486ms`, max `90/107ms` in `logs/nsmb-mvl-rollback-rbwait500-contact/20260622-134756`.
    - GUI-standard `InputMaxFrameLead=2` with the same delay2/tinycoreramdelta full-reset candidate stalled under strong jitter and averaged `33.393ms` on the active side in `logs/nsmb-mvl-rollback-lead2-contact/20260622-134142`.
  - New JIT-configuration boundary found and narrowed:
    - Added env overrides for NSML JIT args. They are useful for isolating rollback/JIT interactions, but skip-reset correctness is not solved.
    - `MELONDS_NSML_JIT_LITERAL_OPTIMIZATIONS=0` plus `MELONDS_NSML_JIT_FAST_MEMORY=0` initially looked promising, but repeat contact runs later reproduced persistent actor mismatch (`playerActor0X` at frame `2010`). It is now diagnostic-only, not a safe branch.
    - More conservative variants also failed contact after the suite env-leak fix: maxBlock8 with branch off, maxBlock4, maxBlock4 with branch off, and `conservativejit4` all mismatched in `logs/nsmb-mvl-rollback-jitmb4-conservative-contact-envfix/20260622-145515`.
    - Disabling JIT restore candidates did not fix skip-reset correctness: `exact-delay2-jitmb8-noliteral-nofastmem-norestorecand-tinycoreramdelta-key2-page256-cp2-maxresim1-skipjit-skiprender` still mismatched `playerActor0X` at frame `2010` in `logs/nsmb-mvl-rollback-skipjit-norestorecand-contact/20260622-151205`.
    - Increasing Main RAM delta page size is also not correctness-safe: page512 and page1024 variants produced contact mismatch, so page256 remains the safer exact-ish delta baseline.
  - Latest JIT reset experiments:
    - Added `MELONDS_NSML_ROLLBACK_JIT_FAST_RESET_INITIAL_DELETE_BUDGET` so rollback fast reset can defer the immediate deletion of detached JIT blocks. This makes deletion cost controllable independently from compile-time deferred deletion.
    - `exact-delay2-fastjitreset-nodelete0-tinycoreramdelta-key2-page256-cp2-maxresim1-skiprender` initially looked like a correctness-oriented baseline: contact/chaos/dualstresslong under `InputSendDelayFrames=6` / `InputSendJitterFrames=6` had no persistent mismatch, but still failed playability. Contact: `17.132/16.952ms`, max `106.596/101.831` in `logs/nsmb-mvl-rollback-fastjitreset-nodelete0-contact/20260622-150136`; chaos/dualstresslong: averages around `18.0-18.15ms`, max `90-100ms`, over33 up to `167/162` in `logs/nsmb-mvl-rollback-fastjitreset-nodelete0-chaos-dual/20260622-151614`. Later repeat tests made fast reset diagnostic-only; current correctness boundary is full `JIT.Reset()`.
    - Spike trace for the same candidate shows checkpoint save and restore are no longer the main cost. Restore max was about `8.7ms`, checkpoint save about `2-3ms`, while rollback/resim phases were `40-70ms` and resim total max around `65-72ms` in `logs/nsmb-mvl-rollback-fastjitreset-nodelete0-spiketrace-contact-v2/20260622-150545`.
    - Keeping JIT code memory while detaching block maps (`MELONDS_NSML_ROLLBACK_JIT_FAST_RESET_KEEP_CODEMEM=1`) is unsafe: it reduced max timing but produced contact mismatch at frame `2820` in `logs/nsmb-mvl-rollback-fastjitreset-keepcodemem-contact/20260622-151454`.
    - `RollbackInputWaitUs=1000/1500` combined with fast reset did not help. `1000us` worsened average timing to about `18.2ms`; `1500us` produced mismatch in `logs/nsmb-mvl-rollback-fastjitreset-nodelete0-rbwait-contact/20260622-150812`.
    - Rechecked the hard delay2 boundary after the user clarified that increasing GUI rollback `InputDelayFrames` from `2` is not acceptable. `InputDelayFrames=2` remains the required test setting; higher delay is not a fix path.
    - Disabling JIT for the whole run is a correctness diagnostic only: the same contact input passed with no persistent mismatch in `logs/nsmbrb-interp-contact/run1`, but it took about `228s` for 3600 frames and is far outside playability.
    - Normal full `JIT.Reset()` on restore is the current correctness boundary: `exact-delay2-tinycoreramdelta-key2-page256-cp2-maxresim1-skiprender` had no persistent contact mismatch in `logs/nsmbrb-normaljitreset-contact/20260622-155447`, but still failed performance (`17.759/17.752ms` average, max `128.823/127.090ms`).
    - Partial/region-masked fast reset is not stable enough to promote. `pv-main` mismatched contact; `pv-maincore`, `pv-mainshared`, and `pv-mainshared-arm7` avoided persistent mismatch in one contact sweep but still failed spike gates; repeat `pv-volatile` / `pv-volatile-j8nf` mismatched contact in `logs/nsmbrb-pv-mask-contact/20260622-154230`.
    - Correction coalescing without increasing InputDelay was tried with `RollbackResimulateDelayFrames=2` on the normal-reset baseline. It still failed the spike gate (`host maxFrameMs=114.364`, client `91.620`) in `logs/nsmbrb-normaljitreset-correctiondelay-contact/delay2`.
    - Current interpretation: the problem is primarily JIT restore/cache boundary, not missing Main RAM pages. Interpreter and full JIT reset preserve correctness; skip/partial reuse can be faster but produces actor position mismatches. The next viable direction is either pointer-safe rollback JIT reuse or a rollback architecture that avoids same-frame cold-JIT re-execution while keeping InputDelay fixed at 2.
    - Reliable input bundle follow-up: practical candidates were setting `InputBundleHistory=8`, but reliable sends only used bundle payloads when `InputUnreliable` was enabled. The sender now bundles reliable input history whenever `InputBundleHistory > 0`.
    - Reliable bundling plus cp1 improves some delay2 exact-ish results but does not solve the goal. Full `JIT.Reset()` with `exact-delay2-tinycoreramdelta-key2-page256-maxresim1-skiprender` passed contact under strong jitter (`17.179/17.026ms`, max `77.298/70.225`, over33 `13/5`) but chaos still failed (`18.161/18.148ms`, max `109.969/86.596`, over33 `67/72`) in `logs/nsmbrb-fullreset-cp1-bundle-chaos-contact-trace/20260622-162822`.
    - Skip/fast reset remain diagnostic-only after repeat tests. Plain skip-reset cp1 mismatched `playerActor1X` at frame `990` on chaos and `playerActor0X` at frame `2010` on contact; fast-reset cp1 also mismatched contact at frame `2010`. Coalescing with `RollbackResimulateDelayFrames=1` reduced max timing but mismatched immediately at frame `990`.
    - Added a diagnostic fast-reset lookup clear (`MELONDS_NSML_ROLLBACK_JIT_FAST_RESET_CLEAR_LOOKUP=1`) and a restore-candidate reuse path (`MELONDS_NSML_ROLLBACK_JIT_FAST_RESET_RESTORE_CANDIDATES=1`). Restore-candidate keys now include instruction hash, block address, and CPU number to avoid same-instruction collisions.
    - These JIT reuse paths are still not promotable. `jitrc-cp1` reduced some cold-resim timing, but repeat strong-jitter runs mismatched `playerActor0X` at frame `2010` on contact and frame `990` on chaos in `logs/rb-jitrc-keyed/20260622-170341`. Omitting lookup clear was faster but also mismatched in `logs/rb-jitrc-noclear/20260622-165619`, confirming stale fast-lookup state is part of the unsafe path. `KEEP_CODEMEM+CLEAR_LOOKUP` and partial-volatile clear candidates stalled before a valid correctness result.
    - The current correctness oracle was rechecked after these diagnostics: full `JIT.Reset()` with `exact-delay2-tinycoreramdelta-key2-page256-maxresim1-skiprender` still had no persistent mismatch under `InputSendDelayFrames=6` / `InputSendJitterFrames=6`, but remained a perf failure. Contact: `17.097/17.084ms`, max `87.370/95.125`, over33 `15/16`; chaos: `18.126/18.016ms`, max `90.768/89.482`, over33 `67/66` in `logs/rb-fullreset-oracle-recheck/20260622-171225`.
    - Strengthened rollback JIT restore-candidate validation with a compile fingerprint that includes decoded instruction metadata, branch flags, cycle/data-region assumptions, JIT options, `StartAddrLocal`, and CPU number. This reduced unsafe reuse, but full reuse still repeated `playerActor0X` mismatch in trace-free contact/chaos (`logs/rb-jitrc-localkey-notrace/20260622-175422`).
    - Added dirty/used lookup-clear candidates. They are not safe enough: `jitrc-dirty-cp1` and `jitrc-used-cp1` both reproduced contact mismatch in repeat runs. Full fast lookup clear remains required for now.
    - Added conservative JIT restore filters. `jitrc-nomem-cp1`, which refuses to reuse JIT blocks containing memory instructions, avoided mismatch in several delay2 strong-jitter runs but is not playable. A retake on contact/chaos/dualstresslong failed performance with averages `18.5-20.5ms`, max spikes `106-188ms`, and high over33 counts in `logs/codex-goal-delay2-jitrc-retake/20260622-202917`. `jitrc-straight-cp1` and `jitrc-nofastmem-cp1` also failed performance; `jitrc-straight-cp1` produced a `986ms` chaos spike.
    - Repeated `jitrc-nomem-cp1` on contact/chaos/dualstresslong. It stayed mismatch-free but not playable: repeat averages were `19-21.5ms`, max spikes `117-230ms`, and dualstresslong over33 reached `171/166` in `logs/rb-jitrc-nomem-repeat-dual/20260622-180927`.
    - Isolated JIT options under restore-candidate reuse. `jitrc-noliteral-cp1`, `jitrc-nofastmem-lit-cp1`, and `jitrc-nofastmem-cp1` all avoided persistent mismatch in contact/chaos, but still failed performance. Disabling fastmem/literal is diagnostic, not a promotion path (`logs/rb-jitrc-opt-isolation/20260622-182437`).
    - Added rollback JIT map/lookup reset experiments. `jitmap-cp1` and `jitlookup-cp1` kept JIT blocks alive and avoided persistent mismatch in initial contact/chaos sweeps, but clearing fastmem mappings or fast lookup tables still caused restore/resim spikes and did not beat the safer skip-reset path.
    - After changing skip-JIT restore invalidation to use 16-byte `CheckAndInvalidate` over restored Main RAM ranges, skip-reset became a viable candidate again. `exact-delay2-tinycoreramdelta-key2-page256-cp1-maxresim1-skipjit-skiprender` passed contact under delay2 strong jitter (`17.393/17.433ms`, max `83.223/81.561`) and stayed mismatch-free on chaos/dualstresslong, but chaos and dual still failed smoothness: chaos `17.907/17.945ms`, max `99.879/92.485`, over33 `59/53`; dualstresslong `18.231/18.250ms`, max `97.863/92.709`, over33 `136/125` in `logs/rb-skipjit-preciseinv-repeat-dual/20260622-185347`.
    - Attempts to reduce that invalidation cost by pre-checking 512-byte CodeMem bitmasks are rejected for correctness: contact reproduced `playerActor0X` mismatch at frame `2820` in `logs/rb-skipjit-bitmaskinv-repeat-dual/20260622-185944`. The safe path keeps the conservative 16-byte `CheckAndInvalidate`.
    - Same-frame waits and earlier larger Main RAM delta page sweeps were not fixes under the delay2 rule. `skipjit-rbwait250/500-cp1` did not reduce over33 enough and worsened average timing; the old `skipjit-page512-cp1` / `skipjit-page1024-cp1` sweep mismatched contact in `logs/rb-skipjit-pagesize-sweep/20260622-191452`. A later page512/cp1 retake improved correctness but still missed the chaos spike gate, as noted at the top of this section.
    - Rechecked correction coalescing and checkpoint cadence on the skip-JIT precise-invalidation path. `resimdelay1` stayed mismatch-free in contact/chaos but worsened spikes badly (`364ms` contact max) in `logs/rb-skipjit-resimdelay1-recheck/20260622-192340`; `cp2` stayed mismatch-free but worsened max spikes versus cp1 in `logs/rb-skipjit-cp2-recheck/20260622-192635`.
    - Two more cost-cutting shortcuts are rejected for correctness. `skipjit-codechunk-cp1`, which only runs the conservative 16-byte invalidation inside 512-byte chunks that already report JIT code, still reproduced contact `playerActor0X` mismatch at frame `2820` in `logs/rb-skipjit-codechunk-sweep/20260622-193251`. `skipjit-skipfinalcp-cp1`, which skips the final checkpoint save inside resim and relies on the later normal checkpoint, produced contact mismatch at frame `2010` and chaos mismatch at frame `990` in `logs/rb-skipjit-skipfinalcp-sweep/20260622-193731`.
    - Diagnostic correction: rollback resimulate restore operations now increment `RollbackRestoreCount`, so `NSMB PerfSpike` restore/resim deltas reflect real rollback work instead of showing restores as zero.
...
- Current blocker: the selected NSMB rollback baseline is now stable for no-artificial-delay GUI-standard routes, but artificial jitter with `InputMaxFrameLead=2` still either throttles inside visible frames or, if that wait is skipped experimentally, creates correction storms and mismatch/spikes.
- Next actions: do not increase InputDelay above `2`. Treat `maxresim2` as the local baseline, keep exact `tinycoreramdelta` as a correctness oracle, and focus next on rollback-aware pacing/lead handling rather than adding per-symptom actor patches.
