...
- **Current conclusion:** 中間catch-up tickでもNSMBのgame render callbackを実行し、そのtickがGPU3Dへ生成したsceneだけをFIFO順序付きbegin/end markerで破棄する境界を実装した。行列、geometry test、FIFO処理、render callbackのMain RAM副作用は通常どおり通し、最終tickだけが頂点・polygonを表示sceneへ残す。自動再生ではLuigi側の土管X重複を消し、state収束と60fps相当を維持した。長時間手動runの約1.2秒停止はruntime traceの非同期化で解消し、初戦同期は手動gateを通過した。再戦では外側frameと復元後のgeneration-local logical frameを混用してdepth 12117とする原因を修正し、2回の再戦を含む自動gateで3試合目まで共有game stateの差ゼロを確認した。再戦timelineの自動correctness gateは通過したが、手動目視と再戦ロード時の短いhitch評価は残る。
...
- **Current blocker:** 初戦同期、stdout起因の秒単位停止、再戦logical timelineは手動gateまで通過した。再戦checkpoint復元の約`0.15秒`停止とstage開始直後の`40-53ms`級hitchは残り、対戦中の持続的60fps低下とは別だが製品上の無停止要件は未達である。現行手動gateの通信条件はローカルENetへ片道2-3 frameの送信待ちを加えたもので、実WANの輻輳、再順序化、帯域制約、切断は再現していない。土管Xぶれの消失そのもの、未通過event（死亡/復帰、土管遷移、item）、2D OAMちらつき、音声波形、depth 11超fallbackも未検証である。
- **Next action:** 人工送信遅延を段階的に増やしてrollback深度分布、cannot-arm、継続state差、outer frame時間の安全域を測る。同時に再戦checkpoint復元とstage warm-upのhitchを別のperformance課題として計測・削減する。
