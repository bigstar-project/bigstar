...
- **Current conclusion:** the user's memory was correct: official melonDS 1.1 can run actual unmodified-NSMB single-player gameplay well above 60fps without JIT on this PC. The severe no-JIT slowdown belongs to the current fork, not to the patched ROM alone. However, no-JIT rollback is not thereby faster than JIT rollback: official 1.1's JIT remains about twice as fast as its interpreter, and one corrected presentation interval must pay for both a replay frame and the normal current frame.
...
- The compile-time A/B is now complete. `NSML_ENABLE_CORE_RUNTIME_HOOKS=OFF` structurally makes the ARM runtime/trace gates and NDS watch-write gate constant false. With the normal compute renderer, actual MvL gameplay was still JIT off `21.68fps` / `46.117ms` and JIT on `91.90fps` / `10.882ms` (`logs/rom-jit-matrix-20260802-hooks-compiled-out-gameplay`). This disproves the previous working theory that those diagnostic branches were the dominant interpreter regression.
...
- **Rollback implication:** official no-JIT proves that DS interpretation is not inherently near the tested machine's limit, but it does not make no-JIT the faster rollback engine. At median `143fps`, two single-player frames alone take about `14.0ms`; adding the measured current restore p95 `3.882ms` already reaches about `17.9ms` before checkpoint/presentation work, and actual MvL may be heavier. Official JIT's `297fps` instead gives about `6.7ms` for two forward frames. No-JIT may simplify state correctness by avoiding JIT-cache concerns, but it is not a performance shortcut.
- **Current blocker:** the exact source-level regression between official 1.1 and the current fork is not yet identified. Direct A/B has ruled out dormant ROM patches on the common route, the obvious ARM/NDS diagnostic gates, frontend hooks, renderer choice, Debug/sanitizer builds, and the benchmark harness itself. The remaining difference is inside the evolved emulation core/scheduler/memory paths reached by `NDS::RunFrame()`.
- **Next action:** bisect or transplant the core between tag `1.1` and the current branch using the visible single-player no-JIT run as the cheap gate, then verify the first recovered candidate on actual patched MvL with the fixed-frame/state-trace matrix. Only if clean-core MvL reaches at least `150fps` should a no-JIT exact-rollback experiment be connected; otherwise JIT remains the only plausible full-frame engine.
