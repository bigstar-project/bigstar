...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は13,645行（リファクタ開始前21,253行から7,608行減）で、単一の共有 `State G`、46箇所の `MELONDS_NSML_*` 設定参照、ENet、入力同期、診断、テスト用runtime patchがまだ集中している。正確なtop-level境界で測ると、現在の`InitFromEnvironment`は839行、`BeforeRunFrame`は269行、`AfterRunFrame`は172行。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は48行のオーケストレーションまで縮小した。file数を増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめる。
...
- 完了: input packetのdrop判定、history bundle選択、送信delay/jitter、遅延queue解放条件を `NsmbInputDelivery.cpp` のpure policyへ抽出した。modulo/rangeの境界、bundle history clampと欠損frame fallback、frame時刻またはwall-clock時刻による解放をROM/ENet不要で直接検証する。
...
- 完了: start-readyの旧packet拒否、最初のgameplay input frame、ready受理、250 ms再送条件を、現在の`NsmbNetplayProtocol.cpp`内の`SessionPolicy`へ抽出した。未接続、非input-only、barrier受理前、未送信、local readyなし、境界直前/直後、post-start input到着済みを6本目のCTestでcharacterization test化し、productionのreceive/wait/resend経路を同じpure policyへ接続した。
...
- 完了: fileの細分化を避けるため、`SessionProtocol`、`SessionPolicy`、`PacketClassifier`、`WireProtocol`のnamespace/APIは保ったまま、7個の小さなsource/headerを534行の`NsmbNetplayProtocol.{h,cpp}` 1組へ統合した。既存3 contract test、game-state test、production build、fast/rollback実ゲームgoldenはすべてpassした。
...
- Next action: 残る839行の`InitFromEnvironment`と共有`State G`をfeature group単位で監査し、repo内launcher/test/docsから使われないPoC実験を削除する。次はframe/peer/network-pump coordination、memory patch、state save/loadを既存の中規模configへまとめられるかを確認し、その後rollback runtime、transport、state syncの物理抽出へ進む。数十行ごとの細分化は行わない。
- Verification status: Release `melonDS` と10 unit targetのbuild成功。CTest 10件は直近0.46秒、config test単体は0.14秒でpass。固定seed・packet loss付き回帰は`logs/refactor-runtime-patch-config-fast`と`logs/refactor-runtime-patch-config-rollback`で、host/client semantic state、applied input、rollback checkpoint coverage、golden一致を確認した。`logs/refactor-runtime-patch-config-forced-pair`はhost/client実ゲーム1,050 framesを完走し、両peerでdeath/lives、active powerup、inventory powerup、star counterの指定値を各production hookログで確認した。変更したPoC本体のClang unused診断は0件。host単体forced smokeの初回試行はpeer不在による各frame remote-input waitでtimeoutしたため、該当processを停止し標準fast相当のpaired条件へ修正した。
