...
- Verification status: Release `melonDS` build、CTest 16件（0.44秒）、変更した`ARM.cpp`自身のClang unused診断0件がpassした（既存header由来警告は判定外）。`logs/refactor-remove-unused-live-fallback-fast`のpacket loss付き同一実ROM・2-process 1,250-frame fast tierは16.4秒でsemantic goldenをpassした。`logs/refactor-remove-unused-live-fallback-standard-{host,client}`では、維持したlatest-before＋window=4経路で両peerが1,700 framesを完走した（140.47秒、12.10 FPS）。日常反復はfast tier、約2分強のstandard splitはCPUコア内PacketBridge変更など意味のある節目だけに使う。runner parse、削除対象のrepo全体参照0件、`git diff --check`もpassしている。今回GUI／Rust差分はなく、新規fileもない。
