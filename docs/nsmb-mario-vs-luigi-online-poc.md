...
- Current blocker: with `InputDelayFrames=2`, correctness-oriented rollback is possible through `tinycoreramdelta` plus full `JIT.Reset()` restore, but still not playable in chaos because same-frame cold-JIT/resim spikes remain. Raising GUI rollback InputDelay above `2` is explicitly rejected for this goal and must not be used as the fix.
- Next actions: do not continue trying to fix this by increasing InputDelay. Use full-reset cp1 `tinycoreramdelta` as the correctness oracle. Continue from `jitrc-nomem-cp1` only as a candidate, not a promotion path: repeat it on contact/chaos/dual, then either further classify safe reusable JIT block types or switch to a new rollback architecture that avoids same-frame cold-JIT re-execution while preserving exact host/client state.
