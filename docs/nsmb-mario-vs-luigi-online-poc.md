...
- **Current blocker:** 初戦同期、stdout起因の秒単位停止、再戦logical timelineはそれぞれ手動または自動gateを通過した。再戦はまだユーザー手動目視を通していない。再戦checkpoint復元の約`0.15秒`停止とstage開始直後の`40-53ms`級hitchも残り、対戦中の持続的60fps低下とは別だが製品上の無停止要件は未達である。土管Xぶれの修正後手動runではずれなしと報告されたが、ちらつき消失そのものは今回明示確認されていない。未通過event（死亡/復帰、土管遷移、item）、2D OAMちらつき、音声波形、depth 11超fallbackも未検証である。
- **Next action:** 同じmanual local commandで少なくとも2回再戦し、両画面の位置・勝敗・次stageと土管Xぶれを目視確認する。同期が通れば、再戦checkpoint復元とstage warm-upのhitchを別のperformance課題として計測・削減する。
