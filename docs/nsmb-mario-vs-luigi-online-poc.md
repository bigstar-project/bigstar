...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は19,186行（リファクタ開始前21,253行から2,067行減）で、単一の共有 `State G` に参照中のfieldが782個、381種類の `MELONDS_NSML_*` 設定、ENet、入力同期、rollback、NSMB RAM access、状態同期、診断、テスト用runtime patchがまだ集中している。`InitFromEnvironment` は1,312行、`BeforeRunFrame` は383行、`AfterRunFrame` は371行。
...
- Next action: 803行のgame-state RAM sample readを依存関係ごと整理し、同じstate-sync implementationへ段階的に移す。その後、rollback storeを中規模moduleとして抽出する。いずれも細かなhelper別fileではなく、責務単位の読みやすいfileへまとめる。
