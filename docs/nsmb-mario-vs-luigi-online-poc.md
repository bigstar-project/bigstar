...
- Current blocker: with `InputDelayFrames=2`, correctness-oriented rollback is possible through `tinycoreramdelta` plus full/fast JIT block detachment, but still not playable. The main remaining cost is cold JIT/resim execution after rollback, not checkpoint copy/restore. Raising GUI rollback InputDelay above `2` is explicitly rejected for this goal and must not be used as the fix.
- Next actions: do not continue trying to fix this by increasing InputDelay. Either find a pointer-safe way to preserve/reuse enough rollback JIT state across restore, or move to a new rollback architecture that avoids same-frame cold-JIT resim spikes while preserving exact state. Keep fast-reset nodelete0 as the current correctness baseline and keep skip-JIT-reset variants as diagnostic-only until they pass repeated contact/chaos/dual routes.
