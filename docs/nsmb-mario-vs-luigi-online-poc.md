...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は4,518行（リファクタ開始前21,253行から16,735行、約78.7%減）で、NDS依存のdiagnostic snapshot RAM採取は既存Readerへ、diagnostic eventのJSON整形・artifact生成・before-frame phase計測は既存Diagnosticsへ、現役MvL runtime patchは380行の`NsmbMvlGameHooks`へ、test savestate／LocalMP snapshotは299行の`NsmbTestStateHarness`へ、rollback checkpoint／restore／resimulation／統計は758行の`NsmbRollbackRuntime`へ、PacketBridge transport／JIT integrationは811行のPacketBridge subsystemへ移った。PoCには共有`State G`、診断eventの発火・snapshot選択、subsystem間を接続する薄いadapter、MvL lifecycle、network/inputとframe orchestrationが残る。process environmentの直接読取は0箇所で、`State`定義43行と`InitFromEnvironment`182行は無理に分割しない。最終的なPoC本体は1,200〜1,800行を目安に、初期化とframe処理順を追える司令塔として残す。
...
- Next action: 次は残る4,518行から、受信dispatch、input delivery、start handshake、remote wait／frame-lead throttleを含むnetwork/input sessionの900〜1,400行級の境界を選び、関連する未使用実験分岐と合わせて1バッチで処理する。182行の`InitFromEnvironment`と43行の共有`State G`、rollback／PacketBridge／AIを接続する薄いadapterは行数だけを目的に分解せず、一般`STATE_APPLY`、GUIの`STATE_SYNC`、観測trace、自然同期World gateは維持する。高コストなfast／standard実ROMgateは引き続き各バッチ末尾に各1回だけ実行する。
- Verification status: Release `melonDS`、config／PacketBridge test target build、CTest 16件（0.79秒）、変更したPoC／`NsmbPacketBridgeIntegration`／`NsmbPacketBridgeRuntime`／configのClang strict unused診断がpassした。`logs/refactor-packet-bridge-integration-fast`の同一実ROM・2-process 1,250-frame fast tierはLAN route smokeとsemantic goldenをpassし、`logs/refactor-packet-bridge-integration-standard-{host,client}`では両peerが1,700 framesを完走した（125.34秒、13.56 FPS）。今回GUI／Rust差分はない。
