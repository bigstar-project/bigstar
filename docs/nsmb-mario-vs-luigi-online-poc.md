...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は17,271行（リファクタ開始前21,253行から3,982行減）で、単一の共有 `State G` に参照中のfieldが782個、381種類の `MELONDS_NSML_*` 設定、ENet、入力同期、診断、テスト用runtime patchがまだ集中している。`InitFromEnvironment` は1,312行、`BeforeRunFrame` は383行、`AfterRunFrame` は371行。旧803行の`ReadGameStateSample`は37行のオーケストレーションまで縮小した。
