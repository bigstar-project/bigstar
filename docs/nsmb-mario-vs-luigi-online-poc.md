...
## Live Slippi-style ROM-loop rollback milestone - updated 2026-08-20

- **Latest one-PC/two-instance result:** 修正版の `bigstar-1787162501949-37192-0`（host/Mario）と `bigstar-1787162501980-40744-0`（client/Luigi）で、Luigi側だけが入力設定中に停止した後、hostがprediction horizon `P=7`の端へ張り付いた。hostは設定中ずっと進まず7未確定frameで停止したが、再開後に両roleが同じ60 fpsへ戻るだけで位相差を回収しないため、7-frame差が固定された。hostのprediction mismatch、ROM-loop arm、completeは各`229`件で全件depth 7、client訂正は0件だった。critical state差、checkpoint missing、失敗訂正、bridge drop、33ms超frameは0であり、同期安全性ではなく最大深度訂正によるLuigi spriteのpresentation snapが今回のカクつきである。
- **Phase-control gap:** input configはlocal `EmuThread`だけをpauseし、別Bigstarプロセスへ停止を通知しない。現行horizon gateは8個目の予測前にleaderを止めても、backlogが8から7へ減ると即再開する。frame limiterはpause時間をcatch-upせず、low-water/hysteresisやremote executed frameの追跡もないため、片側停止で作ったframe advantageは自然に縮まらない。
- **Normal-play scope:** 通常のpacket遅延だけで必ず発生するとは判定しない。両clockが進み、対称な通信断で双方がほぼ同時に止まる場合は恒久差を作りにくい。一方、片側の設定dialog、OS suspend、scheduler stall、長いhitch、または強い非対称配送で一方だけがhorizon待機した場合は通常プレイ中にも同じ最大深度張り付きが起こり得る。online中のlocal-only pause禁止または協調pauseに加え、remote実行frameを用いたframe-phase pacingとlow-water hysteresisが必要である。
...
- **Current blocker:** raw/logical frame-domain不一致は修正版GUI実runでもcheckpoint missing、failed correction、critical mismatch 0となり解消を確認した。新しいblockerは、片側emulation停止後にframe advantageがP=7へ固定され、最大深度訂正によるremote spriteのカクつきが継続することである。既知のrollback音声取り消し不能、player spriteの最大3-frame位相差、常時8-9 frame人工遅延、再戦checkpoint復元の約`0.15秒`停止、stage開始直後hitchも別件として残る。
- **Next action:** clientだけを5～8秒停止する2-process回帰を作り、再開後にframe advantageが有限時間で通常目標へ戻る合格条件を固定する。次にremote executed logical frameを交換するframe-phase pacingとlow-water hysteresis、online設定dialogの協調pauseまたは禁止を実装し、訂正深度7への張り付き解消、critical state一致、60 fpsを検証する。
