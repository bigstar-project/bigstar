...
- Next action: 182行の`InitFromEnvironment`と43行の共有`State G`は既にオーケストレーションとして無理のない大きさなので、行数だけを目的に分解しない。約250行のstate save/load harnessも現役のclean-state adapter smokeが利用しているため維持する。次は残る6,525行のinput／PacketBridge／rollback／MvL／diagnostics orchestrationを責務・参照元・実行scenario別に再監査し、複数の現役consumerを持つまとまりは中規模moduleのまま保ちつつ、PoC試行時だけの未使用feature groupを優先して縦に削除する。一般`STATE_APPLY`、GUIの`STATE_SYNC`、観測trace、自然同期World gateは維持する。
- Verification status: Release `melonDS`とdiagnostics test targetのbuild、変更した主要C++ 7 translation unitのClang strict unused診断、CTest 16件（0.45秒）、Diagnostics contract 20回連続がpassした。`logs/refactor-diagnostics-artifacts-software`の同一実ROM・software renderer・2-process 1,100-frame runはhost／client合計6 PNGを生成し、frame 900の画像を目視して上画面の対戦stageと下画面のminimap／inventoryを確認した。設定固定後の`logs/refactor-diagnostics-artifacts-standard-byte-restore`は2-process 3,000 framesを完走し、host／client各10 PNG、保存失敗・skip 0件だった。standard実行前後の`melonDS.toml` SHA-256一致も確認し、ユーザー設定をbyte-for-byteで復元している。`logs/refactor-diagnostics-artifacts-fast`のpacket loss付き2-process 1,250-frame fast tierもsemantic goldenをpassし、自然同期World gateの既存結果も維持している。今回GUI／Rust差分はなく、新規fileもない。
