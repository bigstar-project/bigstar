...
- 最新の成功ログ:
  ...
  - `logs/smvl-hybrid-safe-bothjump-3000-20260528`
    - `tests/nsmb_us_direct_mvl_safe_short.inputs`
    - Mario/Luigiをその場ジャンプさせ、frame 1500-3000 で `RequirePlayer0Input -RequirePlayer1Input -RequireNoLifeLossUntilFrame 3000` 通過
- 最新の未解決:
  ...
  - `tests/nsmb_us_direct_mvl_safe_short.inputs` はMario/Luigi両者入力あり・死亡なしの3000frame安全ルート。次はこれを長時間化し、実操作に近い左右移動やスター/8コインアイテム検証へ広げる必要がある。
...
- 死亡しない両者入力スクリプトを、3000 frameから長時間へ伸ばす。
...
- 追加のROM patch診断
  ...
  - `tests/nsmb_us_direct_mvl_safe_short.inputs` を追加。Mario/Luigiをその場ジャンプさせ、最初のGoomba接触を避ける3000frame安全ルート。
