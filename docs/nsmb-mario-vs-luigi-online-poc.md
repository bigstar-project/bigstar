...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は9,453行（リファクタ開始前21,253行から11,800行減）で、単一の共有 `State G`、3箇所のprocess environment直接読取、game-state hash送信、テスト用runtime patchがまだ集中している。正確なtop-level境界で測ると、現在の`State`定義は171行、`InitFromEnvironment`は671行。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は48行のオーケストレーションまで縮小した。file数や短いfileを増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめ、死んだ実験コードが見つかれば総量も削る。
...
- Next action: 残る671行の`InitFromEnvironment`と171行の共有`State G`をfeature group単位で監査する。現存するplayer/star/powerup/script/JIT runtime patchはAI収集・smoke・goldenから現役利用されていることを確認済みなので維持し、次はframe heartbeatと成果物streamの所有権、またはgame-state hash送信境界を責務単位で整理する。数十行ごとの細分化は行わず、抽出による読みやすさと未使用実験コードの純減を分けて計測する。
- Verification status: Release `melonDS` build、PoC/input timelineのClang unused診断、CTest 12件（0.24秒）がpass。Input timeline/recorder testは20回連続passし、`logs/refactor-input-recorder`の実ROM記録内容も完全一致で検査した。固定seed・packet loss・4-frame input delay付き2-process実ゲーム回帰は最終codeで`logs/refactor-input-recorder-fast-final`と`logs/refactor-input-recorder-rollback-final`を各1,250 frames完走し、host/client semantic state、applied input、checkpoint coverage、golden一致を確認した。Coordinator contract testと同一process実ROMのbarrier/serial/NDS+LocalMP save-load回帰も引き続きpassしている。diagnostics tierの`logs/refactor-hang-diagnostics-tier`はhost/client別watchdog/phase JSONL schemaとroleを検査済み。state sync/apply専用`logs/refactor-state-sync-builders-apply`ではplayer/world/actor snapshot実適用とmoving-hazard GUID map更新を確認済み。送信delay/jitterの実queue経路は`logs/refactor-input-delivery-delay-jitter`、ImitationAI production pathは`logs/refactor-mvl-ai-imitation`、GUIが有効化する`coredelta`は`logs/refactor-remove-rejected-rollback-coredelta`が引き続きpassしている。
