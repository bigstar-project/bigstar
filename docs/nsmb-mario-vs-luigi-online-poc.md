...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は11,755行（リファクタ開始前21,253行から9,498行減）で、単一の共有 `State G`、27箇所の `MELONDS_NSML_*` 設定参照、入力同期、診断runtime、テスト用runtime patchがまだ集中している。正確なtop-level境界で測ると、現在の`State`定義は320行、`InitFromEnvironment`は559行、`BeforeRunFrame`は269行、`AfterRunFrame`は171行。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は48行のオーケストレーションまで縮小した。file数を増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめる。
