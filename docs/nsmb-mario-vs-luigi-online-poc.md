...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は12,168行（リファクタ開始前21,253行から9,085行減）で、単一の共有 `State G`、27箇所の `MELONDS_NSML_*` 設定参照、入力同期、診断、テスト用runtime patchがまだ集中している。正確なtop-level境界で測ると、現在の`State`定義は371行、`InitFromEnvironment`は603行、`BeforeRunFrame`は269行、`AfterRunFrame`は172行。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は48行のオーケストレーションまで縮小した。file数を増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめる。
...
- Verification status: Release `melonDS` build、Clang unused診断、CTest 11件（0.19秒）がpass。最新の固定seed・packet loss付き2-process実ゲーム回帰は`logs/refactor-input-delivery-fast`と`logs/refactor-input-delivery-rollback`で、savestate/tinycorepreimage双方の1,250 frames完走、host/client semantic state、applied input、checkpoint coverage、golden一致を確認した。さらに`logs/refactor-input-delivery-delay-jitter`で送信delay 2 frames、jitter 1 frame、packet loss、history bundleを同時に有効化し、実際の遅延queue経路を通して1,250 framesのsemantic syncを確認した。RuleAI production pathの`logs/refactor-mvl-ai-rule-direct`とImitationAI production pathの`logs/refactor-mvl-ai-imitation`もpassしている。GUIが有効化する`coredelta`の`logs/refactor-remove-rejected-rollback-coredelta`も引き続きpassしている。
