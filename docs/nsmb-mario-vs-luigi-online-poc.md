...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は17,408行（リファクタ開始前21,253行から3,845行減）で、単一の共有 `State G` に参照中のfieldが782個、381種類の `MELONDS_NSML_*` 設定、ENet、入力同期、rollback、診断、テスト用runtime patchがまだ集中している。`InitFromEnvironment` は1,312行、`BeforeRunFrame` は383行、`AfterRunFrame` は371行。旧803行の`ReadGameStateSample`は37行のオーケストレーションまで縮小した。
...
- Next action: rollback checkpoint/history/page deltaの保存・検索・破棄を、細かなhelper別fileにせず中規模のrollback store moduleへ抽出する。その後、frame orchestrationと診断/runtime patch群を責務単位で整理する。
- Verification status: Release `melonDS` と8 unit targetのbuild成功。CTest 8件は合計約0.05-0.28秒でpass。旧binary baseline 2,300 framesはpass。固定seedのfast tierはpacket lossありでもhost/client semantic stateとapplied inputがbyte-for-byte golden一致した。rollback tierも約14-15秒でcheckpoint保存・packet loss回復・golden一致を確認した。直近のobject/state reader統合後は`logs/refactor-object-state-reader-fast`と`logs/refactor-object-state-reader-rollback`で再検証済み。Clang unused診断はPoC本体・readerとも0件。
