...
- Latest delay-2 Plan-D-style snapshot/repair experiments:
  ...
  - Current Plan-D interpretation: selected actor/global repair must become a real active-object lifecycle model, not a list of per-symptom patches. The latest `chaos` failure includes `significantActiveObjects` drift with host-only `0x10C`, so moving hazard/star fixes alone cannot make this exact.
