...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は19,189行（リファクタ開始前21,253行から2,064行減）で、単一の共有 `State G` に参照中のfieldが782個、381種類の `MELONDS_NSML_*` 設定、ENet、入力同期、rollback、NSMB RAM access、状態同期、診断、テスト用runtime patchがまだ集中している。`InitFromEnvironment` は1,312行、`BeforeRunFrame` は383行、`AfterRunFrame` は371行。
...
- 完了: match seed/start-readyの共用16-byte packetを `NsmbSessionProtocol.cpp` へ抽出した。`SEED`/`STRT`双方の既存little-endian layoutをgolden bytesで固定し、round-trip、null/短縮packet、magic/version/kind不正を5本目のCTestで検査する。productionの送受信も同じcodecを使い、fast/rollbackの2-process実ゲーム回帰でseed同期とstart-ready handshakeを通した。
- 完了: start-readyの旧packet拒否、最初のgameplay input frame、ready受理、250 ms再送条件を `NsmbSessionPolicy.cpp` へ抽出した。未接続、非input-only、barrier受理前、未送信、local readyなし、境界直前/直後、post-start input到着済みを6本目のCTestでcharacterization test化し、productionのreceive/wait/resend経路を同じpure policyへ接続した。
- 完了: ENet受信packetのサイズ分類を `NsmbPacketClassifier.cpp` へ抽出した。input、bundle候補、session、NSML/player/world/hazard/actor/effect/game stateの全class、unknown、既存のinput/session優先および同サイズgame packet間の優先順位を7本目のCTestで固定し、411行のnetwork pumpから重複したサイズ比較を除去した。
...
- Next action: 803行のgame-state RAM sample readを依存関係ごと整理し、同じstate-sync implementationへ段階的に移す。その後、network protocol/session/classifierの小moduleをまとまりに沿って統合し、rollback storeを中規模moduleとして抽出する。
- Verification status: Release `melonDS` と8 unit targetのbuild成功。CTest 8件は合計約0.05-0.20秒でpass。旧binary baseline 2,300 framesはpass。固定seedのfast tierはpacket lossありでもhost/client semantic stateとapplied inputがbyte-for-byte golden一致した。rollback tierも約14-15秒でcheckpoint保存・packet loss回復・golden一致を確認した。typed bootstrap/connection/input/rollback config、unused/旧runtime patch削除、input timeline/protocol/delivery/session protocol/policy、packet classifier、game-state codec/schema抽出後もpassし、Clang unused診断は0件。
