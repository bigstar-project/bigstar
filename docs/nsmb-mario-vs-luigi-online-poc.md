...
- Latest delay-2 exact-ish Main RAM delta experiment:
  ...
  - New JIT-configuration boundary found:
    - Added env overrides for NSML JIT args. `MELONDS_NSML_JIT_LITERAL_OPTIMIZATIONS=0` plus `MELONDS_NSML_JIT_FAST_MEMORY=0` makes `RollbackSkipJitReset` usable with forced Main RAM/tiny-core JIT invalidation in the measured page256 candidate.
    - `exact-delay2-jitnoliteral-nofastmem-tinycoreramdelta-key2-page256-cp2-maxresim1-skipjit-skiprender` had no persistent mismatch on contact/chaos/dualstresslong under `InputSendDelayFrames=6` / `InputSendJitterFrames=6`, but still failed performance gates. Contact: `17.174/17.184ms`, max `74.836/183.450`, over33 `19/20` in `logs/nsmb-mvl-rollback-jitnoliteral-nofastmem-contact/20260622-140614`. Chaos/dualstresslong: no mismatch, averages `17.7-17.9ms`, max `74-105ms`, over33 `81-168` in `logs/nsmb-mvl-rollback-jitnoliteral-nofastmem-chaos-dual/20260622-140918`.
    - Boundary checks: literal-only off still mismatched; literal+branch off still mismatched; fastmem-only off still mismatched. So the current safe skip-reset condition is the combination of literal optimization off and fast memory off.
    - Increasing Main RAM delta page size is not correctness-safe for this branch: page1024 improved chaos timing slightly but contact mismatched at frame `2820` in `logs/nsmb-mvl-rollback-jitnoliteral-nofastmem-page1024-contact-dual/20260622-141838`.
...
- Current blocker: with `InputDelayFrames=2`, correctness-oriented rollback is possible through `tinycoreramdelta`, and the new no-literal/no-fastmem JIT mode removes the earlier skip-reset desync in measured page256 routes. It still produces too many over33 frames and occasional large spikes under strong jitter, so it is not yet playable. Raising GUI rollback InputDelay above `2` is explicitly rejected for this goal and must not be used as the fix.
- Next actions: keep optimizing the no-literal/no-fastmem `tinycoreramdelta` branch first because it is the first measured path that avoids full JIT reset without persistent mismatch across contact/chaos/dualstresslong. Focus on reducing over33 spikes without changing delay2: checkpoint cadence, delta/invalidation cost, resim scheduling, or rollback-specific JIT state reuse. Continue treating broad actor arena/tail copying and InputDelay increases as rejected.
