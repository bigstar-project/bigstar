...
- Latest delay-2 exact-ish Main RAM delta experiment:
  ...
  - Latest JIT reset experiments:
    ...
    - Added conservative JIT restore filters. `jitrc-nomem-cp1`, which refuses to reuse JIT blocks containing memory instructions, is the best new direction so far: contact/chaos under delay2 strong jitter had no mismatch in `logs/rb-jitrc-nomem/20260622-175856`. It still failed playability, with contact barely above the spike gate (`90.034/89.234ms`) and chaos still too spiky (`114.307/148.275ms`).
...
- Current blocker: with `InputDelayFrames=2`, the best exact-ish path is now `tinycoreramdelta` plus skip-JIT reset with conservative 16-byte restored-range invalidation. It can pass contact and preserve state in chaos/dualstresslong under strong jitter, but still misses playable 60fps smoothness because rollback restore/resim bursts remain around `90-100ms` and over33 counts are high.
- Next actions: do not increase InputDelay above `2`. Keep full-reset `tinycoreramdelta` as the correctness oracle, but continue from the skip-JIT precise-invalidation path as the best practical candidate. The next improvement should reduce restored-range invalidation/resim burst cost without weakening correctness, or move to a new rollback architecture that avoids same-frame correction bursts while preserving exact host/client state.
