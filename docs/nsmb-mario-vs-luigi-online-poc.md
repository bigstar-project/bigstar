...
- Next action: 182行の`InitFromEnvironment`と43行の共有`State G`は既にオーケストレーションとして無理のない大きさなので、行数だけを目的に分解しない。約250行のstate save/load harnessも現役のclean-state adapter smokeが利用しているため維持する。次は残る6,329行のinput／PacketBridge／rollback／MvL／diagnostics orchestrationを責務・参照元・実行scenario別に再監査し、複数の現役consumerを持つまとまりは中規模moduleのまま保ちつつ、PoC試行時だけの未使用feature groupを優先して縦に削除する。一般`STATE_APPLY`、GUIの`STATE_SYNC`、観測trace、自然同期World gateは維持する。
- Verification status: Release `melonDS`と関連test targetのbuild、CTest 16件（0.56秒）、変更したPoC／Config／Diagnostics／PacketBridgeRuntime自身のClang unused診断0件がpassした（既存header由来警告は判定外）。`logs/refactor-remove-tick-lead-fast`のpacket loss付き同一実ROM・2-process 1,250-frame fast tierは17.9秒でsemantic goldenをpassした。`logs/refactor-remove-tick-lead-standard-{host,client}`では、維持したframe-lead throttleを含む両peerが1,700 framesを完走した（135.49秒、12.55 FPS）。日常反復はfast tier、約2分強のstandard splitはPacketBridge変更など意味のある節目だけに使う。runner parse、削除対象の`src/tests/scripts`参照0件、`git diff --check`もpassしている。今回GUI／Rust差分はなく、新規fileもない。
