...
- Next action: 次は残る2,534行から、diagnostic event／snapshot／life・heartbeatとAI config／入力adapterの境界を監査し、既存`NsmbNetplayDiagnostics`／RuleAI／ImitationAIへ寄せられる処理と未使用診断分岐を大きな1バッチで整理する。174行の`InitFromEnvironment`、38行の共有`State G`、公開facade、Before／After frameのphase順、subsystem hookは司令塔として残し、1,200〜1,800行の終了帯へ入れる。高コストなfast／standard実ROMgateは引き続きバッチ末尾に集約する。
- Verification status: Release `melonDS` build、CTest 16件（4.20秒）、変更したPoC／`NsmbMvlLifecycle`／`NsmbGameStateSync`のClang strict unused診断がpassした。`logs/refactor-mvl-lifecycle-state-sync-fast-retry`の同一実ROM・2-process 1,250-frame fast tierはLAN route smokeとsemantic goldenをpassし、`logs/refactor-mvl-lifecycle-state-sync-standard-{host,client}`では両peerが1,700 framesを完走した（182.00秒、9.34 FPS）。今回は別processが約2.8 CPU coreを継続使用する高負荷環境だったため性能値は前バッチとの比較判定に使わず、timeout／wait増加がないこととsemantic gate成功を採用した。今回GUI／Rust差分はない。
