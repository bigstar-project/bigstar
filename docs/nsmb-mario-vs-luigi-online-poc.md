...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は8,781行（リファクタ開始前21,253行から12,472行減）で、単一の共有 `State G`、game-state sample採取、テスト用runtime patchがまだ集中している。process environmentの直接読取は0箇所になった。brace境界で再計測した現在の`State`定義は70行、`InitFromEnvironment`は468行（旧記載の656行は現ソースと矛盾していたため修正済み）。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は28行のオーケストレーションまで縮小した。file数や短いfileを増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめ、死んだ実験コードが見つかれば総量も削る。
...
- Next action: 残る468行の`InitFromEnvironment`と70行の共有`State G`をfeature group単位で監査する。現存するplayer/star/powerup/script/JIT runtime patchはAI収集・smoke・goldenから現役利用されていることを確認済みなので維持する。次はqueue・wire tick・JIT helper・lockstep markerが混在するPacketBridge runtimeを部分counterだけ移さず一つの責務として切り出せるか、またはruntime patch適用状態を既存configに対応するruntimeへまとめられるかを調べる。自然な中規模境界が得られない場合は無理にfileを増やさない。抽出による読みやすさと未使用実験コードの純減は分けて計測する。
- Verification status: Release `melonDS` build、PoCとDiagnostics moduleのClang strict unused診断、CTest 15件（0.33秒）がpassし、拡張したDiagnostics runtime contract testは初回に加えて20回連続passした。`logs/refactor-diagnostic-ring-runtime`のpacket loss付き2-process実ゲームdiagnostics tierは1,250 framesをpassし、fast golden、hang watchdog、frame heartbeat、両peer各12件のparse可能なevent JSONL、最大360-frameのring出力を確認した。直前の`logs/refactor-mvl-runtime-secondgame`に対するtinycorepreimage rollback付き2-process実ゲームsmokeも10,000 framesをpassしており、両peerがframe 3928で同じwinner/scoreを確定し、frame 4047で同じ19,228,045-byte bootstrap checkpoint・seedを復元して第2試合へ入り、frame 10,000まで同期した。AI logのneutral/linear-model byte一致、hash、state sync/apply、送信delay/jitter、GUIの`coredelta`各production回帰も引き続きpassしている。
