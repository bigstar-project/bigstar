...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は9,344行（リファクタ開始前21,253行から11,909行減）で、単一の共有 `State G`、3箇所のprocess environment直接読取、game-state hash送信、テスト用runtime patchがまだ集中している。正確なtop-level境界で測ると、現在の`State`定義は162行、`InitFromEnvironment`は656行。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は28行のオーケストレーションまで縮小した。file数や短いfileを増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめ、死んだ実験コードが見つかれば総量も削る。
...
- Next action: 残る656行の`InitFromEnvironment`と162行の共有`State G`をfeature group単位で監査する。現存するplayer/star/powerup/script/JIT runtime patchはAI収集・smoke・goldenから現役利用されていることを確認済みなので維持する。次はgame-state hash送信・照合のfacade境界を優先し、`NsmbAiObservation.cpp`へ広く結合しているAI streamは責務がまとまる境界が得られるまで細分化しない。抽出による読みやすさと未使用実験コードの純減は分けて計測する。
- Verification status: Release `melonDS` build、PoC/DiagnosticsのClang unused診断、CTest 13件（0.40秒）がpassし、diagnostics contract testは20回連続passした。`logs/refactor-hash-log-runtime`ではhash log有効の実ROM・2-process・packet loss走行を1,250 frames完走して両peerのCSV schema/data rowとsemantic同期を確認した。hash log無効の既定高速経路も`logs/refactor-hash-log-runtime-fast`、`logs/refactor-hash-log-runtime-rollback`、`logs/refactor-hash-log-runtime-diagnostics`を各1,250 frames完走し、host/client semantic CSV、applied input、checkpoint coverage、watchdog/phase/heartbeat、golden一致を確認した。GameState/Input recorder/Coordinatorのcontract testと実ROM回帰も引き続きpassしている。state sync/apply専用`logs/refactor-state-sync-builders-apply`ではplayer/world/actor snapshot実適用とmoving-hazard GUID map更新を確認済み。送信delay/jitterの実queue経路は`logs/refactor-input-delivery-delay-jitter`、ImitationAI production pathは`logs/refactor-mvl-ai-imitation`、GUIが有効化する`coredelta`は`logs/refactor-remove-rejected-rollback-coredelta`が引き続きpassしている。
