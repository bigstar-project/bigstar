...
- Next actions: do not continue trying to fix this by increasing InputDelay. Use full-reset cp1 `tinycoreramdelta` as the correctness oracle. Treat JIT reuse as unsafe until it passes repeated contact/chaos/dual routes; the next useful work is either a truly pointer-safe/block-state-safe JIT reuse design or a rollback architecture that avoids same-frame cold-JIT re-execution while preserving exact host/client state.
