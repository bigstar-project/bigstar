...
- Next actions:
  - Downgrade basic-only mismatch handling so it does not look like a confirmed desync in GUI/status logs.
  - Inspect and harden the no-rollback input wait path around rematch/restart, not just first-match steady-state input: exact-frame remote input should be required before simulation advances after every start barrier/rebase.
  - Inspect rematch reset/start-ready state around `ResetMvlRuntimeSyncStateForRestart`, start-ready send/accept, input queue rebasing, and timeout handling, because the second-match failure logs include `remote start ready wait timeout` and post-result throttle timeouts.
