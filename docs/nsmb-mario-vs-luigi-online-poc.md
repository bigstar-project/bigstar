...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は12,571行（リファクタ開始前21,253行から8,682行減）で、単一の共有 `State G`、27箇所の `MELONDS_NSML_*` 設定参照、ENet、入力同期、診断、テスト用runtime patchがまだ集中している。正確なtop-level境界で測ると、現在の`State`定義は488行、`InitFromEnvironment`は741行、`BeforeRunFrame`は269行、`AfterRunFrame`は172行。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は48行のオーケストレーションまで縮小した。file数を増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめる。
...
- Next action: 残る741行の`InitFromEnvironment`と共有`State G`をfeature group単位で監査し、repo内launcher/test/docsから使われないPoC実験を削除する。残るgeneric rollback、ENet transport、state sync apply/send、診断の規模と依存を比較し、最も境界が明確な一まとまりを既存moduleまたは一つの中規模moduleへ物理抽出する。数十行ごとの細分化は行わない。
- Verification status: Release `melonDS` buildとCTest 10件（0.12秒）がpass。最新の固定seed・packet loss付き2-process実ゲーム回帰は`logs/refactor-statesync-direct-fast`と`logs/refactor-statesync-direct-rollback`で、savestate/tinycorepreimage双方の1,250 frames完走、host/client semantic state、applied input、checkpoint coverage、golden一致を確認した。拒否済みbackend削除時のClang unused診断、PowerShell変更3本のparser検査、GUIが有効化する`coredelta`の`logs/refactor-remove-rejected-rollback-coredelta`も引き続きpassしている。
