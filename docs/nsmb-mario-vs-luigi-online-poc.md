...
- 現状: `src/frontend/qt_sdl/NsmbNetplayPoC.cpp` は14,085行（リファクタ開始前21,253行から7,168行減）で、単一の共有 `State G`、87箇所の `MELONDS_NSML_*` 設定参照、ENet、入力同期、診断、テスト用runtime patchがまだ集中している。正確なtop-level境界で測ると、現在の`InitFromEnvironment`は911行、`BeforeRunFrame`は269行、`AfterRunFrame`は172行。旧803行の`ReadGameStateSample`は37行、旧449行の`TraceGameState`は48行のオーケストレーションまで縮小した。file数を増やすこと自体は目的にせず、最終的に責務が読める中規模moduleへまとめる。
...
- Next action: 残る911行の`InitFromEnvironment`と共有`State G`をfeature group単位で監査し、repo内launcher/test/docsから使われないPoC実験を削除する。そのうえでrollback runtime、transport、state syncなど依存がまとまる境界を選び、中規模moduleとして物理抽出する。数十行ごとの細分化は行わない。
- Verification status: Release `melonDS` と9 unit targetのbuild成功。CTest 9件は直近0.36秒でpass。旧binary baseline 2,300 framesはpass。固定seedのfast tierはpacket lossありでもhost/client semantic stateとapplied inputがbyte-for-byte golden一致した。rollback tierもcheckpoint保存・packet loss回復・golden一致を確認した。直近は`logs/refactor-ai-config-fast`と`logs/refactor-ai-config-rollback`で両方初回passした。加えて`logs/refactor-ai-config-rule-final`はRuleAI有効化後に実ゲーム1,100 framesを完走し、起動設定ログ1件と実入力判断trace 8件を確認した。変更したPoC本体のClang unused診断は0件で、全launcherのPowerShell parser errorも0件。
