...
- 最優先: `run-nsmb-mvl-ai-closed-loop-eval.ps1` でneutral / RuleAI / ImitationAIを複数seedで順番に回し、`closed-loop-eval.json` を比較する。初回2600FではImitationAIが1スター取れたが全policyで死亡が残ったため、まず死亡frame周辺をSVG/viewerで見て、落下/危険回避の特徴量・RuleAI・模倣モデルを改善する。
