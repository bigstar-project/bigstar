...
- `logs/nsmb-mvl-human-recording-stage0-20260608-012640` の人間ログでは、Luigiがファイアーボールを撃つ場面でも `players[1].powerup` は0固定、`inventoryPowerup` は1固定だった。一方で `players[1].visualState.actorPowerupState` / `actorPowerupFormState` は0/1/2に変化し、最終的に2が長く出る。画面上のFire状態や変身中/適用済み状態は、当面 `actorPowerupState/FormState` を併用して扱い、`powerup.raw` だけでFire Mario判定しない。
