...
- melonDS実行時の逐次推論は、runtime JSONを絶対パスで渡して使う。例: `MELONDS_NSML_IMITATION_AI=1`、`MELONDS_NSML_IMITATION_AI_MODEL=C:\...\human-stage0-player1-runtime-model.json`、`MELONDS_NSML_IMITATION_AI_PLAYER=remote`、`MELONDS_NSML_IMITATION_AI_THRESHOLD=0.5`。`MELONDS_NSML_IMITATION_AI_TRACE=1` と `MELONDS_NSML_IMITATION_AI_TRACE_INTERVAL=120` で、feature coverage、held、button確率をstdoutへ出せる。
