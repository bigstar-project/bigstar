...
- 完了: RuleAIの `PlayerFrameState` にplayer別hazard threatを追加し、単一の最新moving hazardではなくobject scan全体から選んだ危険対象を見るようにした。tileProbeの壁だけでなく `contact.wallLeft/right` も内部壁判定へ反映し、近距離hazard時は逃げ方向または水平中立+jumpへ切り替える。`MELONDS_NSML_RULE_AI_HAZARD_HORIZONTAL_RANGE` / `MELONDS_NSML_RULE_AI_HAZARD_VERTICAL_RANGE` で閾値を調整できる。
...
`players[].visualState` には、画面上の変身状態やストック表示に相当する候補を保存する。`powerup.raw` / `inventoryPowerup.raw` はメモリから読んだ値で、`name` は暫定mapping、`mappingVerified=0` はstage 0実ログで値と画面表示をまだ照合していないことを示す。`canShootFireCandidate`、`isShellCandidate`、`isMegaCandidate`、`hasReserveItemCandidate` はraw値からの候補bitであり、模倣学習では使えるが、評価時は `mappingVerified` を見て未検証値を過信しない。`visualPowerupKindCandidate` は `powerup.raw`、reserve item、actor内のpowerup/form state、shell stateを統合した画面上のpowerup候補、`visualPowerupSourceMask` は候補の根拠bit、`isFireVisualCandidate` はreserve表示も含むFire候補、`canShootFireVisualCandidate` は現在変身状態から見てファイアーボールを撃てる候補を示す。`damageGuardTimer` は `0x0208B344 + player*2` の16bit値、`damageCooldown` はplayer actor `+0x79C`、`damageGuardFlag` は `+0x7C1`、`damagePhysicsGuard` は `physicsFlag & 0x80000000`、`shellState` は `PlayerBase::getShellState` と同じ `actionFlag & 0x400000` と shell actor pointerから計算する。`invincibleKnown=1` はこれらのdamage guard系ソースをログ化できていることを示す。ただし、スター無敵、ダメージ後無敵、powerup適用中無敵の原因分類はまだ候補扱いで、stage 0実ログで画面表示との照合が必要。
...
- 閉ループ評価は `powershell -NoProfile -ExecutionPolicy Bypass -File scripts\run-nsmb-mvl-ai-closed-loop-eval.ps1 -Policy imitation -Frames 2600 -AllowJit` で実行する。`-Policy neutral` / `-Policy rule` へ切り替えると同条件baselineを作れる。結果は `<LogDir>\closed-loop-eval.json`。`-Policy rule` では `<LogDir>\host\ruleai-audit.json` と `<LogDir>\client\ruleai-audit.json` も自動生成される。既存ログだけを評価する場合は `python scripts\nsmb_mvl_ai_evaluate_closed_loop.py <ai-playlog.jsonl|game-state.csv> ... --player 1 --policy <name> --output <closed-loop-eval.json>` を使う。
...
- 5seed x 2600Fの最新評価では、RuleAIはobject scan全体のhazard threatとcontact壁反映で `avgStarPickups=0.2`、`avgDeathTransitions=1.0`、`avgAliveRatio=0.8833333333`、`avgBigStarApproachDelta=1597239840230.0`。5seedでもBig Star接近はプラスだが、毎run 1 death程度は残る。RuleAIは人間ログ前の相手/分布拡張器として使える段階に近づいたが、まだ「強いAI」ではない。
...
- 2026-06-08 `logs\human-stage0-item-box-001` を後処理。`recording-audit.json` pass、dataset 6012行、`itemVisible=1562`、`blockCandidateVisible=1898`、`coinChange=33`、`visualPowerupChange=1`。Luigiのcoinが `7 -> 0` になる8コイン成立候補はframe 2012 / 3111 / 4582 / 6379で、直後に `objectId=0x01F` のitemが出る。例: frame 2013は `coin_reward_item_found=1`、`coin_reward_item_settings=0x00011089`、`coin_reward_item_powerup_kind_candidate=2`、frame 2118は同item取得近傍で `visualPowerupKindCandidate 1 -> 2`。frame 6380 / 6456では `coin_reward_item_settings=0x0001108B` を疑似マメ候補として `coin_reward_item_is_suspected_mini_candidate=1`、`coin_reward_item_avoid_candidate=1` にすることを確認した。settings mappingはまだ完全確定ではないので、raw settings/vtable/objectIdをCSVに残して再分類可能にしている。
