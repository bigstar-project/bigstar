...
- 2seed x 2600Fの最新評価では、RuleAIはobject scan全体のhazard threatとcontact壁反映で `avgStarPickups=0.5`、`avgDeathTransitions=1.0`、`avgAliveRatio=0.8833333333` まで改善した。ただし死亡はまだ残り、`0x2f52869f` はstar取得後の壁/空中復帰、`0x13579bdf` はitem/enemy/hazardが重なる場面で不安定。RuleAIは人間ログ前の相手/分布拡張器として使える段階に近づいたが、まだ「強いAI」ではない。
- ImitationAIはobject scan全体のhazard threat化後も2seed平均では `avgDeathTransitions=1.0` のまま。`0x2f52869f` では旧1470死亡が消えたが2310死亡が残り、`0x13579bdf` では1470死亡が残る。単純な「危険方向を消す/逃げ方向へ入れる」だけでは、壁際・着地・敵/hazard同時接近を解けないため、次は失敗frameをviewer/SVGで見ながら人間ラベル追加またはDAgger対象にする。
...
- 最優先: 最新2seed評価で残った死亡/不安定さを潰す。RuleAIは `0x2f52869f` のstar取得後死亡と `0x13579bdf` のitem/enemy/hazard混在死亡をSVG/viewerで確認し、落下復帰・敵回避・star取得後の目標切替を改善する。ImitationAIは `0x13579bdf` の1470死亡と `0x2f52869f` の2310死亡をDAgger対象にし、単純guardで無理に直すより失敗frame周辺の人間ラベル追加/再学習を優先する。その後 `run-nsmb-mvl-ai-closed-loop-batch.ps1` でseed数を3-5本へ増やす。
