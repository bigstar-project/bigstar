...
- 閉ループ評価は `powershell -NoProfile -ExecutionPolicy Bypass -File scripts\run-nsmb-mvl-ai-closed-loop-eval.ps1 -Policy imitation -Frames 2600 -AllowJit` で実行する。`-Policy neutral` / `-Policy rule` へ切り替えると同条件baselineを作れる。結果は `<LogDir>\closed-loop-eval.json`。既存ログだけを評価する場合は `python scripts\nsmb_mvl_ai_evaluate_closed_loop.py <ai-playlog.jsonl|game-state.csv> ... --player 1 --policy <name> --output <closed-loop-eval.json>` を使う。
