...
- 2026-06-08 `logs\human-stage0-item-box-001` を後処理。`recording-audit.json` pass、dataset 6012行、`itemVisible=1562`、`blockCandidateVisible=1898`、`coinChange=33`、`visualPowerupChange=1`。Luigiのcoinが `7 -> 0` になる8コイン成立候補はframe 2012 / 3111 / 4582 / 6379で、直後に `objectId=0x01F` のitemが出る。例: frame 2013は `nearest_item_found=1`、`nearest_item_dy=589824`、frame 2118は `nearest_item_dist=49695` かつ `visualPowerupKindCandidate 1 -> 2`。現状でも降ってくるitemの座標/相対距離は学習CSVに入るが、CSVにはitem settings/subtypeや「8コイン報酬由来」フラグはまだ入っていない。
...
- 8コイン報酬itemをより明示的に学習させるため、datasetに最近傍itemの `objectId/settings/vtable`、velocity、screen座標、item source候補を追加する。現状は `nearest_item_found/dx/dy/dist` と `count_item` までは入るが、reward由来か通常item box由来かは明示列では分からない。
