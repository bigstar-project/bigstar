...
- 完了: 手動人間記録の実プレイ速度を測定し、peer/start ready/local remote input待ちで停止する経路を外した。`run-nsmb-mvl-manual-local.ps1` は手動記録時に `-NoLocalWait` と `-NoImplicitInputNetplayPeerWait` を渡し、`-WaitForPeerAtNetplayStart` を使わない。`run-nsmb-mvl-human-recording.ps1` は実プレイ用にJITを既定で有効にし、検証目的でJITを切る場合だけ `-NoJit` を指定する。
...
- 人間プレイを収集する場合は、まず `pwsh scripts\run-nsmb-mvl-human-recording.ps1` を使う。これはstage 0固定でhost/client別AI play logとpacket captureを出し、終了後に `recording-session.json` の後処理コマンドで `packet-replay.csv`、`recording.json`、`recordings-index.json`、datasetを作る。デフォルトは `HumanSide=client` で、client/Luigiだけを人間操作対象にし、host/Mario側の物理入力は `-NeutralizeHostInput` 相当で受け付けない。実プレイ用はJITありを既定にする。JITなしは900F測定で約11fpsしか出ないため、再現性確認などの検証目的でのみ `-NoJit` を指定する。packet captureが不要な検証では `-NoPacketCapture` を指定する。
...
- 2026-06-08 手動人間記録の速度実測。revert後の未修正状態では `logs\codex-perf-human-recording-nojit-20260608` が900Fを240秒で完走せず、frame 856以降で `remote gameplay start ready wait timeout waitedMs=10000` が毎frame発生。`-WaitForPeerAtNetplayStart` を外してもframe 824のpeer waitで停止。`-NoImplicitInputNetplayPeerWait` と `-NoLocalWait` 追加後、JITなしは `logs\codex-perf-human-recording-nojit-nolocalwait-20260608` で900F完走、host 11.14fps / client 11.15fps。JITありは `logs\codex-perf-human-recording-jit-nolocalwait-20260608` で900F完走、host 58.82fps / client 59.01fps、`remoteWaitCount=0`。JIT既定化後の通常コマンド相当は `logs\codex-perf-human-recording-default-jit-20260608` でhost 58.54fps / client 59.06fps、`remoteWaitCount=0`。
