...
- 完了: ImitationAIの実行時入力に、近傍 `moving_hazard` / `hazard` / `enemy_goomba` / `enemy_koopa` を見るhazard guardを追加した。`MELONDS_NSML_IMITATION_AI_HAZARD_GUARD=0` または `MELONDS_NSML_IMITATION_AI_DISABLE_HAZARD_GUARD=1` で無効化でき、横/縦/近距離閾値は `MELONDS_NSML_IMITATION_AI_HAZARD_GUARD_HORIZONTAL_RANGE` / `VERTICAL_RANGE` / `CLOSE_RANGE` で調整できる。これは学習データ生成には影響せず、逐次推論時だけ危険方向入力を抑える実験的な安全フィルタ。
- 完了: RuleAIの `FrameState` にmoving hazardの座標/速度を追加し、近距離hazard時は逃げ方向または水平中立+jumpへ切り替えるhazard modeを追加した。`MELONDS_NSML_RULE_AI_HAZARD_HORIZONTAL_RANGE` / `MELONDS_NSML_RULE_AI_HAZARD_VERTICAL_RANGE` で閾値を調整できる。
...
- 2600F単一seedでは、ImitationAI hazard guardを入れても死亡frame 1470は残った。frame 1320/2070系の事故は縦監視範囲を広げると抑えられたが、1470はmoving hazardが左、右壁あり、着地タイミングが絡むため、単純な「近ければジャンプ/逃げ方向」だけでは不十分。次はhazard相対速度、着地予測、壁詰まり時の早めの切り返し、または人間/RuleAIログ追加で改善する。
- 2seed x 3policyの2600F評価では、ImitationAIは `0x2f52869f` で1スター、`0x13579bdf` で0スター。平均では `avgStarPickups=0.5` / `avgDeathTransitions=1.0`。neutralはstar 0 / death 1.0、RuleAIはstar 0 / death 1.5。つまり現行模倣AIは一部seedで星を取れるが安定しておらず、RuleAIも対戦相手/DAgger補助器として死亡回避を強化する必要がある。
- RuleAI hazard modeは2seed平均でalive ratioを0.85から0.867へ上げ、Big Star接近スコアも改善したが、seed別には `0x2f52869f` がdeath 1 -> 0、`0x13579bdf` がdeath 2 -> 3 と割れている。単純なhazard回避だけでは安定しないため、複数hazard/着地/壁/穴を同時に扱う必要がある。
...
- 最優先: 2seed評価で見えた死亡/不安定さを潰す。`0x2f52869f` のImitationAI死亡frame 1470、`0x13579bdf` のImitationAI star 0 / approach悪化、RuleAI hazard mode後の `0x13579bdf` death 3をSVG/viewerで確認し、hazard相対速度、着地予測、壁詰まり時の切り返し、複数hazardのnearest選択を改善する。その後 `run-nsmb-mvl-ai-closed-loop-batch.ps1` でseed数を3-5本へ増やす。
