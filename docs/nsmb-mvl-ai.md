...
- 完了: `specialObjects.fireballs.slots[]` にactive slotの `stateBytes` (`+0x80` から12 bytes) と `debugWords` (`+0x40` から16 words) を追加した。owner/lifetime/当たり判定状態はまだ未検証なので、Fire Mario実ログで値変化を画面と照合するためのraw監査情報として扱う。datasetには近傍fireballの代表byte/wordを追加した。
...
- 完了: `specialObjects.fireballs.slots[]` のowner推定をslot単位で追跡するようにした。spawn直後など位置/速度ベース推定の信頼度が高いframeでownerを記録し、同じactive slotが続く間は `ownerTracked=1` として維持する。真owner offsetは未確定なので `ownerVerified=0` は継続する。
...
`specialObjects` には、通常のactor object list外で管理されるものを保存する。現時点では `fireballs.active`、`fireballs.activeSlots`、`fireballs.handler`、`fireballs.handlerPtr`、`fireballs.words[]`、`fireballs.slots[]`、`projectiles.handler`、`projectiles.words[]` を持つ。`fireballs.active` は `Fireballs::activeFireballs` のraw wordで、kind別countがpackedされる可能性があるため、学習側ではactive slot件数の `activeSlots` / `slots.length` も併用する。handler addressはNSMB symbol table上の `Fireballs::fireballHandler` / `Projectiles::projectileHandler` に対応する。`fireballs.slots[]` は `FireballHandler::spawn/update` の逆アセンブルに基づき、handler pointer + 4 を起点に16 slot、stride `0x8C`、active byte `+0x80`、kind `+0x81`、state `+0x83`、facing `+0x85`、pos `+0x10`、prev pos `+0x20`、velocity `+0x30` として読んでいる。active slotでは `stateBytesOffset=0x80` / `stateBytes[]` と `debugWordsOffset=0x40` / `debugWords[]` も保存する。`statelessOwnerCandidate` / `statelessOwnerConfidence` はそのframe単体の位置/速度推定、`ownerCandidate` / `ownerConfidence` / `ownerTracked` はslot継続中の追跡込み推定を示す。真owner offset未確定のため `ownerVerified=0` のまま保存する。owner/lifetimeの正確なoffset、当たり判定状態、kind/state/facingの意味は、Fire Mario stage 0実ログでraw値変化と画面表示を照合して検証する。
...
- fireballなどのprojectile系objectは、通常actor object listではなく専用handlerで管理されるものがある。Fireball slot別の座標/前フレーム座標/速度/kind/state/facing、frame単体owner候補、slot追跡込みowner候補は取れるようになったが、真のowner offset、寿命、当たり判定状態、kind/state/facingの意味はまだFire Mario実ログで未検証。Fireball owner推定と相対Xは横ラップ補正を入れたので、次のFireログで `ownerTracked` と画面上のownerを再照合し、可能ならowner推定をheuristicからverified offsetへ置き換える必要がある。
...
- `logs\codex-visual-powerup-fields-scripted-20260608`: `tests\nsmb_us_direct_mvl_luigi_star_right.inputs` で2200Fを走らせ、後処理manifest/index/dataset/visual audit/recording auditまでpass。dataset 1332行、`self_visual_powerup_kind_candidate`、`self_visual_powerup_source_mask`、`self_can_shoot_fire_visual_candidate`、`nearest_fireball_owner_tracked` の列生成を確認。短いscripted smokeではfireballが出ないため、owner trackingの実戦品質は次のFire Mario実ログで検証する。
- 既存人間ログ `logs\nsmb-mvl-human-recording-stage0-20260608-012640` は新フィールド追加前のログとして後処理互換性を確認。Fire場面はあるが `ownerTracked` と `visualPowerupKindCandidate` は過去ログからは復元できないため、新ログで取り直す必要がある。
