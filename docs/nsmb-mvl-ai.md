...
- 完了: `scripts/nsmb_mvl_ai_evaluate_closed_loop.py` と `scripts/run-nsmb-mvl-ai-closed-loop-eval.ps1` を追加した。neutral / RuleAI / ImitationAIを同じstage 0 seedで実走し、AI play logまたはgame-state CSVから survival、death、Big Star距離、star/coin変化、item/projectile/fireball/hazard/block可視frame、入力分布、最終star差、heuristic scoreを `closed-loop-eval.json` に保存できる。
...
- 最優先: `run-nsmb-mvl-ai-closed-loop-eval.ps1` でneutral / RuleAI / ImitationAIを2600F以上、複数seedで順番に回し、`closed-loop-eval.json` を比較する。短時間smokeでは評価基盤の動作確認まで完了したが、AI品質判断には時間が短すぎる。
