...
- 最優先: `run-nsmb-mvl-ai-closed-loop-eval.ps1` でneutral / RuleAI / ImitationAIを複数seedで順番に回し、`closed-loop-eval.json` を比較する。初回2600FではImitationAIが1スター取れたが全policyで死亡が残った。hazard guardは入ったが1470死亡は未解決なので、次はhazard相対速度、着地予測、壁詰まり時の切り返し、または人間/RuleAIログ追加で改善する。
