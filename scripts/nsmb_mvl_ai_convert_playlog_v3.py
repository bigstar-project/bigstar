#!/usr/bin/env python3
"""Convert exact-hitbox NSMB MvL AI play log v1 records into observation v3 JSONL."""
...
def body_size_class(powerup_kind: int) -> int:
    """Return the stable size category: mini=0, small=1, large=2, mega=3."""
    if powerup_kind == legacy.ITEM_KIND_MINI_MUSHROOM:
        return 0
    if powerup_kind == legacy.ITEM_KIND_MEGA_MUSHROOM:
        return 3
    if powerup_kind in (
        legacy.ITEM_KIND_SUPER_MUSHROOM,
        legacy.ITEM_KIND_FIRE_FLOWER,
        legacy.ITEM_KIND_SHELL,
    ):
        return 2
    return 1
...
def compact_player(player: dict[str, Any]) -> dict[str, Any]:
    ...
    powerup_kind = legacy.visual_powerup_kind_candidate(player)
    return {
        ...
        "bodySize": {
            "class": body_size_class(powerup_kind),
            "source": "visualPowerupKind",
        },
        "hitbox": player.get("hitbox") or {"found": 0},
        "collisionSensors": ((player.get("collisionMgr") or {}).get("sensors")) or {},
        "visual": {
            "powerupKind": powerup_kind,
...
def scalar_features_for_player(record: dict[str, Any], player: int) -> dict[str, int]:
    ...
    self_hitbox = self_player.get("hitbox") or {}
    opponent_hitbox = opponent.get("hitbox") or {}
    return {
        ...
        "self_powerup_kind": self_powerup_kind,
        "self_body_size_class": body_size_class(self_powerup_kind),
        "self_hitbox_found": legacy.num(self_hitbox.get("found")),
        "self_hitbox_center_offset_x": legacy.num(self_hitbox.get("centerOffsetX")),
        "self_hitbox_center_offset_y": legacy.num(self_hitbox.get("centerOffsetY")),
        "self_hitbox_half_width": legacy.num(self_hitbox.get("halfWidth")),
        "self_hitbox_half_height": legacy.num(self_hitbox.get("halfHeight")),
        ...
        "opponent_powerup_kind": opponent_powerup_kind,
        "opponent_body_size_class": body_size_class(opponent_powerup_kind),
        "opponent_hitbox_found": legacy.num(opponent_hitbox.get("found")),
        "opponent_hitbox_center_offset_x": legacy.num(opponent_hitbox.get("centerOffsetX")),
        "opponent_hitbox_center_offset_y": legacy.num(opponent_hitbox.get("centerOffsetY")),
        "opponent_hitbox_half_width": legacy.num(opponent_hitbox.get("halfWidth")),
        "opponent_hitbox_half_height": legacy.num(opponent_hitbox.get("halfHeight")),
...
def compact_record(
    ...
    *,
    ...
    players = record.get("players") or []
    if len(players) < 2 or any(legacy.num((player.get("hitbox") or {}).get("found")) != 1 for player in players[:2]):
        raise ValueError(
            "observation v3 requires live exact hitboxes for both players; "
            "v1/v2 records without hitbox data cannot be upgraded"
        )
    labels = {f"player{player}": player_label(record, player, label_source) for player in (0, 1)}
    compact = {
        "schema": "nsmb_mvl_compact_observation_v3",
...
def main() -> int:
    ...
    parser.add_argument("output", type=Path, help="compact observation v3 JSONL(.gz)")
